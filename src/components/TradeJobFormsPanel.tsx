...
export function TradeJobFormsPanel({ user, workOrderId, readOnly = false }: { user: User; workOrderId: string; readOnly?: boolean }) {
  ...
  return <div className="crm-job-forms">
    <header><div><span>Versioned field records</span><h4>{readOnly ? "Review completed field forms" : "Complete the right form without paperwork hunting"}</h4></div><small>These forms organise technical job evidence. They do not replace licences, permits, formal certificates, standards or scheme documents required for the actual work.</small></header>
    {!readOnly && <section className="crm-form-library"><div><strong>Available for this work type</strong><span>{(result.templates || []).length} supporting form{(result.templates || []).length === 1 ? "" : "s"}</span></div><div>{(result.templates || []).map((template) => {
