...
export function TradePaymentPanel({ user, workOrderId, isProtected, suggestedAmountCents, onOpenIntegrations, onChanged }: { user: User; workOrderId: string; isProtected: boolean; suggestedAmountCents: number; onOpenIntegrations?: () => void; onChanged?: () => Promise<void> }) {
  ...
  const load = useCallback(async () => {
    ...
    setLinks((result.paymentLinks || []).filter((link) => link.workOrderId === workOrderId && link.purpose === "deposit"));
  }, [user, workOrderId]);
  ...
  async function createLink(provider: "stripe" | "square") {
    ...
    try {
      ...
      const response = await fetch("/api/trade-payment-links", {
        ...
        body: JSON.stringify({ provider, workOrderId }),
      });
  ...
  const depositRequested = links.length > 0;
  return <section className="crm-payment-panel">
    <header><div><span>Deposit request</span><h4>Secure provider checkout</h4><p>Request the agreed deposit through Stripe or Square. Card data stays with the payment provider.</p></div></header>
    <div className="crm-payment-create"><div><span>Deposit to request</span><strong>{new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD" }).format(suggestedAmountCents / 100)}</strong><small>{depositRequested ? "The existing provider request stays authoritative." : "Locked to the accepted quote handoff."}</small></div><button type="button" disabled={depositRequested || Boolean(busy) || suggestedAmountCents < 100} onClick={() => stripeConnected ? void createLink("stripe") : onOpenIntegrations?.()}>{depositRequested ? "Deposit already requested" : busy === "stripe" ? "Creating..." : stripeConnected ? "Request with Stripe" : "Connect Stripe"}</button><button type="button" disabled={depositRequested || Boolean(busy) || suggestedAmountCents < 100} onClick={() => squareConnected ? void createLink("square") : onOpenIntegrations?.()}>{depositRequested ? "Deposit already requested" : busy === "square" ? "Creating..." : squareConnected ? "Request with Square" : "Connect Square"}</button><button type="button" className="crm-payment-refresh" disabled={Boolean(busy)} onClick={() => void load().then(async () => { setStatus("Payment status refreshed from the verified provider ledger."); await onChanged?.(); }).catch((error) => setStatus(error instanceof Error ? error.message : "Payment status could not be refreshed."))}>Refresh status</button></div>
