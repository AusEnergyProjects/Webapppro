...
type PaymentLink = { id: string; workOrderId: string; provider: string; amountCents: number; checkoutUrl: string; status: string; createdAt: string };
...
export function TradePaymentPanel({ user, workOrderId, isProtected, suggestedAmountCents }: { user: User; workOrderId: string; isProtected: boolean; suggestedAmountCents: number }) {
  ...
  return <section className="crm-payment-panel">
    ...
    <div className="crm-payment-create"><label><span>Amount to collect</span><input type="number" min="1" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><button type="button" disabled={!stripeConnected || Boolean(busy)} onClick={() => void createLink("stripe")}>{busy === "stripe" ? "Creating..." : stripeConnected ? "Create Stripe link" : "Connect Stripe first"}</button><button type="button" disabled={!squareConnected || Boolean(busy)} onClick={() => void createLink("square")}>{busy === "square" ? "Creating..." : squareConnected ? "Create Square link" : "Connect Square first"}</button></div>
    {links.length > 0 && <ol>{links.map((link) => <li key={link.id}><div><span>{link.provider} | {new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD" }).format(link.amountCents / 100)}</span><small>Created {new Date(link.createdAt).toLocaleString("en-AU", { dateStyle: "medium", timeStyle: "short" })}</small></div><a href={link.checkoutUrl} target="_blank" rel="noreferrer">Open checkout</a></li>)}</ol>}
