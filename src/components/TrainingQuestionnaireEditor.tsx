...
export function TrainingQuestionnaireEditor({ api, canEdit, onDirtyChange }: { api: Api; canEdit: boolean; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  function create() {
    ...
    const next = newQuestionnaire(); next.assignment.serviceCategory = serviceFilter;
  ...
  const visibleModules = catalogue?.modules.filter((item) => (!serviceFilter || item.assignment.serviceCategory === serviceFilter) && `${item.title} ${item.programCode} ${item.id} ${serviceLabel(item.assignment.serviceCategory)}`.toLowerCase().includes(search.trim().toLowerCase())) || [];
  ...
  return <div className={styles.shell}>
    ...
    {view === "questions" && catalogue && <>
      <section className={styles.panel} aria-label="Choose an activity"><div className={styles.fields}><label className={styles.field}>Find an activity<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Activity number, service, name or program" /></label><label className={styles.field}>Filter by service category<select value={serviceFilter} onChange={(event) => setServiceFilter(event.target.value)}><option value="">All service categories</option>{catalogue.services.map((service) => <option key={service.id} value={service.id}>{service.label} ({catalogue.modules.filter((item) => item.assignment.serviceCategory === service.id).length})</option>)}</select></label><label className={styles.field}>Questionnaire<select value={visibleModules.some((item) => item.id === course?.id) ? course?.id : ""} disabled={Boolean(busy)} onChange={(event) => { if (event.target.value) void loadModule(event.target.value); }}><option value="">Choose an activity</option>{catalogue.services.map((service) => { const modules = visibleModules.filter((item) => item.assignment.serviceCategory === service.id); return modules.length > 0 ? <optgroup key={service.id} label={service.label}>{modules.map((item) => <option key={item.id} value={item.id}>{item.title}{item.hasDraft ? " · draft changes" : ""}</option>)}</optgroup> : null; })}</select></label></div><small>{visibleModules.length} questionnaires found. Saved drafts are private until you publish them.</small></section>
