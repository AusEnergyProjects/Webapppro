...
const presets: Array<[ReportPreset, string]> = [["weekly", "Weekly"], ["monthly", "Monthly"], ["quarterly", "Quarterly"], ["fytd", "Financial year to date"], ["custom", "Custom dates"]];
type ReportResponse = { ok?: boolean; report?: BusinessReport; error?: string };

export function TradeBusinessReports({ user, onOpenJobs, onOpenSchedule, onOpenInvoices }: {
  user: User; onOpenJobs: () => void; onOpenSchedule?: () => void; onOpenInvoices?: () => void;
}) {
  ...
  const requestKey = [user.uid, preset, anchor, service, state, range.from, range.to, refresh].join("|");
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ period: preset, service, state });
  ...
  }, [user, preset, anchor, service, state, range.from, range.to, requestKey]);
  ...
  function applyCustom(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setRange({ from, to }); setPreset("custom"); setAnchor(""); }
  ...
  return <section className={styles.reports} aria-label="Business reports">
    ...
    <div className={styles.controls}>
      ...
      {custom && <form className={styles.custom} onSubmit={applyCustom}><label>From<input type="date" required value={from} max={to || report?.period.today} data-date-range-group="business-reports" data-date-range-role="start" onChange={event => setFrom(event.target.value)} /></label><label>To<input type="date" required value={to} min={from} max={report?.period.today} data-date-range-group="business-reports" data-date-range-role="end" onChange={event => setTo(event.target.value)} /></label><button type="submit">Apply dates</button><small>Up to 366 days</small></form>}
      <div className={styles.filters}><label>Service<select value={service} onChange={event => setService(event.target.value)}><option value="">All services</option>{report?.options.services.map(key => <option key={key} value={key}>{services[key] || key}</option>)}</select></label><label>Job region<select value={state} onChange={event => setState(event.target.value)}><option value="">All regions</option>{report?.options.states.map(key => <option key={key} value={key}>{key === "unknown" ? "Region not recorded" : key}</option>)}</select></label>{(service || state) && <button type="button" onClick={() => { setService(""); setState(""); }}>Clear filters</button>}</div>
    </div>
    ...
    {!loading && !error && report && current && previous && <>
      <div className={styles.period}><div><strong>{date(report.period.start)} to {date(report.period.end)}</strong><small>Compared with {date(report.period.previous.start)} to {date(report.period.previous.end)} · {report.period.timeZone.replace("Australia/", "")} time</small></div>{preset !== "custom" && <div className={styles.actions}><button type="button" aria-label="Previous reporting period" onClick={() => setAnchor(report.period.previousAnchor)}>Previous</button><button type="button" onClick={() => setAnchor("")} disabled={!anchor}>Current</button><button type="button" aria-label="Next reporting period" disabled={!report.period.nextAnchor} onClick={() => setAnchor(report.period.nextAnchor || "")}>Next</button></div>}</div>
      <div className={styles.metrics}>
        {current.invoicedCents !== null && <Metric label="Net TLink invoicing" value={money(current.invoicedCents)} detail="Issued invoices less credits, ex GST" comparison={reportChange(current.invoicedCents, previous.invoicedCents || 0)} />}
        <Metric label="New jobs" value={String(current.newJobs)} detail="Jobs created in this period" comparison={reportChange(current.newJobs, previous.newJobs)} />
        <Metric label="Jobs completed" value={String(current.completedJobs)} detail="Recorded job completions" comparison={reportChange(current.completedJobs, previous.completedJobs)} />
        {current.wonCents !== null && <Metric label="Work won" value={money(current.wonCents)} detail={`${current.wonQuotes} accepted quote decisions, ex GST`} comparison={reportChange(current.wonCents, previous.wonCents || 0)} />}
        {current.wonQuotes !== null && <Metric label="Quote decision win rate" value={winRate === null ? "No decisions" : `${winRate.toFixed(0)}%`} detail={`${current.wonQuotes} accepted · ${current.declinedQuotes} declined`} comparison={previousWinRate === null ? "No previous decisions" : `Previous: ${previousWinRate.toFixed(0)}%`} />}
        <Metric label="Booked hours" value={hours(current.bookedMinutes)} detail={`${current.visits} visits · ${current.completedVisits} completed`} comparison={reportChange(current.bookedMinutes, previous.bookedMinutes)} />
      </div>
      <div className={styles.twoColumns}>
        <section className={styles.card} aria-label="Performance trend"><header><div><h4>Performance over time</h4><p>{chartIsMoney ? "Net TLink invoicing, excluding GST" : "Jobs created"}</p></div><div className={styles.actions}><button type="button" aria-pressed={!chartIsMoney} onClick={() => setChart("jobs")}>Jobs</button>{report.permissions.invoices && <button type="button" aria-pressed={chartIsMoney} onClick={() => setChart("invoices")}>Invoicing</button>}</div></header>
          ...
          <details><summary>View exact figures</summary><div className={styles.tableScroll}><table><thead><tr><th>Period</th><th>New jobs</th><th>Completed</th>{report.permissions.invoices && <th>Net invoicing ex GST</th>}</tr></thead><tbody>{trend.map(bucket => <tr key={bucket.start}><th>{shortDate(bucket.start)}{bucket.start !== bucket.end ? ` to ${shortDate(bucket.end)}` : ""}</th><td>{bucket.newJobs}</td><td>{bucket.completedJobs}</td>{bucket.invoicedCents !== null && <td>{exactMoney(bucket.invoicedCents)}</td>}</tr>)}</tbody></table></div></details>
        </section>
...
function Metric({ label, value, detail, comparison }: { label: string; value: string; detail: string; comparison: string }) {
  return <article className={styles.metric}><span>{label}</span><strong>{value}</strong><small>{detail}</small><p>{comparison}</p></article>;
}
