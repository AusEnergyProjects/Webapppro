  ...
  return <section className={styles.workspace} aria-label="Certificate submissions" aria-busy={loading || locked}>
    ...
    {loading && !workspace ? <p className={styles.empty}>Loading certificate submissions…</p> : workspace && <>
      ...
      <nav className={styles.navigation} aria-label="Submission workspace views">
        {([ ["claims", "Claims"], ["files", "Registry files"], ["accounts", "Scheme accounts"], ["fees", "Fees and payments"] ] as const).map(([key, label]) => <button key={key} type="button" aria-pressed={view === key} disabled={locked} onClick={() => setView(key)}>{label}</button>)}
      </nav>
      ...
      {view === "files" && <div className={styles.section}>
        <header className={styles.sectionHeading}><div><h3>Official registry files</h3><p>Create a CSV from approved claims, check its fields and retain an independent review before lodging through the official portal.</p></div></header>
        ...
        {workspace.capabilities.canOperate && exportAccount && exportFormat ? <form className={styles.form} onSubmit={prepareExport}>
        ...
        {workspace.exports.map((item) => <article className={styles.card} key={item.id}>
