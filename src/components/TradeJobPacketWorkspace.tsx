...
export function TradeJobPacketWorkspace({ user, onOpenItems }: { user: User; onOpenItems: () => void }) {
  ...
  const request = useCallback(async (category: string, init: RequestInit = {}) => {
    ...
    if (!response.ok || result.ok === false) throw new Error(result.error || "The job packets could not be loaded."); return result;
  }, [user]);
  ...
  } catch (error) { setMessage(error instanceof Error ? error.message : "The packet could not be saved."); } finally { setBusy(""); } }
  async function archive(packet: JobPacket) { if (!window.confirm(`Archive ${packet.name}? Existing quote snapshots stay unchanged.`)) return;
    setBusy(`archive:${packet.id}`); try { await request(packet.serviceCategory, { method: "PATCH", body: JSON.stringify({ action: "archive", packetId: packet.id }) }); await load(packet.serviceCategory); setEditing(null); setMessage("Packet archived. Existing quote snapshots remain unchanged."); }
    catch (error) { setMessage(error instanceof Error ? error.message : "The packet could not be archived."); } finally { setBusy(""); } }

  return <section className={styles.workspace} aria-labelledby="job-packets-title">
    <header className={styles.hero}><div><span>Repeatable scope</span><h3 id="job-packets-title">Job packets</h3><p>Bundle the items, checklist and forms once, then start a standard quote in one choice.</p></div><button type="button" onClick={startNew} disabled={!priceItems.length}>New packet</button></header>
    {!loading && !priceItems.length && <section className={styles.firstRun}><span>One small setup step</span><h4>Add a price-book item first</h4><p>A packet reuses your saved labour and materials, so prices stay accurate without entering them again.</p><button type="button" onClick={onOpenItems}>Open price-book items</button></section>}
    {!loading && priceItems.length > 0 && !packets.length && !editing && <section className={styles.firstRun}><span>Start in under a minute</span><h4>Build the service you quote most often</h4><p>Name it, choose its saved items and quantities, then mark it ready. Tasks, forms and crew detail remain optional.</p><button type="button" onClick={startNew}>Create first job packet</button></section>}
    {editing && <form className={styles.editor} onSubmit={save}><header><div><span>{editing === "new" ? "Fast standard scope" : editing.packetCode}</span><h4>{editing === "new" ? "New job packet" : `Edit ${editing.name}`}</h4><p>The essential path is name, service and saved items. Everything else is optional.</p></div><button type="button" onClick={() => setEditing(null)}>Back to packets</button></header>
      {editing !== "new" && editing.recordStatus === "archived" && <p className={styles.warning}>Archived packets are read only.</p>}
      {editing !== "new" && editing.unavailableItemCount > 0 && <p className={styles.warning}>{editing.unavailableItemCount} saved item is unavailable. Choose its active replacement before marking this packet ready.</p>}
      <fieldset disabled={editing !== "new" && editing.recordStatus === "archived"}><div className={styles.core}>
        <label><span>Packet name</span><input required maxLength={140} value={draft.name} onChange={(event) => change("name", event.target.value)} placeholder="e.g. Standard heat pump install" /></label>
        ...
        {draft.lines.map((line, index) => <div className={styles.line} key={`${index}:${line.id || "new"}`}><select aria-label={`Packet item ${index + 1}`} value={line.priceBookItemId} onChange={(event) => updateLine(index, "priceBookItemId", event.target.value)}>{priceItems.filter((item) => item.id === line.priceBookItemId || !draft.lines.some((existing) => existing.priceBookItemId === item.id)).map((item) => <option key={item.id} value={item.id}>{item.name} | {money(item.sellPriceCentsExGst)} / {item.unitLabel}</option>)}</select><label><span>Quantity</span><input aria-label={`Packet item ${index + 1} quantity`} required inputMode="decimal" value={line.quantity} onChange={(event) => updateLine(index, "quantity", event.target.value)} /></label><button type="button" onClick={() => change("lines", draft.lines.filter((_, position) => position !== index))}>Remove</button></div>)}
      ...
      {(editing === "new" || editing.recordStatus !== "archived") && <div className={styles.actions}><button type="submit" disabled={Boolean(busy) || !draft.lines.length}>{busy === "save" ? "Saving..." : draft.recordStatus === "active" ? "Save ready packet" : "Save draft"}</button>{editing !== "new" && <button type="button" className={styles.danger} disabled={Boolean(busy)} onClick={() => void archive(editing)}>Archive packet</button>}</div>}
    </form>}
    ...
    {loading && <p className={styles.loading}>Loading job packets...</p>}{message && <p className={styles.message} role="status">{message}</p>}
  </section>;
}
