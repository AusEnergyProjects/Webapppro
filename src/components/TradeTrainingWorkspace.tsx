...
export function TradeTrainingWorkspace({ user }: { user: User }) {
  ...
  const matchingModules = (data?.modules || []).filter((module) => (!programFilter || moduleProgram(module) === programFilter)
    && `${module.title} ${module.id} ${module.activityTemplateIds.join(" ")}`.toLowerCase().includes(moduleSearch.trim().toLowerCase()));
  const matchingUnavailable = (data?.unavailableActivities || []).filter((item) => (!programFilter || item.programCode.replaceAll("-", " ") === programFilter)
    && `${item.title} ${item.id} ${item.programCode} ${item.serviceCategory}`.toLowerCase().includes(moduleSearch.trim().toLowerCase()));

  return <div className={styles.shell}>
    <header className={styles.hero}><span className={styles.eyebrow}>Your compliance to-do list</span><h2>Training &amp; onboarding</h2><p>Complete business onboarding and your own activity training. A selected work type becomes eligible for government-program work only after the required checks pass.</p><span>{passed} of {data?.modules.length || 0} activity modules current</span></header>
    <TradeCreditexOnboarding user={user} />
    {error && <p className={`${styles.notice} ${styles.error}`} role="alert">{error}</p>}
    {!data && !error && <p role="status">Loading your activity training...</p>}
    {data && <section className={styles.panel} aria-label="Your activity modules"><header><div><h3>Your activity modules</h3><p className={styles.muted}>Each person completes their own assessment. Passing records learning completion; Creditex approval, licences and job evidence remain required.</p></div><button className={styles.secondary} type="button" disabled={Boolean(busy)} onClick={() => { setError(""); void load().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Status could not be refreshed.")); }}>Refresh status</button></header>
      <div className={styles.fields}><label className={styles.field}>Find an activity<input type="search" value={moduleSearch} placeholder="Activity number, program or work type" onChange={(event) => { setModuleSearch(event.target.value); setVisibleModuleCount(12); }} /></label><label className={styles.field}>Program<select aria-label="Program" value={programFilter} onChange={(event) => { setProgramFilter(event.target.value); setVisibleModuleCount(12); }}><option value="">All programs</option>{programs.map((program) => <option key={program} value={program}>{program}</option>)}</select></label></div><small>Showing {Math.min(visibleModuleCount, matchingModules.length)} of {matchingModules.length} matching activities.</small><div className={styles.grid}>{matchingModules.slice(0, visibleModuleCount).map((module) => <article className={styles.card} key={module.id}><header><div><small>{module.activityTemplateIds.join(" · ")}</small><h4>{module.title}</h4></div><span className={`${styles.badge} ${module.status === "passed" ? styles.passed : ""}`}>{module.status === "passed" ? "✓ Passed" : label(module.status)}</span></header><p className={styles.muted}>{module.estimatedMinutes} minutes · Version {module.version} · Pass mark {module.passPercent}%</p>{module.completion && module.status === "passed" && <div className={styles.completion}><strong>Learning completion reference</strong><code>{module.completion.reference}</code><small>Valid until {date(module.completion.expiresAt)}</small></div>}{module.availability !== "active" && <p className={styles.muted}>{module.availability === "awaiting_review" ? "Creditex must review and activate this exact curriculum before assessment is available." : "This curriculum is unavailable. Government-program booking stays locked until a current module is available."}</p>}<button type="button" className={styles.secondary} onClick={() => openModule(module)} disabled={Boolean(busy)}>{module.status === "passed" ? "Review learning material" : "Open learning material"}</button></article>)}</div>
      ...
      {data.modules.length > 0 && !matchingModules.length && <p>No activity matches these filters. Change the search or program.</p>}
