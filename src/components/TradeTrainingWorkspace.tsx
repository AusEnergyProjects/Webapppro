...
type TeamProgress = { memberId: string; displayName: string; modules: { id: string; title: string; serviceCategory?: string; status: string; reference?: string; expiresAt?: string }[] };
...
type TrainingResult = ApiResult & { business: BusinessStatus; memberId: string; trainingServiceStates?: string[]; modules: Module[]; team?: TeamProgress[]; unavailableActivities?: UnavailableActivity[]; submissions?: SavedSubmission[] };
...
export function TradeCreditexOnboarding({ user, initialExpanded = false, businessName = "", businessAddress = "" }: { user: User; initialExpanded?: boolean; businessName?: string; businessAddress?: string }) {
  ...
  return <section className={`${styles.shell} ${styles.panel}`} aria-label="Creditex business onboarding">
    ...
    {data?.business && <><ol className={styles.steps}>{data.business.blockedReasons.map((reason) => <li key={reason}>{reason}</li>)}</ol>{setupComplete && <div className={styles.completion}><strong>Business setup complete</strong>{data.business.completionReference && <code>{data.business.completionReference}</code>}{data.business.completedAt && <small>Completed {date(data.business.completedAt)}</small>}<p>Each person still needs current activity training, licences and compliance evidence.</p></div>}</>}
...
export function TradeTrainingWorkspace({ user }: { user: User }) {
  ...
  const matchingModules = (data?.modules || []).filter((module) => (statusFilter === "all" || (statusFilter === "passed" ? module.status === "passed" : module.status !== "passed")) && (!programFilter || moduleProgram(module) === programFilter)
    && (!serviceFilter || module.serviceCategory === serviceFilter)
    && `${module.title} ${module.id} ${module.activityTemplateIds.join(" ")} ${serviceLabel(module.serviceCategory)}`.toLowerCase().includes(moduleSearch.trim().toLowerCase()));
  const matchingUnavailable = (data?.unavailableActivities || []).filter((item) => statusFilter !== "passed" && (!programFilter || item.programCode.replaceAll("-", " ") === programFilter)
    ...
    && `${item.title} ${item.id} ${item.programCode} ${serviceLabel(item.serviceCategory)}`.toLowerCase().includes(moduleSearch.trim().toLowerCase()));

  const assignedServices = new Set([...(data?.modules || []), ...(data?.unavailableActivities || [])].map((item) => item.serviceCategory || ""));
  const serviceGroups = [...ENERGY_SERVICE_CATALOGUE.filter((service) => assignedServices.has(service.id)),
    ...(assignedServices.has("") ? [{ id: "", label: serviceLabel() }] : [])].map((service) => ({
      ...service,
      modules: matchingModules.filter((module) => (module.serviceCategory || "") === service.id),
      unavailable: matchingUnavailable.filter((item) => item.serviceCategory === service.id),
      total: (data?.modules || []).filter((module) => (module.serviceCategory || "") === service.id).length,
      passed: (data?.modules || []).filter((module) => (module.serviceCategory || "") === service.id && module.status === "passed").length,
    })).filter((group) => group.modules.length || group.unavailable.length);
  return <div className={styles.shell}>
    <header className={styles.hero}><span className={styles.eyebrow}>Your compliance to-do list</span><h2>To do &amp; training</h2><p>Saving your services assigns the relevant activity modules here. Complete your own learning and assessments to qualify for government-program work, alongside business setup, current insurance and required credentials.</p>{data && <><strong>{pending} training {pending === 1 ? "task" : "tasks"} to do</strong><span>{passed} of {data.modules.length} activity modules passed</span></>}</header>
    {data?.trainingServiceStates?.length ? <p className={styles.muted}>Training for {data.trainingServiceStates.join(", ")}, plus relevant national programs. Change states in Business &gt; Services and areas.</p> : null}
    <TradeCreditexOnboarding user={user} />
    {error && <p className={`${styles.notice} ${styles.error}`} role="alert">{error}</p>}
    {!data && !error && <p role="status">Loading your activity training...</p>}
    {data && <section className={styles.panel} aria-label="Your activity modules"><header><div><h3>Your training to-do list</h3><p className={styles.muted}>Each person completes their own assessment. Passing records learning completion; business setup, current insurance, licences and job evidence remain required.</p></div><button className={styles.secondary} type="button" disabled={Boolean(busy)} onClick={() => { setError(""); void load().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Status could not be refreshed.")); }}>Refresh status</button></header>
      <div className={styles.fields}><label className={styles.field}>Find an activity<input type="search" value={moduleSearch} placeholder="Activity number, program or work type" onChange={(event) => { setModuleSearch(event.target.value); setVisibleModuleCount(12); }} /></label><label className={styles.field}>Service category<select aria-label="Service category" value={serviceFilter} onChange={(event) => { setServiceFilter(event.target.value); setVisibleModuleCount(12); }}><option value="">All assigned services</option>{ENERGY_SERVICE_CATALOGUE.filter((service) => assignedServices.has(service.id)).map((service) => <option key={service.id} value={service.id}>{service.label}</option>)}</select></label><label className={styles.field}>Program<select aria-label="Program" value={programFilter} onChange={(event) => { setProgramFilter(event.target.value); setVisibleModuleCount(12); }}><option value="">All programs</option>{programs.map((program) => <option key={program} value={program}>{program}</option>)}</select></label><label className={styles.field}>Status<select aria-label="Training status" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setVisibleModuleCount(12); }}><option value="todo">To do</option><option value="passed">Passed</option><option value="all">All modules</option></select></label></div>
      ...
      <small>{matchingModules.length + matchingUnavailable.length} matching activities across {serviceGroups.length} service categories.</small>
    ...
    {selected && <section className={styles.panel} aria-label={`${selected.title} learning and assessment`}><header><div><span className={styles.eyebrow}>{selected.activityTemplateIds.join(" · ")}</span><h3 ref={lessonHeading} tabIndex={-1}>{selected.title}</h3><p className={styles.muted}>Service: {serviceLabel(selected.serviceCategory)} · Required for this activity under {moduleProgram(selected)}</p></div><span className={styles.badge}>Version {selected.version}</span></header>
