...
export function DirectTradePartnerForm() {
  ...
  return <main className="wrap direct-trade-request-page">
    ...
    {!authReady ? <section className="trade-auth-card trade-auth-loading" aria-live="polite"><span className="trade-auth-loader" aria-hidden="true" /><div><strong>Checking your secure sign-in</strong><p>Loading account options...</p></div></section> : !user ? <section className="trade-auth-card" aria-labelledby="trade-account-title">
      ...
      {profileSaved && partnerType === "installer" && <section className="direct-trade-form-section" aria-label="Step 5 Creditex onboarding"><div className="direct-trade-form-heading"><span>Step 5</span><h2>Complete Creditex onboarding</h2><p>Provide the business details, required private documents and declarations to complete Creditex setup. The business owner and the technician carrying out the work complete the required activity training before certificate-program bookings. Office-only staff need no installation modules. Training does not stop your business receiving TLink leads.</p></div><TradeCreditexOnboarding user={user} initialExpanded businessName={businessName} businessAddress={[addressLine1, suburb, addressState, postcode].filter(Boolean).join(", ")} /><p><a href="/direct-trade/dashboard?workspace=training">Open your training and onboarding to-do list</a></p></section>}
    </>}
