...
const providerKeys = new Set(["xero", "myob", "quickbooks"]);
...
export function TradeQuickInvoiceStep({ user, active, busy, customerName, deliveryEmail, onBack }: { user: User; active: boolean; busy: boolean; customerName: string; deliveryEmail: string; onBack: () => void }) {
  ...
  return <section data-step="6" hidden={!active} className="crm-wizard-panel">
    ...
    {mode === "send" && <>
      ...
      {connected.length > 0 ? <div className="crm-connected-provider-note"><strong>Connected</strong><span>{connected.map((provider) => provider.label).join(", ")}. Connection prompts stay hidden while an accounting provider is connected.</span></div> : <details className="crm-invoice-connect-prompt"><summary>Connect accounting, optional</summary><p>TLink can send this invoice without a connection. Connect an accounting service now if you also want it available in your invoice workspace.</p>{configured.length > 0 ? <div>{configured.map((provider) => <button type="button" key={provider.provider} onClick={() => void connect(provider)}>Connect {provider.label}</button>)}</div> : <small>TLink is preparing secure accounting connections. You can still send the TLink invoice.</small>}</details>}
    </>}
