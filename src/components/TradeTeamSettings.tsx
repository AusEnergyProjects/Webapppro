...
export type TradeTeamMember = {
  ...
  assignedServiceStates?: string[] | null;
  lastActiveAt?: string;
  updatedAt: string;
...
type MemberFile = {
  ...
  expiresAt: string;
  category: string;
  expiryStatus: "no_expiry" | "expired" | "expiring" | "current";
  createdAt: string;
  credential: null | {
    id: string;
    type: string;
...
export function TradeTeamSettings({ user, navigationTarget, onOpenOwnTraining }: { user: User; navigationTarget?: TLinkCommandTarget | null; onOpenOwnTraining?: () => void }) {
  ...
  const openFiles = useCallback(async (member: TradeTeamMember) => {
    ...
    setMenu(null); setFilesMember(member); setFilesLoading(true); setFiles([]); setUploadRentalGate(""); setUploadCategory("other"); setError("");
  ...
  async function saveMember(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!editedMember?.isOwner && memberServiceStates !== null && memberServiceStates.length === 0) {
        throw new Error("Choose at least one service region, or use all business service regions.");
      }
      ...
      if (isNew || !editedMember?.isOwner) Object.assign(body, {
        ...
        serviceStates: memberServiceStates,
      });
  ...
  async function copyFieldSetup() {
    ...
    const text = `TLink\nUsername: ${fieldSetup.username || fieldSetup.displayName}\nPIN: ${fieldSetup.pin}\nExpires: ${new Date(fieldSetup.expiresAt).toLocaleString("en-AU")}`;
    try { await navigator.clipboard.writeText(text); setMessage("TLink sign-in details copied."); }
    catch { setError("Copy was blocked. Select the username and PIN and copy them manually."); }
  }
  ...
  async function uploadFile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset(); setUploadRentalGate(""); setUploadCategory("other"); await load(); setMessage(result.file.credential ? "Credential and supporting file saved." : "Member file saved.");
  ...
  return <div className={styles.workspace}>
    ...
    {editing && <div className={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) closeMemberDialog(); }}><div ref={dialogRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="team-member-dialog-title" tabIndex={-1} onKeyDown={(event) => trapDialogKey(event, closeMemberDialog)}><header className={styles.dialogHeader}><div><span>{editing === "new" ? "Add team member" : "Edit team member"}</span><h4 id="team-member-dialog-title">Person and access</h4></div><button type="button" className={styles.iconButton} aria-label="Close" disabled={Boolean(busy)} onClick={closeMemberDialog}>X</button></header>
      <form className={styles.form} onSubmit={saveMember}>
        ...
        <fieldset className={styles.permissionGroup} disabled={Boolean(busy)}><legend>Services performed on site</legend><p className={styles.hint}>Choose only the work this person personally carries out on site. Leave all services unchecked for office-only staff. This does not change the services your business offers.</p><p className={styles.hint}>Office staff can book work for trained technicians when their access permissions allow it. The business and the technician must meet the activity requirements.</p><div className={styles.actions}><button type="button" className={styles.secondary} disabled={Boolean(busy) || memberServices.length === 0} onClick={() => setMemberServices([])}>Office only: no on-site services</button><small className={styles.hint}>Clears the selections below. Save changes to apply, or tick services again if this person also works on site.</small></div><div className={styles.grid}>{ENERGY_SERVICE_CATALOGUE.map((service) => <label className={styles.check} key={service.id}><input type="checkbox" checked={memberServices.includes(service.id)} onChange={(event) => setMemberServices((current) => event.target.checked ? [...new Set([...current, service.id])] : current.filter((id) => id !== service.id))} /><span>{service.label}</span></label>)}</div></fieldset>
        <fieldset className={styles.permissionGroup} disabled={Boolean(busy)}><legend>Service regions</legend>
          <p className={styles.hint}>Choose the states or territories where this person works. Their training will match these regions and their on-site services. A Victorian technician will not be assigned NSW training unless NSW is selected. National modules still apply.</p>
          <label className={styles.check}><input type="checkbox" checked={memberServiceStates === null} onChange={event => setMemberServiceStates(event.target.checked ? null : [...businessServiceStates])} /><span>All business service regions<small>Follows the business if its service regions change.</small></span></label>
          {memberServiceStates === null ? <p className={styles.hint}>Current regions: {businessServiceStates.join(", ") || "Set service locations in Business settings first."}</p> : <div className={styles.grid}>{[...new Set([...businessServiceStates, ...memberServiceStates])].map(state => <label className={styles.check} key={state}><input type="checkbox" checked={memberServiceStates.includes(state)} onChange={event => setMemberServiceStates(current => event.target.checked ? [...new Set([...(current || []), state])] : (current || []).filter(value => value !== state))} /><span>{state}{!businessServiceStates.includes(state) && <small>No longer served by the business. Remove this selection to save.</small>}</span></label>)}</div>}
          {memberServiceStates !== null && memberServiceStates.length === 0 && <p className={styles.hint}>Select at least one region before saving.</p>}
        </fieldset>
        {trainingTodos}
        {showAccessEditor ? <>
      ...
      <div className={styles.filesBody}><aside className={styles.filesSidebar}>
        <form className={styles.uploadForm} onSubmit={uploadFile}>
          ...
          <label>Credential use<select name="rentalGate" value={uploadRentalGate} onChange={(event) => setUploadRentalGate(event.target.value)}>
            ...
            <option value="licensed_electrician">Electrical safety-check licence</option>
            <option value="licensed_gasfitter">Gas safety-check licence or registration</option>
            <option value="licensed_plumber">Licensed plumber licence</option>
            <option value="registered_plumber">Registered plumber registration</option>
            <option value="refrigerant_handler">Refrigerant handling licence</option>
            <option value="suitably_qualified_smoke_alarm_worker">Smoke alarm qualification or training</option>
            <option value="sres_installer_accreditation">SRES installer accreditation</option>
            <option value="sres_designer_accreditation">SRES designer accreditation</option>
          </select></label>
          {!uploadRentalGate && <label>Document category<select name="category" value={uploadCategory} onChange={(event) => setUploadCategory(event.target.value)}>
            <option value="other">General document</option><option value="insurance">Insurance</option>
            <option value="id">Identification</option><option value="licence">Licence</option>
            <option value="compliance">Compliance document</option><option value="training">Training</option>
          </select></label>}
          <label>Title<input name="title" required maxLength={180} placeholder={!uploadRentalGate && uploadCategory === "insurance" ? "Public liability, professional indemnity or workers compensation" : isSresCredential ? "For example, SAA grid-connected installer accreditation" : "For example, Victorian electrical licence"} /></label>
          {uploadRentalGate && <>
            <label>Credential type<select key={uploadRentalGate} name="credentialType" required defaultValue={fixedCredentialType || (uploadRentalGate === "suitably_qualified_smoke_alarm_worker" ? "training" : "licence")}>
              {fixedCredentialType ? <option value={fixedCredentialType}>{fixedCredentialType === "accreditation" ? "Accreditation" : fixedCredentialType === "registration" ? "Registration" : "Licence"}</option> : <><option value="licence">Licence</option><option value="registration">Registration</option>{uploadRentalGate === "suitably_qualified_smoke_alarm_worker" && <option value="training">Training or qualification</option>}</>}
            </select></label>
            <label>Credential name<input name="credentialName" required maxLength={180} placeholder={isSresCredential ? "Include grid-connected or stand-alone type" : "Name shown on the credential"} /></label>
            <label>Credential number<input name="credentialNumber" required maxLength={120} /></label>
            <label>Issuer<input name="credentialIssuer" required maxLength={180} placeholder={isSresCredential ? "For example, Solar Accreditation Australia" : "Issuing authority or training provider"} /></label>
            <label>Jurisdiction<select key={uploadRentalGate} name="credentialJurisdiction" required defaultValue={isSresCredential ? "NATIONAL" : "VIC"}>{!isSresCredential && <option value="VIC">Victoria</option>}<option value="NATIONAL">National</option></select></label>
          </>}
          <label>{uploadRentalGate ? "Credential expiry" : uploadCategory === "insurance" ? "Insurance expiry" : "Expiry, optional"}<input type="date" name="expiresAt" required={Boolean(uploadRentalGate) || uploadCategory === "insurance"} /></label>
