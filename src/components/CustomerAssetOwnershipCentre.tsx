...
export function CustomerAssetOwnershipCentre({ user }: { user: User }) {
  ...
  if (loading) return <section className="customer-loading-state"><span /><div><strong>Opening your home asset library</strong><p>Checking ownership, protected documents and lifecycle records...</p></div></section>;

  return <section className="customer-assets-centre">
    <header className="customer-assets-hero"><div><span>Free household asset passport</span><h1>Your home products, documents and ownership record</h1><p>Keep approved installed assets for the life of the home. Transfer access with dual household consent without showing either household&apos;s identity to the other.</p></div><aside><strong>Privacy by design</strong><ul><li>One-time code stored only as a secure hash</li><li>No customer identity exchanged</li><li>Administrator review before access changes</li><li>Full history retained after every transfer</li></ul></aside></header>
    <section className="customer-asset-claim"><div><span>Moving into a home?</span><h2>Claim an existing asset record</h2><p>Ask the current household for its one-time transfer code. Both accounts must consent, then an administrator reviews the record.</p></div><form onSubmit={claimTransfer}><label><span>Transfer code</span><input name="claimCode" required autoComplete="off" placeholder="AEA-XXXX-XXXX-XXXX-XXXX" maxLength={40} /></label><label className="customer-asset-consent"><input type="checkbox" name="consent" /><span>I accept responsibility for this home asset record and understand access changes only after review.</span></label><button type="submit" disabled={busy === "claim"}>{busy === "claim" ? "Checking..." : "Accept transfer invitation"}</button></form></section>
    ...
    <div className="customer-asset-metrics"><article><strong>{packs.length}</strong><span>active asset records</span></article><article><strong>{packs.reduce((sum, pack) => sum + pack.assets.length, 0)}</strong><span>installed products</span></article><article><strong>{packs.reduce((sum, pack) => sum + pack.documents.length, 0)}</strong><span>protected documents</span></article><article><strong>{transfers.filter((item) => ["awaiting_recipient", "awaiting_admin"].includes(item.status)).length}</strong><span>active transfers</span></article></div>
    {!packs.length ? <section className="customer-empty-state"><span>Nothing to transfer yet</span><h2>Your approved asset records will appear here</h2><p>When an installer handover is reviewed and published, the installed products, documents, service schedules and transfer controls are added automatically.</p></section> : <div className="customer-owned-packs">{packs.map((pack) => <article key={pack.id} className="customer-owned-pack"><header><div><span>{pack.workNumber}</span><h2>{readable(pack.serviceCategory)}</h2><small>Ownership active since {dateLabel(pack.ownershipStartedAt)}</small></div><strong>{pack.sourceType === "transfer" ? "Transferred ownership" : "Original household"}</strong></header>
      ...
      <section className="customer-transfer-panel"><div><span>Home ownership change</span><h3>Transfer this complete asset record</h3><p>The new household receives the products, live documents, service schedules and safety notices only after dual consent and administrator approval. Your live access ends when the transfer completes.</p></div>{pack.activeTransfer ? <div className="customer-active-transfer"><strong>{readable(pack.activeTransfer.status)}</strong><p>{pack.activeTransfer.status === "awaiting_recipient" ? "Waiting for the new household to use the one-time code." : "Both households consented. Administrator review is pending."}</p><small>Invitation expires {dateLabel(pack.activeTransfer.expiresAt)}</small><button type="button" disabled={busy === `cancel:${pack.activeTransfer.id}`} onClick={() => void cancelTransfer(pack.activeTransfer!.id)}>Withdraw consent</button></div> : <form onSubmit={(event) => void createTransfer(event, pack.id)}><label className="customer-asset-consent"><input type="checkbox" name="consent" /><span>I intend to transfer this home asset record and understand my live access ends after approval.</span></label><button type="submit" disabled={busy === `create:${pack.id}`}>{busy === `create:${pack.id}` ? "Creating..." : "Create one-time transfer code"}</button></form>}</section>
    </article>)}</div>}
