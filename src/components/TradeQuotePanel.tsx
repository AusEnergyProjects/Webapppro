...
const editLine = (line: SavedLine, activeIds: Set<string>): QuoteLine => ({ ...line,
  priceBookItemId: activeIds.has(line.priceBookItemId) ? line.priceBookItemId : "",
  quantity: (line.quantityMilli / 1000).toString(), unitPrice: (line.unitPriceCents / 100).toFixed(2) });
...
export function TradeQuotePanel({ user, workOrderId, available, onChanged }: { user: User; workOrderId: string; available: boolean; onChanged?: () => void | Promise<void> }) {
  ...
  function addPriceBookItem(itemId: string) {
    const item = priceBookItems.find((candidate) => candidate.id === itemId); if (!item) return;
    const line: QuoteLine = { priceBookItemId: item.id, lineType: item.lineType, description: item.name, quantity: "1",
