...
type QuoteVersion = { id: string; versionNumber: number; status: string; customerEmail: string; subtotalCents: number; taxCents: number; totalCents: number; terms: string; validUntil: string; consentStatement: string; issuedAt: string; items: SavedLine[]; choices: SavedChoice[]; internalSummary?: { costCentsExGst: number; sellCentsExGst: number; marginCentsExGst: number }; acceptance: null | { decision: string; actorEmail: string; decidedAt: string; consentStatement: string; selectionSummary: string; selectedTotalCents: number } };
...
export function TradeQuotePanel({ user, workOrderId, available, onChanged }: { user: User; workOrderId: string; available: boolean; onChanged?: () => void | Promise<void> }) {
  ...
  function lineEditor(line: QuoteLine, index: number, onChange: (field: keyof QuoteLine, value: string) => void, onRemove: () => void, canRemove: boolean) {
    return <div className="trade-quote-line" key={`${index}:${line.id || "new"}`}>
      <select aria-label={`Line ${index + 1} type`} value={line.lineType} onChange={(event) => onChange("lineType", event.target.value)}><option value="product">Product</option><option value="labour">Labour</option><option value="adjustment">Adjustment</option></select>
      <label className="trade-quote-description"><span className="sr-only">Line {index + 1} description</span><input aria-label={`Line ${index + 1} description`} value={line.description} maxLength={500} onChange={(event) => onChange("description", event.target.value)} placeholder="Description" /><input className="trade-quote-section-input" aria-label={`Line ${index + 1} section heading`} value={line.sectionHeading} maxLength={120} onChange={(event) => onChange("sectionHeading", event.target.value)} placeholder="Customer section heading" />{line.priceBookItemId && <small>{line.jobPacketId ? "Packet item" : "Saved item"}, current price applied on save</small>}</label>
      <input aria-label={`Line ${index + 1} quantity`} value={line.quantity} inputMode="decimal" onChange={(event) => onChange("quantity", event.target.value)} />
      <input aria-label={`Line ${index + 1} unit price`} value={line.unitPrice} inputMode="decimal" onChange={(event) => onChange("unitPrice", event.target.value)} />
      <select aria-label={`Line ${index + 1} tax`} value={line.taxCode} onChange={(event) => onChange("taxCode", event.target.value)}><option value="gst">GST 10%</option><option value="none">No GST</option></select>
