      ...
      <aside className={`admin-panel ${styles.detail}`}>
        {selectedAccount ? <>
          ...
          <form className={styles.moderationForm} onSubmit={saveAccount}>
            ...
            <section className={`${styles.featureControls} ${styles.full}`}><div><span>Access and entitlements</span><h3>Premium feature grants</h3><p>Paid membership includes the role-specific commercial tools. Use grants for trials, service recovery or individually approved premium add-ons. Every change is audited.</p></div><div className={styles.featureSummary}><strong>{selectedAccount.entitlements.accessLabel}</strong><span>{selectedAccount.account.partnerType === "supplier" ? "Unpaid wholesalers stay invisible to installers unless visibility is granted." : "Free installers receive no leads unless lead access is granted."}</span></div><div className={styles.featureGrid}>{FEATURE_DEFINITIONS.filter((feature) => feature.roles.includes(selectedAccount.account.partnerType as "installer" | "supplier")).map((feature) => { const grant = selectedAccount.featureGrants.find((item) => item.featureKey === feature.key); const enabled = grant?.status === "active"; const included = feature.tier === "membership" && selectedAccount.entitlements.paidMembership; return <article key={feature.key} className={enabled ? styles.enabled : ""}><label><input type="checkbox" checked={enabled} disabled={!['owner', 'admin'].includes(role)} onChange={(event) => updateFeatureGrant(feature.key, { status: event.target.checked ? "active" : "revoked" })} /><span><strong>{feature.label}</strong><small>{feature.description}</small></span></label><div><span className={styles.featureTier}>{included ? "Included in paid plan" : feature.tier === "premium" ? "Premium add-on" : "Membership override"}</span><label>Grant expiry<input type="date" value={grant?.expiresAt?.slice(0, 10) || ""} disabled={!enabled || !['owner', 'admin'].includes(role)} onChange={(event) => updateFeatureGrant(feature.key, { expiresAt: event.target.value })} /></label><label>Grant note<input value={grant?.note || ""} disabled={!enabled || !['owner', 'admin'].includes(role)} placeholder="Reason, approval or service case" onChange={(event) => updateFeatureGrant(feature.key, { note: event.target.value })} /></label></div></article>; })}</div></section>
