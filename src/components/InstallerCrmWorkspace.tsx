...
type View = "today" | "jobs" | "schedule" | "customers" | "reports";
...
export function InstallerCrmWorkspace({ user, teamAccess }: { user: User; teamAccess: boolean }) {
  ...
  return <section id="business-hub" className="installer-crm" aria-labelledby="installer-crm-title">
    ...
    <nav className="crm-nav" aria-label="Installer CRM">
      {(["today", "jobs", "schedule", "customers", "reports"] as View[]).map((item) => <button key={item} type="button" className={view === item ? "active" : ""} onClick={() => setView(item)}>{item === "today" ? "Today" : item[0].toUpperCase() + item.slice(1)}</button>)}
    </nav>
