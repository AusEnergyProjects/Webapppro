  ...
  return <article className="crm-job-card"><header className="crm-job-card-header"><div><span>{job.workNumber}</span><h3>{job.title}</h3><small>{serviceLabels[job.serviceCategory] || job.serviceCategory}{job.siteArea ? ` | ${job.siteArea}` : ""}</small></div><div className="crm-job-header-actions"><strong>{pipelineLabels[job.pipelineStage] || job.pipelineStage}</strong><span className={isProtected ? "protected" : "owned"}>{isProtected ? "Australian Energy Assessments protected" : customer ? "Your customer" : "Internal"}</span>{!isProtected && customer && <button type="button" className="crm-request-info-button" onClick={() => setTab("field")}>Request info</button>}</div></header>
    ...
    {tab === "summary" && <section className="crm-job-section crm-summary-workspace">
      <form className="crm-form" onSubmit={saveSummary}>
        ...
        <div className={isProtected || isReleasedLead ? "crm-customer-boundary protected" : "crm-customer-boundary owned"}><span>{isReleasedLead ? "Released lead quote" : isProtected ? "Australian Energy Assessments protected customer" : "Your customer record"}</span><strong>{isProtected || isReleasedLead ? `Protected reference ${job.customerReference || job.workNumber}` : customer?.displayName || "No customer linked"}</strong><p>{isReleasedLead ? "Current customer-shared contact and property details appear only in this job's Quote tab while the exact lead release remains active." : isProtected ? "Australian Energy Assessments manages the household relationship. The installer sees only the project scope, broad service region and protected reference." : customer ? [customer.phone, customer.email, [customer.addressLine1, customer.addressLine2, customer.suburb, customer.addressState, customer.postcode].filter(Boolean).join(", ")].filter(Boolean).join(" | ") : "Link a customer who contacted your business directly, or keep this as internal work."}</p></div>
    ...
    {tab === "quote" && <section className="crm-job-section"><TradeQuotePanel user={user} workOrderId={job.id} available={isReleasedLead || (!isProtected && Boolean(customer) && Boolean(job.serviceSiteId))} onOpenPriceBook={onOpenPriceBook} onOpenCustomer={onOpenCustomer} onChanged={onReload} /></section>}
