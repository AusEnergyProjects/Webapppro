...
import { TradeCommercialHandoffPanel } from "./TradeCommercialHandoffPanel";
...
import { TradeAssetWorkspace } from "./TradeAssetWorkspace";
...
import { TradeNewJobForm } from "./TradeNewJobForm";
...
import { appointmentDurationMinutes, browserLocalDateTime, durationLabel } from "@/lib/trade-schedule";
...
type CreateJobResult = { ok?: boolean; id?: string; workNumber?: string; customerId?: string; serviceSiteId?: string; requestSent?: boolean; deliveryError?: string; duplicateCandidates?: DuplicateCandidate[]; error?: string };
...
export function InstallerCrmWorkspace({ user, teamAccess, navigationTarget }: { user: User; teamAccess: boolean; navigationTarget?: TLinkCommandTarget | null }) {
  ...
  async function createJob(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await load(); setRefreshNonce((value) => value + 1); setStatus(result.deliveryError || `${result.workNumber || "Job"} scheduled and the information request was sent.`);
  ...
  return <section id="business-hub" className="installer-crm" aria-labelledby="installer-crm-title">
    ...
    {view === "jobs" && creating !== "job" && focusedJobId && <div className="crm-view crm-job-focus">
      ...
      {selectedJobDetail?.id === focusedJobId ? <JobDetail key={`${selectedJobDetail.id}:${focusedJobTab}:${refreshNonce}`} job={selectedJobDetail} customer={selectedJobCustomer || undefined} sites={selectedJobSites} user={user} busy={busy} teamMembers={teamMembers} initialTab={focusedJobTab} onCrm={crmRequest} onWorkOrder={workOrderRequest} onOpenIntegrations={() => setView("integrations")} onReload={async () => setRefreshNonce((value) => value + 1)} /> : <div className="crm-empty"><strong>Loading job...</strong><span>The full job record will open here.</span></div>}
    </div>}
    ...
    {view === "jobs" && creating !== "job" && !focusedJobId && <div className="crm-view">
      ...
      {jobLayout === "list" ? <div className="crm-jobs-layout">
        ...
        <main className="crm-job-detail">{selectedJobDetail?.id === selectedJobId ? <JobDetail key={`${selectedJobDetail.id}:${refreshNonce}`} job={selectedJobDetail} customer={selectedJobCustomer || undefined} sites={selectedJobSites} user={user} busy={busy} teamMembers={teamMembers} onCrm={crmRequest} onWorkOrder={workOrderRequest} onOpenIntegrations={() => setView("integrations")} onReload={async () => setRefreshNonce((value) => value + 1)} /> : <div className="crm-empty"><strong>{selectedJobId ? "Loading job..." : "Select a job"}</strong><span>The job record will open here.</span></div>}</main>
    ...
    {view === "customers" && creating !== "customer" && <div className="crm-view">
      ...
      <details className="crm-granular-filters"><summary>Detailed customer filters</summary><div>
        ...
        <label><span>Job ID</span><input value={customerJobId} onChange={(event) => { setCustomerJobId(event.target.value); setCustomerPage(1); }} placeholder="e.g. TLJ-00000124" /></label>
...
function JobDetail({ job, customer, sites, user, busy, teamMembers, initialTab = "summary", onCrm, onWorkOrder, onOpenIntegrations, onReload }: { job: Job; customer?: Customer; sites: ServiceSite[]; user: User; busy: string; teamMembers: TeamMember[]; initialTab?: JobTab; onCrm: (method: "POST" | "PATCH", body: Record<string, unknown>, key: string, success: string) => Promise<boolean>; onWorkOrder: (method: "POST" | "PATCH", body: Record<string, unknown>, key: string, success: string) => Promise<boolean>; onOpenIntegrations: () => void; onReload: () => Promise<void> }) {
  const [tab, setTab] = useState<JobDetailTab>(initialTab);
  const [appointmentDuration, setAppointmentDuration] = useState(60);
  const minimumStart = browserLocalDateTime();
