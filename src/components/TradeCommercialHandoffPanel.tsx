...
export function TradeCommercialHandoffPanel({ user, workOrderId, isProtected, hasDirectCustomer, customerName, jobTitle, onOpenIntegrations, onChanged }: {
  ...
  return <section className="crm-commercial-handoff">
    <header><div><span>Accepted quote handoff</span><h4>{handoff.commercialReference}</h4><p>One accepted scope now drives the deposit record and accounting draft. No retyping or provider calculations.</p></div><strong>{money(handoff.totalCents)}</strong></header>
    ...
    <div className="crm-deposit-choice"><div><span>Deposit amount</span><h5>{money(handoff.depositAmountCents)}</h5><p>10% is the simple default. TLink records the amount but does not initiate its payment while hosted on Sites.</p></div><label><span>Method</span><select value={depositKind} onChange={(event) => setDepositKind(event.target.value as "percentage" | "fixed")}><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></label><label><span>{depositKind === "percentage" ? "Percent" : "Amount"}</span><input type="number" min={depositKind === "percentage" ? 1 : 1} max={depositKind === "percentage" ? 100 : handoff.totalCents / 100} step={depositKind === "percentage" ? 1 : 0.01} value={depositValue} onChange={(event) => setDepositValue(event.target.value)} /></label><button type="button" disabled={busy} onClick={() => void saveDeposit()}>{busy ? "Saving..." : "Use this deposit"}</button></div>
