...
type DashboardView = "overview" | "editor" | "profile" | "detail" | "assets" | "quotes";
...
function ProjectDetail({ user, project, busy, onAction, onDownloadHandover }: { user: User; project: CustomerProject; busy: boolean; onAction: (action: string, extra?: Record<string, unknown>) => Promise<void>; onDownloadHandover: (document: CustomerHandoverPack["documents"][number]) => Promise<void> }) {
...
export function CustomerDashboard({ initialView = "overview", initialProjectId = "", initialEdit = false, initialPlannerSelection }: { initialView?: "overview" | "new" | "profile" | "assets" | "quotes"; initialProjectId?: string; initialEdit?: boolean; initialPlannerSelection?: { goal?: string; pace?: string; situation?: string; features?: string[]; categories?: string[]; postcode?: string } }) {
  ...
  const [view, setView] = useState<DashboardView>(initialProjectId ? initialEdit ? "editor" : "detail" : initialView === "new" ? "editor" : initialView === "profile" ? "profile" : initialView === "assets" ? "assets" : initialView === "quotes" ? "quotes" : "overview");
  ...
  return <main id="main-content" className="wrap customer-account-page">
    ...
    {!authReady || loading ? <section className="customer-loading-state" aria-live="polite"><span /><div><strong>Preparing your private dashboard</strong><p>Loading saved homes, projects and roadmaps...</p></div></section> : !user ? <>
      ...
      <header className="customer-dashboard-hero"><div><span>Welcome back, {account.profile.displayName}</span><h1>Your home upgrade plans</h1><p>See what to do next, start another project or review installer options without sharing your contact details.</p></div><aside><span>Household account</span><strong>Always free</strong><small>All planning, projects and response tools are included.</small></aside></header>
      <nav className="customer-dashboard-nav" aria-label="Customer account"><a className={view === "overview" ? "active" : ""} href="/account">Overview</a><a className={view === "assets" ? "active" : ""} href="/account/assets">Home records</a><a className={view === "quotes" ? "active" : ""} href="/account/quotes">Direct quotes</a><a className={view === "editor" && !editingId ? "active" : ""} href="/account/projects/new">New project</a><a href="/account/profile">Privacy and profile</a>{account.tradeWorkspace && <a href="/direct-trade/dashboard">Trade workspace</a>}<button type="button" onClick={() => void signOut(firebaseAuth)}>Sign out</button></nav>
