...
type Preference = { assetId: string; remindersEnabled: boolean; reminderLeadDays: number };
  ...
  return <details className="customer-detail-panel customer-lifecycle-library customer-lifecycle-simple">
    ...
    <div className="customer-lifecycle-assets">{assets.map((asset) => {
      ...
      return <article key={asset.id}><header><div><span>{readable(asset.assetCategory)}</span><h3>{asset.brand} {asset.modelNumber}</h3><small>{asset.workNumber} | {asset.warrantyEnd ? `Warranty ends ${asset.warrantyEnd}` : "Warranty end not recorded"}</small></div><strong>{assetPlans.length} schedule{assetPlans.length === 1 ? "" : "s"}</strong></header>
        <div className="customer-reminder-preference"><label><input type="checkbox" checked={preference.remindersEnabled} disabled={busy === `pref:${asset.id}`} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: event.target.checked, leadDays: preference.reminderLeadDays }, `pref:${asset.id}`, "Dashboard reminder preference saved.")} /><span><strong>Show dashboard reminders</strong><small>Private to this customer account</small></span></label><select aria-label={`Reminder window for ${asset.brand} ${asset.modelNumber}`} value={preference.reminderLeadDays} disabled={!preference.remindersEnabled || busy === `pref:${asset.id}`} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: true, leadDays: Number(event.target.value) }, `pref:${asset.id}`, "Dashboard reminder window saved.")}><option value="7">7 days before</option><option value="14">14 days before</option><option value="30">30 days before</option><option value="60">60 days before</option><option value="90">90 days before</option></select></div>
