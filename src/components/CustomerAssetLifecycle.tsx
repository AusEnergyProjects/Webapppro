...
type Preference = { assetId: string; remindersEnabled: boolean; emailEnabled: boolean; smsEnabled: boolean; reminderLeadDays: number; recorded: boolean };
type ReminderContact = { mobileMasked: string; mobileVerified: boolean; smsVerificationConfigured: boolean };
type Result = { ok?: boolean; assets?: Asset[]; plans?: Plan[]; events?: Event[]; notices?: Notice[]; preferences?: Preference[]; contact?: ReminderContact; error?: string };
...
export function CustomerAssetLifecycle({ user, projectId = "", packId = "" }: { user: User; projectId?: string; packId?: string }) {
  ...
  const request = useCallback(async (body?: Record<string, unknown>) => {
    ...
    if ("assets" in payload) setResult(payload);
  ...
  return <details className="customer-detail-panel customer-lifecycle-library customer-lifecycle-simple">
    ...
    <section className="customer-reminder-contact"><div><span>Reminder destinations</span><strong>Email uses your verified private account address</strong><small>{result.contact?.mobileVerified ? `Verified mobile ending ${result.contact.mobileMasked.slice(-3)}` : result.contact?.smsVerificationConfigured ? "Verify a mobile number before choosing SMS" : "SMS setup is not active yet"}</small></div>{!result.contact?.mobileVerified && result.contact?.smsVerificationConfigured && <div className="customer-mobile-verification"><input aria-label="Australian mobile number" value={mobile} placeholder="04xx xxx xxx" onChange={(event) => setMobile(event.target.value)} />{verificationStarted && <input aria-label="SMS verification code" inputMode="numeric" value={verificationCode} placeholder="Verification code" onChange={(event) => setVerificationCode(event.target.value)} />}<button type="button" disabled={Boolean(busy) || !mobile} onClick={() => { if (!verificationStarted) { setVerificationStarted(true); void update({ action: "start_sms_verification", assetId: assets[0].id, mobile }, "mobile:start", "Verification code sent by SMS."); } else void update({ action: "confirm_sms_verification", assetId: assets[0].id, mobile, code: verificationCode }, "mobile:confirm", "Mobile number verified for service reminders."); }}>{verificationStarted ? "Verify code" : "Send verification code"}</button></div>}</section>
    ...
    <div className="customer-lifecycle-assets">{assets.map((asset) => {
      const preference = preferences.find((item) => item.assetId === asset.id) || { assetId: asset.id, remindersEnabled: false, emailEnabled: false, smsEnabled: false, reminderLeadDays: 30, recorded: false };
      ...
      return <article key={asset.id}><header><div><span>{readable(asset.assetCategory)}</span><h3>{asset.brand} {asset.modelNumber}</h3><small>{asset.workNumber} | {asset.warrantyEnd ? `Warranty ends ${asset.warrantyEnd}` : "Warranty end not recorded"}</small></div><strong>{assetPlans.length} schedule{assetPlans.length === 1 ? "" : "s"}</strong></header>
        <div className="customer-reminder-preference"><label><input type="checkbox" checked={preference.remindersEnabled} disabled={busy === `pref:${asset.id}`} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: event.target.checked, emailEnabled: preference.emailEnabled, smsEnabled: preference.smsEnabled, leadDays: preference.reminderLeadDays }, `pref:${asset.id}`, event.target.checked ? "Service reminder consent saved. Choose at least one delivery channel when you want messages sent." : "Service reminders turned off.")} /><span><strong>Allow service reminders</strong><small>{preference.recorded ? "Saved to this private customer account" : "Off until you explicitly enable it"}</small></span></label><div className="customer-reminder-channels"><label><input type="checkbox" checked={preference.emailEnabled} disabled={!preference.remindersEnabled || Boolean(busy)} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: true, emailEnabled: event.target.checked, smsEnabled: preference.smsEnabled, leadDays: preference.reminderLeadDays }, `pref:${asset.id}`, event.target.checked ? "Email reminders enabled." : "Email reminders stopped.")} />Email</label><label><input type="checkbox" checked={preference.smsEnabled} disabled={!preference.remindersEnabled || !result.contact?.mobileVerified || Boolean(busy)} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: true, emailEnabled: preference.emailEnabled, smsEnabled: event.target.checked, leadDays: preference.reminderLeadDays }, `pref:${asset.id}`, event.target.checked ? "SMS reminders enabled." : "SMS reminders stopped.")} />SMS</label></div><select aria-label={`Reminder window for ${asset.brand} ${asset.modelNumber}`} value={preference.reminderLeadDays} disabled={!preference.remindersEnabled || busy === `pref:${asset.id}`} onChange={(event) => void update({ action: "update_reminders", assetId: asset.id, enabled: true, emailEnabled: preference.emailEnabled, smsEnabled: preference.smsEnabled, leadDays: Number(event.target.value) }, `pref:${asset.id}`, "Service reminder window saved.")}><option value="7">7 days before</option><option value="14">14 days before</option><option value="30">30 days before</option><option value="60">60 days before</option><option value="90">90 days before</option></select></div>
        {asset.warrantyEnd && <div className="customer-lifecycle-warranty"><div><span>Warranty reminder</span><strong>Coverage recorded to {asset.warrantyEnd}</strong></div><a href={googleCalendarUrl({ title: `Warranty review: ${asset.brand} ${asset.modelNumber}`, date: asset.warrantyEnd, details: "Review the warranty record in your private Australian Energy Assessments customer dashboard." })} target="_blank" rel="noreferrer">Add to Google Calendar</a></div>}
        {assetPlans.map((plan) => <section className={`customer-service-plan status-${plan.lifecycleStatus}`} key={plan.id}><div><span>{serviceLabels[plan.serviceType] || readable(plan.serviceType)}</span><strong>{plan.status === "paused" ? "Schedule paused" : `Next due ${plan.nextDueAt}`}</strong><small>Every {plan.cadenceMonths} months | {readable(plan.lifecycleStatus)}</small></div>{plan.status === "active" && <a href={googleCalendarUrl({ title: `${serviceLabels[plan.serviceType] || "Asset service"}: ${asset.brand} ${asset.modelNumber}`, date: plan.nextDueAt, details: "Open your private Australian Energy Assessments dashboard for the asset and service history." })} target="_blank" rel="noreferrer">Add to Google Calendar</a>}<details><summary>Service history</summary>{events.filter((item) => item.servicePlanId === plan.id).length ? <ul>{events.filter((item) => item.servicePlanId === plan.id).map((item) => <li key={item.id}><strong>{item.servicedAt}</strong><span>{item.summary || "Service completed"}</span><small>{item.providerReference ? `Reference ${item.providerReference}` : "No provider reference recorded"}</small></li>)}</ul> : <p>No completed service entries yet.</p>}</details></section>)}
      </article>;
    })}</div>
