...
import { PHOTO_RETAKE_REASONS, type PhotoRequirementReview, type PhotoRetakeReason, type PhotoReviewStatus } from "@/lib/photo-request-review";
...
type RequestRecord = {
  ...
  proof: null | { completion: null | { revision: number; completedAt: string; evidenceCurrent: boolean; current: boolean };
    reviews: PhotoRequirementReview[]; counts: { total: number; required: number; supplied: number; accepted: number; retakeRequested: number; notNeeded: number; pending: number };
    proofReady: boolean; outstandingRequirementIds: string[] };
};
...
type DeliveryRecord = { id: string; channel: "email" | "sms"; provider: string; intent: string; status: string; eligibilityReason: string; attempts: number; providerStatus: string; lastError: string; queuedAt: string; sentAt: string; deliveredAt: string; failedAt: string; updatedAt: string; reviewRevision: number; photoRequirementId: string };
...
export function TradePhotoRequestPanel({ user, workOrderId }: { user: User; workOrderId: string }) {
  ...
  const [retakeReasons, setRetakeReasons] = useState<Record<string, PhotoRetakeReason>>({});
  ...
  async function reviewRequirement(requirementId: string, reviewStatus: PhotoReviewStatus) {
    setBusy(`review:${requirementId}`); setStatus("Saving the proof review...");
    try {
      const token = await user.getIdToken();
      const response = await fetch("/api/trade-photo-requests", { method: "POST", headers: {
        "Content-Type": "application/json", Authorization: `Bearer ${token}`,
      }, body: JSON.stringify({ action: "review_requirement", workOrderId, requirementId, reviewStatus,
        reasonCode: retakeReasons[requirementId] || "", expectedRevision: record?.revision || 0 }) });
      const result = await response.json().catch(() => ({})) as Result;
      if (!response.ok || !result.ok) throw new Error(result.error || "The proof review could not be saved.");
      apply(result); setStatus(reviewStatus === "retake_requested" ? "Retake guidance saved. Send one targeted follow-up when the destination is ready."
        : reviewStatus === "accepted" ? "Requested photo accepted." : "This photo requirement is marked not needed.");
    } catch (error) { setStatus(error instanceof Error ? error.message : "The proof review could not be saved."); }
    finally { setBusy(""); }
  }

  async function sendRetake(channel: "email" | "sms", review: PhotoRequirementReview) {
    setBusy(`retake:${review.requirementId}:${channel}`); setStatus("Sending the targeted retake guidance through the current link...");
    try {
      const token = await user.getIdToken();
      const response = await fetch("/api/trade-photo-requests", { method: "POST", headers: {
        "Content-Type": "application/json", Authorization: `Bearer ${token}`,
      }, body: JSON.stringify({ action: "send_retake", workOrderId, channel, requirementId: review.requirementId,
        reviewRevision: review.reviewRevision, consentConfirmed }) });
      const result = await response.json().catch(() => ({})) as Result;
      if (!response.ok || !result.ok) throw new Error(result.error || "The retake follow-up could not be delivered.");
      apply(result); setConsentConfirmed(false); setStatus("Targeted retake follow-up accepted for delivery through the current secure link.");
    } catch (error) { setStatus(error instanceof Error ? error.message : "The retake follow-up could not be delivered."); }
    finally { setBusy(""); }
  }
  ...
  return <section className={styles.panel} aria-labelledby="trade-photo-request-title">
    ...
    {record?.proof && <section className={record.proof.proofReady ? styles.proofReady : styles.proofReview}><header><div><span>Job proof readiness</span><strong>{record.proof.proofReady ? "Customer photo proof ready" : record.proof.completion?.evidenceCurrent ? "Customer set ready for review" : "Waiting for customer completion"}</strong></div><small>{record.proof.counts.accepted} accepted | {record.proof.counts.retakeRequested} retake | {record.proof.counts.pending} pending</small></header><div><span>{record.proof.counts.supplied} of {record.proof.counts.total} requirements supplied</span><span>{record.proof.counts.required} required</span><span>{record.proof.counts.notNeeded} not needed</span></div></section>}
    ...
    <div className={styles.requirements}>
      {requirements.map((item, index) => <article key={item.id}>
        ...
        {record?.proof && (() => { const review = record.proof?.reviews.find((value) => value.requirementId === item.id); const canReview = Boolean(record.proof?.completion?.evidenceCurrent); return <section className={styles.requirementReview}><header><div><span>Installer review</span><strong>{readable(review?.status || "pending")}</strong></div>{review?.reviewedAt && <small>{new Date(review.reviewedAt).toLocaleString("en-AU")}</small>}</header>{review?.status === "retake_requested" && <p>{review.guidance}{review.retakeAnswered ? " A replacement has been added for review." : " The original evidence remains attached."}</p>}<div><button type="button" disabled={!canReview || !(record.uploadCounts[item.id] || 0) || Boolean(busy)} onClick={() => void reviewRequirement(item.id, "accepted")}>Accept</button><button type="button" disabled={!canReview || Boolean(busy)} onClick={() => void reviewRequirement(item.id, "not_needed")}>Not needed</button></div><label><span>Safe retake reason</span><select value={retakeReasons[item.id] || review?.reasonCode || ""} disabled={!canReview || !(record.uploadCounts[item.id] || 0) || Boolean(busy)} onChange={(event) => setRetakeReasons((current) => ({ ...current, [item.id]: event.target.value as PhotoRetakeReason }))}><option value="">Choose a reason</option>{Object.entries(PHOTO_RETAKE_REASONS).map(([value, guidance]) => <option value={value} key={value}>{guidance}</option>)}</select></label><button type="button" className={styles.retakeButton} disabled={!canReview || !(record.uploadCounts[item.id] || 0) || !(retakeReasons[item.id] || review?.reasonCode) || Boolean(busy)} onClick={() => void reviewRequirement(item.id, "retake_requested")}>Request retake</button></section>; })()}
      </article>)}
    </div>
