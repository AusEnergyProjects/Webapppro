...
export function AdminCatalogueWorkspace({ api, role, setStatus }: AdminCatalogueWorkspaceProps) {
  ...
  const [productListingStatus, setProductListingStatus] = useState("open");
  ...
  return <div className={styles.workspace}>
    ...
    <section className="admin-panel admin-catalogue-workspace"><div className="admin-panel-heading"><span>Catalogue controls</span><h2>Review products without exposing customer information</h2><p>Confirm the model identity, description, ex-GST price, minimum order, availability, warranty and dependency count.</p></div>
      <div className="admin-catalogue-granular crm-granular-filters"><div>
        ...
        <label><span>Listing status</span><select value={productListingStatus} onChange={(event) => { setProductListingStatus(event.target.value); setProductPage(1); }}><option value="open">Current listings (exclude archived)</option><option value="">Any listing status</option><option value="draft">Draft</option><option value="published">Published</option><option value="paused">Paused</option><option value="archived">Archived</option></select></label>
        ...
        <button type="button" onClick={() => { setProductSearch(""); setProductWholesaler(""); setProductBrand(""); setProductModel(""); setProductCategory(""); setProductStock(""); setProductReviewStatus(""); setProductListingStatus("open"); setProductMinimumPrice(""); setProductMaximumPrice(""); setProductPage(1); }}>Clear filters</button>
      </div></div>
