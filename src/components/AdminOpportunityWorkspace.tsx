...
export function AdminOpportunityWorkspace({ api, demoOnlyRequest, role, setStatus }: AdminOpportunityWorkspaceProps) {
  ...
  async function allocateOpportunity(id: string) {
    setStatus("Finding every verified installer whose service area and capabilities match...");
    try {
      const result = await api("/api/admin/opportunities/allocate", { method: "POST", body: JSON.stringify({ opportunityId: id }) });
      await loadOpportunities();
      setStatus(`${result.allocated?.length || 0} new installer${result.allocated?.length === 1 ? "" : "s"} allocated. ${result.eligibleCount || 0} verified businesses currently match the service area and capability requirements.`);
    } catch (error) { setStatus(errorMessage(error)); }
  }
