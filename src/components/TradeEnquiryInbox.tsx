...
const readable = (value: string) => ENERGY_SERVICE_LABELS[value]
...
export function TradeEnquiryInbox({ user, onConverted }: { user: User; onConverted?: (seed: ConvertedEnquiryJobSeed) => void | Promise<void> }) {
  ...
  const [newAddress, setNewAddress] = useState({ addressLine1: "", addressLine2: "", suburb: "", addressState: "", postcode: "" });
  ...
  function selectNewAddress(selection: AustralianAddressSuggestion) {
    setNewAddress({
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset(); setNewAddress({ addressLine1: "", addressLine2: "", suburb: "", addressState: "", postcode: "" }); setCreating(false); await loadList(); if (result.id) selectEnquiry(result.id); setStatus("Direct enquiry added to the inbox.");
  ...
  return <section className="enquiry-workspace" aria-labelledby="enquiry-inbox-title">
    ...
    {creating && <form className="crm-form enquiry-create" onSubmit={create}><div className="crm-form-grid"><label><span>Source</span><input name="sourceType" required placeholder="website, referral, phone" /></label><label><span>External record ID</span><input name="externalRecordId" placeholder="Optional source ID" /></label><label><span>Customer type</span><select name="customerType"><option value="residential">Residential</option><option value="business">Business</option></select></label><label><span>First name</span><input name="firstName" /></label><label><span>Last name</span><input name="lastName" /></label><label><span>Business name</span><input name="businessName" /></label><label><span>Business number</span><input name="businessNumber" inputMode="numeric" /></label><label><span>Email</span><input type="email" name="email" /></label><label><span>Phone</span><input type="tel" name="phone" /></label><AustralianAddressLookup className="wide" name="addressLine1" value={newAddress.addressLine1} onChange={(addressLine1) => setNewAddress((current) => ({ ...current, addressLine1 }))} onSelect={selectNewAddress} /><label className="wide"><span>Address line 2</span><input name="addressLine2" value={newAddress.addressLine2} onChange={(event) => setNewAddress((current) => ({ ...current, addressLine2: event.target.value }))} maxLength={140} autoComplete="address-line2" placeholder="Unit, level or suite, optional" /></label><label><span>Suburb</span><input name="suburb" value={newAddress.suburb} onChange={(event) => setNewAddress((current) => ({ ...current, suburb: event.target.value }))} /></label><label><span>State</span><select name="addressState" value={newAddress.addressState} onChange={(event) => setNewAddress((current) => ({ ...current, addressState: event.target.value }))}><option value="">Select state</option>{["ACT", "NSW", "NT", "QLD", "SA", "TAS", "VIC", "WA"].map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Postcode</span><input name="postcode" pattern="[0-9]{4}" value={newAddress.postcode} onChange={(event) => setNewAddress((current) => ({ ...current, postcode: event.target.value.replace(/\D/g, "").slice(0, 4) }))} /></label><label><span>Service</span><select name="serviceCategory">{SERVICES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>Urgency</span><select name="urgency"><option value="standard">Standard</option><option value="low">Low</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><label><span>Preferred date</span><input type="date" name="preferredDate" /></label><label className="wide"><span>Enquiry details</span><textarea name="description" required rows={4} /></label></div><p className="crm-form-note">Only add identity and address details supplied directly to your business.</p><button className="btn" disabled={busy === "create"}>{busy === "create" ? "Adding..." : "Add enquiry"}</button></form>}
