...
import {
  moveInvoiceLine,
  moveInvoiceLineTo,
} from "@/lib/trade-invoice-line-reorder";
import {
...
export function TradeQuickInvoicePanel({ user, workOrderId, customerName, jobTitle, readOnly = false, onOpenIntegrations, onChanged }: { user: User; workOrderId: string; customerName: string; jobTitle: string; readOnly?: boolean; onOpenIntegrations?: () => void; onChanged: () => Promise<void> }) {
  ...
  const draggedDraftLineIdRef = useRef("");
  ...
  function moveDraftLine(line: EditLine, direction: -1 | 1) {
    setDraftLines((current) => moveInvoiceLine(current, line.id, direction));
    setStatus(`Moved ${line.description.trim() || "invoice line"} ${direction < 0 ? "up" : "down"}.`);
  }

  function dropDraftLine(targetLine: EditLine, draggedLineId: string) {
    if (!draggedLineId || draggedLineId === targetLine.id) return;
    setDraftLines((current) => moveInvoiceLineTo(current, draggedLineId, targetLine.id));
    setStatus("Invoice line order updated.");
  }
  ...
  return <><section className="crm-quick-invoice-panel">
    ...
    {canManageInvoice && invoice.canCorrect && <details className="crm-invoice-correction"><summary>Correct this draft before sending</summary><p>Saving creates a new revision. Earlier invoice snapshots remain unchanged in history.</p><div className="crm-invoice-correction-lines">{draftLines.map((line, index) => <div key={line.id} onDragOver={(event) => { if (draggedDraftLineIdRef.current && draggedDraftLineIdRef.current !== line.id) { event.preventDefault(); event.dataTransfer.dropEffect = "move"; } }} onDrop={(event) => { event.preventDefault(); const draggedLineId = draggedDraftLineIdRef.current || event.dataTransfer.getData("text/plain"); dropDraftLine(line, draggedLineId); draggedDraftLineIdRef.current = ""; }}><div style={{ display: "grid", gridTemplateColumns: "auto minmax(0, 1fr)", gap: "7px", alignItems: "stretch", minWidth: 0 }}><div role="group" aria-label={`Reorder ${line.description.trim() || `invoice line ${index + 1}`}`} style={{ display: "grid", gridTemplateColumns: "repeat(2, 44px)", gap: "4px" }}><button type="button" draggable aria-label={`Drag ${line.description.trim() || `invoice line ${index + 1}`} to reorder`} title="Drag to reorder" onDragStart={(event) => { draggedDraftLineIdRef.current = line.id; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", line.id); }} onDragEnd={() => { draggedDraftLineIdRef.current = ""; }} style={{ gridColumn: "1 / -1", minHeight: "44px", marginTop: 0, cursor: "grab" }}>Drag</button><button type="button" aria-label={`Move ${line.description.trim() || `invoice line ${index + 1}`} up`} title="Move up" disabled={index === 0} onClick={() => moveDraftLine(line, -1)} style={{ minWidth: "44px", minHeight: "44px", padding: 0, marginTop: 0, fontSize: "1.1rem" }}>↑</button><button type="button" aria-label={`Move ${line.description.trim() || `invoice line ${index + 1}`} down`} title="Move down" disabled={index === draftLines.length - 1} onClick={() => moveDraftLine(line, 1)} style={{ minWidth: "44px", minHeight: "44px", padding: 0, marginTop: 0, fontSize: "1.1rem" }}>↓</button></div><input aria-label="Invoice line description" maxLength={180} value={line.description} onChange={(event) => updateLine(line.id, { description: event.target.value })} style={{ minWidth: 0 }} /></div><input aria-label="Invoice line amount before GST" type="number" min="0.01" step="0.01" value={line.amount} onChange={(event) => updateLine(line.id, { amount: event.target.value })} /><select aria-label="Invoice line GST" value={line.taxCode} onChange={(event) => updateLine(line.id, { taxCode: event.target.value as "gst" | "none" })}><option value="gst">Add GST</option><option value="none">GST-free</option></select><button type="button" disabled={draftLines.length === 1} onClick={() => setDraftLines((current) => current.filter((item) => item.id !== line.id))}>Remove</button></div>)}</div><button type="button" onClick={() => setDraftLines((current) => [...current, { id: crypto.randomUUID(), description: "", amount: "", taxCode: "gst" }])}>Add line</button><div className="crm-invoice-correction-meta">{canApplyDiscounts && <label><span>Discount (ex GST)</span><input aria-label="Corrected invoice discount before GST" type="number" min="0" step="0.01" value={draftDiscount} onChange={(event) => setDraftDiscount(event.target.value)} /></label>}<label><span>Due date</span><input type="date" min={australiaSydneyToday} value={draftDueAt} onChange={(event) => setDraftDueAt(event.target.value)} /></label><label><span>Reason, optional</span><input maxLength={240} value={draftReason} onChange={(event) => setDraftReason(event.target.value)} placeholder="What changed?" /></label></div><button type="button" className="btn" disabled={Boolean(busy) || !draftLines.every((line) => line.description.trim() && toCents(line.amount) > 0) || draftDiscountCents < 0 || draftDiscountCents >= draftSubtotalCents || !draftDueAt} onClick={() => void correctDraft()}>{busy === "correct_draft" ? "Saving correction..." : "Save corrected draft"}</button></details>}
