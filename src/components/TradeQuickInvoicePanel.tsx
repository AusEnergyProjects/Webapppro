...
export function TradeQuickInvoicePanel({ user, workOrderId, customerName, jobTitle, onOpenIntegrations, onChanged }: { user: User; workOrderId: string; customerName: string; jobTitle: string; onOpenIntegrations?: () => void; onChanged: () => Promise<void> }) {
  ...
  async function request(action: string, values: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      setStatus(success); await onChanged();
    } catch (error) { setStatus(error instanceof Error ? error.message : "The invoice could not be updated."); }
    finally { setBusy(""); }
  }
  ...
  return <><section className="crm-quick-invoice-panel">
    ...
    {invoice.deliveryStatus !== "sent" && <button type="button" className="btn" disabled={Boolean(busy)} onClick={() => void request("retry_delivery", { consentConfirmed: true }, "Invoice sent.")}>{busy === "retry_delivery" ? "Sending..." : "Send invoice email"}</button>}
