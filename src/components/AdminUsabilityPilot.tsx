...
export function AdminUsabilityPilot({ api, role }: { api: (path: string, init?: RequestInit) => Promise<Record<string, unknown>>; role: AdminRole }) {
  ...
  return <section className="admin-pilot-workspace">
    ...
    <div className="admin-pilot-layout">
      ...
      {canManageCohort && openSlots.length > 0 && <form className="admin-pilot-recruit" onSubmit={addParticipant}><div><span>Add a live installer</span><h3>Fill the next pilot slot</h3><p>Synthetic demo accounts are excluded. Contact and consent still need to be arranged with the business.</p></div><label><span>Slot</span><select name="slotNumber">{openSlots.map((slot) => <option key={slot} value={slot}>Slot {slot}</option>)}</select></label><label className="wide"><span>Business</span><select name="firebaseUid" required defaultValue=""><option value="" disabled>Choose an installer</option>{availableCandidates.map((item) => <option key={item.firebaseUid} value={item.firebaseUid}>{item.businessName} | {item.addressState} {item.postcode}</option>)}</select></label><label><span>Current system</span><input name="baselineSystem" placeholder="ServiceM8, Tradify, paper" /></label><label><span>Team size</span><input type="number" min="1" max="10000" name="teamSize" defaultValue="1" /></label><label><span>Primary trade</span><input name="primaryTrade" placeholder="Electrical" /></label><label><span>Australian Energy Assessments owner</span><select name="ownerUid" defaultValue=""><option value="">Current administrator</option>{admins.map((admin) => <option key={admin.firebaseUid} value={admin.firebaseUid}>{admin.name}</option>)}</select></label><label className="wide"><span>Next action</span><input name="nextAction" defaultValue="Arrange a 30-minute guided onboarding session." /></label><button disabled={busy === "add" || availableCandidates.length === 0}>{busy === "add" ? "Adding..." : "Add to pilot"}</button></form>}
      </section>

      <aside className="admin-pilot-detail">{selected ? <><header><span>Slot {selected.slotNumber}</span><h2>{selected.businessName}</h2><p>{selected.ownerName ? `Owned by ${selected.ownerName}` : "No Australian Energy Assessments owner assigned"}</p></header><form onSubmit={updateParticipant}><label><span>Status</span><select name="status" defaultValue={selected.status}>{["invited", "onboarding", "active", "feedback_due", "completed", "paused", "withdrawn"].map((item) => <option key={item} value={item}>{readable(item)}</option>)}</select></label><label><span>Current system</span><input name="baselineSystem" defaultValue={selected.baselineSystem} /></label><label><span>Team size</span><input type="number" min="1" max="10000" name="teamSize" defaultValue={selected.teamSize} /></label><label><span>Primary trade</span><input name="primaryTrade" defaultValue={selected.primaryTrade} /></label><label><span>Australian Energy Assessments owner</span><select name="ownerUid" defaultValue={selected.ownerUid}><option value="">Unassigned</option>{admins.map((admin) => <option key={admin.firebaseUid} value={admin.firebaseUid}>{admin.name}</option>)}</select></label><label><span>Next action</span><textarea name="nextAction" rows={3} defaultValue={selected.nextAction} /></label><button disabled={busy === "participant"}>Save participant</button></form>
