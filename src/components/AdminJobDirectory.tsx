...
export function AdminJobDirectory({ api }: { api: (path: string, init?: RequestInit) => Promise<Record<string, unknown>> }) {
  ...
  return <section className="admin-workspace admin-job-directory">
    ...
    <label className="admin-job-search"><span>Search all jobs</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="TLJ-00000124, customer or installer" autoFocus /></label>
