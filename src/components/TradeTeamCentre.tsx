...
export function TradeTeamCentre({ user }: { user: User }) {
  ...
  const assignableMembers = members.filter((member) => member.status === "active" || member.status === "invited");
  ...
  return <div className="crm-team-centre">
    ...
    {view === "dispatch" && <section className="crm-dispatch-board">
      ...
      {scheduled.length ? <div className="crm-dispatch-list">{scheduled.map((job) => <article key={job.id} className={!job.assigneeMemberId ? "unassigned" : assignableMembers.some((member) => member.id === job.assigneeMemberId && member.status === "invited") ? "pending-assignee" : ""}><div className="crm-dispatch-date"><strong>{job.scheduledStart ? new Date(`${job.scheduledStart}T00:00:00`).toLocaleDateString("en-AU", { day: "2-digit" }) : "?"}</strong><span>{job.scheduledStart ? new Date(`${job.scheduledStart}T00:00:00`).toLocaleDateString("en-AU", { month: "short" }) : "Unscheduled"}</span></div><div className="crm-dispatch-job"><span>{job.workNumber} | {stageLabels[job.stage] || job.stage}</span><strong>{job.title}</strong><small>{job.protectedJob ? "AEA protected, region only" : job.serviceAddress || "Direct customer address not added"}</small>{assignableMembers.some((member) => member.id === job.assigneeMemberId && member.status === "invited") && <em>Preassigned. Access begins after the invitation is accepted.</em>}</div><label><span>Assigned technician</span><select value={job.assigneeMemberId} disabled={busy === `assign:${job.id}`} onChange={(event) => void request("PATCH", { action: "assign_job", workOrderId: job.id, memberId: event.target.value }, `assign:${job.id}`, event.target.value && assignableMembers.find((member) => member.id === event.target.value)?.status === "invited" ? "Technician preassigned. They will receive access after accepting the invitation." : "Job assignment saved.")}><option value="">Unassigned</option>{assignableMembers.map((member) => <option key={member.id} value={member.id}>{member.displayName}{member.status === "invited" ? " (invite pending)" : ""}</option>)}</select></label></article>)}</div> : <div className="crm-empty"><strong>No active jobs</strong><span>New jobs will appear here for assignment.</span></div>}
    </section>}
