...
type Result = { ok?: boolean; error?: string; followUps?: FollowUp[]; members?: Member[]; businessName?: string };
...
export function TradeServiceFollowUpWorkspace({ user }: { user: User }) {
  ...
  return <section className="dashboard-panel follow-up-workspace">
    <header className="follow-up-heading"><div><span>Service follow-ups</span><h2>Consent-aware reminder preparation</h2><p>Prepare lifecycle reminders from existing installed assets and service schedules. This workspace never sends a message.</p></div><button type="button" onClick={() => void load()}>Refresh queue</button></header>
