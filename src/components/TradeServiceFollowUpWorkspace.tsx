...
type FollowUp = {
  ...
  brand: string; modelNumber: string; serviceType: string; cadenceMonths: number; dueAt: string; dueState: string;
  status: string; storedStatus: string; assigneeMemberId: string; assigneeLabel: string;
...
type Result = { ok?: boolean; error?: string; followUps?: FollowUp[]; members?: Member[] };
...
export function TradeServiceFollowUpWorkspace({ user }: { user: User }) {
  ...
  const [memberFilter, setMemberFilter] = useState(""); const [statusFilter, setStatusFilter] = useState("");
  ...
  async function update(item: FollowUp, action: string, success: string) {
    ...
    try {
      const token = await user.getIdToken(); const response = await fetch("/api/trade-service-follow-ups", {
        ...
        body: JSON.stringify({ action, servicePlanId: item.servicePlanId, dueAt: item.dueAt, expectedRevision: item.revision, ...edit }),
      });
      const result = await response.json().catch(() => ({})) as Result;
  ...
  const followUps = useMemo(() => data.followUps || [], [data.followUps]); const members = useMemo(() => data.members || [], [data.members]);
  ...
  const unassigned = followUps.filter((item) => !item.assigneeMemberId && item.status === "preparing").length;
  const completed = followUps.filter((item) => item.status === "completed").length;

  if (loading) return <section className="dashboard-panel follow-up-workspace"><div className="crm-empty"><strong>Building the service follow-up queue</strong><span>Checking asset schedules and assigned staff.</span></div></section>;
  return <section className="dashboard-panel follow-up-workspace">
    <header className="follow-up-heading"><div><span>Service follow-ups</span><h2>Track service follow-ups</h2><p>Assign staff, record notes and track due services. Previous delivery receipts remain available.</p></div><button type="button" onClick={() => void load()}>Refresh queue</button></header>
    <section className="follow-up-metrics"><article><strong>{followUps.length}</strong><span>active service plans</span></article><article><strong>{followUps.filter((item) => item.dueState === "overdue").length}</strong><span>overdue</span></article><article><strong>{unassigned}</strong><span>unassigned</span></article><article><strong>{completed}</strong><span>completed</span></article></section>
    <div className="follow-up-filters"><label><span>Customer</span><input value={customerFilter} placeholder="Name or number" onChange={(event) => setCustomerFilter(event.target.value)} /></label><label><span>Site</span><select value={siteFilter} onChange={(event) => setSiteFilter(event.target.value)}><option value="">All sites</option>{sites.map((site) => <option key={site}>{site}</option>)}</select></label><label><span>Asset</span><select value={assetFilter} onChange={(event) => setAssetFilter(event.target.value)}><option value="">All categories</option>{categories.map((category) => <option key={category}>{readable(category)}</option>)}</select></label><label><span>Due state</span><select value={dueFilter} onChange={(event) => setDueFilter(event.target.value)}><option value="">All due states</option><option value="overdue">Overdue</option><option value="due_soon">Due soon</option><option value="upcoming">Upcoming</option></select></label><label><span>Assignee</span><select value={memberFilter} onChange={(event) => setMemberFilter(event.target.value)}><option value="">All staff</option><option value="unassigned">Unassigned</option>{members.map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></label><label><span>Status</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All follow-up states</option><option value="preparing">Preparing</option><option value="suppressed">Suppressed</option><option value="completed">Completed</option></select></label></div>
    <div className="follow-up-list">{visible.map((item) => { const edit = edits[item.key] || { memberId: item.assigneeMemberId, internalNotes: item.internalNotes, suppressionReason: item.suppressionReason }; return <article key={item.key} className={`due-${item.dueState}`}><header><div><span>{readable(item.dueState)} | due {item.dueAt}</span><h3>{item.customerName}</h3><p>{item.customerNumber} | {item.siteLabel}{item.siteSummary ? ` | ${item.siteSummary}` : ""}</p></div><div><strong>{readable(item.status)}</strong></div></header><section className="follow-up-asset"><div><span>{readable(item.assetCategory)}</span><strong>{item.brand} {item.modelNumber}</strong><small>{readable(item.serviceType)} | every {item.cadenceMonths} months{item.lastServicedAt ? ` | last serviced ${item.lastServicedAt}` : ""}</small></div><div><span>Source record</span><strong>{item.workNumber || "Asset register"}</strong><small>{item.protectedJob ? "Authorised customer ownership" : "Direct customer record"}</small></div></section><div className="follow-up-controls"><label><span>Assigned staff</span><select value={edit.memberId} onChange={(event) => setEdits((current) => ({ ...current, [item.key]: { ...edit, memberId: event.target.value } }))}><option value="">Unassigned</option>{members.map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></label><label><span>Internal notes</span><textarea maxLength={1000} value={edit.internalNotes} placeholder="Internal context only" onChange={(event) => setEdits((current) => ({ ...current, [item.key]: { ...edit, internalNotes: event.target.value } }))} /></label><label><span>Suppression reason</span><input maxLength={300} value={edit.suppressionReason} placeholder="Required only when suppressing" onChange={(event) => setEdits((current) => ({ ...current, [item.key]: { ...edit, suppressionReason: event.target.value } }))} /></label></div>{item.reminderSubject && <section className="follow-up-draft"><span>Previous reminder draft</span><strong>{item.reminderSubject}</strong><p>{item.reminderBody}</p></section>}{Boolean(item.deliveries.length) && <section className="follow-up-deliveries">{item.deliveries.map((delivery) => <article key={delivery.id}><strong>{delivery.channel.toUpperCase()} | {readable(delivery.status)}</strong><span>{delivery.providerStatus || delivery.lastError || delivery.provider}</span><small>{delivery.deliveredAt || delivery.sentAt || delivery.queuedAt}</small></article>)}</section>}<footer><button type="button" disabled={Boolean(busy)} onClick={() => void update(item, "save_preparation", "Follow-up saved.")}>Save follow-up</button><button type="button" disabled={!edit.suppressionReason || Boolean(busy)} onClick={() => void update(item, "suppress", "Follow-up suppressed and audited.")}>Suppress</button>{item.status === "completed" ? <button type="button" disabled={Boolean(busy)} onClick={() => void update(item, "reopen", "Follow-up reopened.")}>Reopen</button> : <button type="button" disabled={Boolean(busy)} onClick={() => void update(item, "complete", "Follow-up completed.")}>Complete</button>}</footer></article>; })}{!visible.length && <div className="crm-empty"><strong>No follow-ups match this view</strong><span>Change a filter or add an active service plan to a confirmed customer asset.</span></div>}</div>
