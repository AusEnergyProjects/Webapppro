...
export function HomeEnergyPlanner({ initialSelection }: { initialSelection?: InitialPlannerSelection }) {
  ...
  return <section className="planner-layout" aria-label="Home energy planning tool">
      ...
      <section className="planner-results" aria-live="polite" aria-labelledby="planner-results-title"><div className="planner-results-heading"><span>Your ordered roadmap</span><h2 id="planner-results-title">{plan.title}</h2><p>{plan.summary}</p></div><div className="planner-result-actions"><a className="planner-save-account" href={`/account/projects/new?${new URLSearchParams({ goal, pace, situation }).toString()}${features.map((feature) => `&feature=${encodeURIComponent(feature)}`).join("")}`}>Save to my free account</a><a href={printableHref}>Open fast print view</a><button type="button" className="planner-reset" onClick={resetPlan}>Start over</button></div><ol>{plan.items.map((item, index) => <li key={item.id}><span className="planner-order">{String(index + 1).padStart(2, "0")}</span><div><small>{item.stage}</small><h3>{item.title}</h3><p>{item.text}</p><a href={item.href}>{item.action}</a></div></li>)}</ol><div className="planner-boundary"><strong>Before committing</strong><p>Replace indicative assumptions with current written quotes, confirm official incentives and approvals, and use licensed professionals for regulated work.</p></div></section>
    </section>;
}
