...
type Member = { id: string; displayName: string; status: string; isOwner: boolean };
...
type ScheduleResult = { ok?: boolean; error?: string; weekStart?: string; weekEnd?: string; rangeStart?: string; rangeEnd?: string; rangeWeeks?: number; calendarSync?: AppointmentCalendarSync; access?: { permissions?: Pick<TradeTeamPermissions, "canAssignJobs" | "canRescheduleJobs" | "jobScope" | "scheduleScope"> }; members?: Member[]; workingHours?: WorkingHours[]; unavailability?: Unavailability[]; appointments?: Appointment[]; rescheduleRequests?: RescheduleRequest[]; unassignedJobs?: Job[] };
...
export function TradeScheduleWorkspace({ user, permissions, onOpenJob = () => undefined, onOpenQuote, initialWeekStart }: { user: User; permissions?: TradeTeamPermissions; onOpenJob?: (workOrderId: string) => void; onOpenQuote?: (workOrderId: string) => void; initialWeekStart?: string }) {
  ...
  const canManageAvailability = canRescheduleJobs;
  ...
  async function update(body: Record<string, unknown>, key: string, success: string, responseWeekStart = activeWeekStart) {
    ...
    if (["schedule_appointment", "schedule_job", "review_reschedule_request"].includes(action) && !canRescheduleJobs) {
