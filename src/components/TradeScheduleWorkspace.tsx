...
export function TradeScheduleWorkspace({ user, onOpenJob = () => undefined, initialWeekStart }: { user: User; onOpenJob?: (workOrderId: string) => void; initialWeekStart?: string }) {
  ...
  const pendingScrollDateRef = useRef(initialTarget);
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const [scheduleResponse, calendarResponse] = await Promise.all([
        fetch(`/api/trade-schedule?rangeStart=${rangeStart}&rangeWeeks=${SCHEDULE_RANGE_WEEKS}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal }),
        fetch("/api/trade-calendar-sync", { headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal }),
      ]);
        ...
        if (calendarReturn) {
          ...
          if (calendarReturn.status === "cancelled") setStatus(`${label} connection cancelled. Nothing was changed.`);
            ...
            const firstSyncResponse = await fetch("/api/trade-calendar-sync", {
              ...
              body: JSON.stringify({ weekStart: addDays(rangeStart, SCHEDULE_LEADING_WEEKS * 7) }),
            });
  ...
  useEffect(() => {
    ...
    if (!data.rangeStart || !target) return;
    pendingScrollDateRef.current = "";
    const frame = window.requestAnimationFrame(() => {
      if (!timetableScrollRef.current) return;
      const dayOffset = Math.max(0, calendarDayDistance(data.rangeStart || rangeStart, target));
      timetableScrollRef.current.scrollLeft = dayOffset * SCHEDULE_DAY_WIDTH;
    });
  ...
  useEffect(() => {
    ...
    pendingScrollDateRef.current = target; setActiveWeekStart(target);
  ...
  function scrollToWeek(targetWeek: string, behavior: ScrollBehavior = "smooth") {
    const container = timetableScrollRef.current;
    if (!container) return;
    const dayOffset = calendarDayDistance(displayRangeStart, targetWeek);
    container.scrollTo({ left: Math.max(0, dayOffset * SCHEDULE_DAY_WIDTH), behavior });
  }
  ...
  function goToWeek(value: string) {
    ...
    if (targetWeek < displayRangeStart || addDays(targetWeek, 7) > displayRangeEnd) {
      pendingScrollDateRef.current = targetWeek;
      setRangeStart(addDays(targetWeek, -SCHEDULE_LEADING_WEEKS * 7));
      return;
    }
    window.requestAnimationFrame(() => scrollToWeek(targetWeek));
  }
  ...
  function handleScheduleScroll() {
    const container = timetableScrollRef.current;
    if (!container) return;
    const dayOffset = Math.max(0, Math.min(SCHEDULE_RANGE_DAYS - 1, Math.floor((container.scrollLeft + SCHEDULE_DAY_WIDTH / 2) / SCHEDULE_DAY_WIDTH)));
  ...
  return <section className="dashboard-panel schedule-workspace" aria-busy={loading}>
    <header className="schedule-heading"><div><span>Plan the work</span><h2>Scroll across the schedule</h2><p>Eight weeks stay together. Drag on desktop, or tap an appointment to move it on phone or tablet.</p></div><div className="schedule-week-nav"><button type="button" onClick={() => goToWeek(addDays(activeWeekStart, -7))}>Earlier</button><button type="button" onClick={() => goToWeek(monday())}>Today</button><label><span>View week containing</span><input type="date" value={activeWeekStart} onChange={(event) => { if (event.target.value) goToWeek(event.target.value); }} /></label><strong>{activeWeekStart} to {addDays(activeWeekStart, 6)}</strong><button type="button" onClick={() => goToWeek(addDays(activeWeekStart, 7))}>Later</button></div></header>
    ...
    <section className="schedule-capacity"><header><strong>Week of {activeWeekStart}</strong><span>{unassignedCount} jobs waiting | {appointments.filter((item) => item.conflicts && item.startsAt.slice(0, 10) >= activeWeekStart && item.startsAt.slice(0, 10) < addDays(activeWeekStart, 7)).length} conflicts</span></header><div>{capacity.map(({ member, available, booked, percent }) => <article key={member.id}><span><i className={`schedule-person-dot ${colourFor(member.id)}`} />{memberLabel(member)}<small>{member.isOwner ? "Owner" : readable(member.role)}</small></span><strong>{Math.round(booked / 60)}h booked of {Math.round(available / 60)}h</strong><div><i style={{ width: `${percent}%` }} /></div></article>)}</div></section>
    ...
    <div ref={timetableScrollRef} className="schedule-timetable-scroll" onScroll={handleScheduleScroll} onDragOver={(event) => { if (draggingId) autoScrollDuringDrag(event.clientX, event.clientY); }}>
      <div className="schedule-timetable" style={{ gridTemplateColumns: `${SCHEDULE_TIME_RAIL_WIDTH}px repeat(${days.length}, ${SCHEDULE_DAY_WIDTH}px)`, minWidth: `${SCHEDULE_TIME_RAIL_WIDTH + days.length * SCHEDULE_DAY_WIDTH}px` }}>
        <div className="schedule-time-rail" style={{ background: "#fff", left: 0, position: "sticky", zIndex: 20 }}><div className="schedule-time-heading">Time</div><div className="schedule-time-track">{timeLabels.map((minute) => <span key={minute} style={{ top: `${((minute - GRID_START_MINUTE) / 15) * GRID_QUARTER_HEIGHT}px` }}>{formatTime(`2000-01-01T${minuteLabel(minute)}`)}</span>)}</div></div>
        {days.map((date) => {
          ...
          return <section key={date} className={`schedule-day-track${dropTarget === date ? " drop-target" : ""}${dayIsPast ? " past" : ""}`} style={startsWeek ? { borderLeft: "2px solid #9fc8ba" } : undefined}>
            <header><strong>{shortDays[new Date(`${date}T00:00:00Z`).getUTCDay()]}</strong><span>{date.slice(5)}</span></header>
            <div className="schedule-day-grid" style={{ height: `${((GRID_END_MINUTE - GRID_START_MINUTE) / 15) * GRID_QUARTER_HEIGHT}px` }}
              onDragOver={(event) => {
              ...
              {dropTarget === date && <span className="schedule-drop-guide" style={{ top: `${((dropMinute - GRID_START_MINUTE) / 15) * GRID_QUARTER_HEIGHT}px` }}>{formatTime(`${date}T${minuteLabel(dropMinute)}`)}</span>}
              {dayAppointments.map((item) => {
                const startMinute = minuteValue(item.startsAt.slice(11, 16)); const duration = appointmentDurationMinutes(item.startsAt, item.endsAt); const top = Math.max(0, ((startMinute - GRID_START_MINUTE) / 15) * GRID_QUARTER_HEIGHT); const height = Math.max(62, (duration / 15) * GRID_QUARTER_HEIGHT);
                ...
                return <article key={item.id} draggable={!busy} tabIndex={0} role="button" aria-label={`View appointment for ${cardLabel}`} className={`schedule-block ${colourFor(item.assigneeMemberId)}${item.conflicts ? " conflict" : ""}${selectedAppointmentId === item.id ? " selected" : ""}${draggingId === item.id ? " dragging" : ""}`} style={{ top: `${top}px`, height: `${height}px`, left: `calc(${lane.lane * 100 / lane.laneCount}% + 4px)`, right: "auto", width: `calc(${100 / lane.laneCount}% - 8px)` }}
                  ...
                  onDragEnd={() => { setDraggingId(""); setDropTarget(""); setDropMinute(GRID_START_MINUTE); window.setTimeout(() => { suppressCardClickRef.current = false; }, 0); }}>
              ...
              {!dayAppointments.length && <span className="schedule-free-day">{dayIsPast ? "Past" : "Free"}</span>}
            </div>
          </section>;
        })}
      </div>
