...
export function TradeScheduleWorkspace({ user, onOpenJob = () => undefined }: { user: User; onOpenJob?: (workOrderId: string) => void }) {
  ...
  async function moveAppointment(appointment: Appointment, targetDate: string) {
    ...
    try {
      const currentStart = editStart(edit);
      const moved = moveAppointmentToDate(currentStart, appointmentEndsAt(currentStart, edit.durationMinutes), targetDate, minimumStart);
      await update({ action: "schedule_appointment", appointmentId: appointment.id, expectedRevision: appointment.revision,
        memberId: edit.memberId, startsAt: moved.startsAt, durationMinutes: edit.durationMinutes }, `appointment:${appointment.id}`, `${appointment.workNumber} moved to ${targetDate}.`);
  ...
  return <section className="dashboard-panel schedule-workspace">
    <header className="schedule-heading"><div><span>Plan the week</span><h2>See the week at a glance</h2><p>Choose a start time, set the job length and drag the block to another available day.</p></div><div className="schedule-week-nav"><button type="button" onClick={() => setWeekStart(addDays(weekStart, -7))}>Previous</button><button type="button" onClick={() => setWeekStart(monday())}>This week</button><strong>{weekStart} to {addDays(weekStart, 6)}</strong><button type="button" onClick={() => setWeekStart(addDays(weekStart, 7))}>Next</button></div></header>
    ...
    <p className="schedule-drag-note">Drag an appointment to another day as a time block. Double-click it to open the job, or select it to change the exact time and duration.</p>
    <div className="schedule-timetable-scroll"><div className="schedule-timetable">
      ...
      {days.map((date) => { const dayIsPast = date < minimumStart.slice(0, 10); const dayAppointments = visibleAppointments.filter((item) => item.startsAt.slice(0, 10) === date); return <section key={date} className={`schedule-day-track${dropTarget === date ? " drop-target" : ""}${dayIsPast ? " past" : ""}`}
        onDragOver={(event) => { if (!dayIsPast && draggingId) { event.preventDefault(); setDropTarget(date); } }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropTarget(""); }} onDrop={(event) => { event.preventDefault(); const appointment = appointments.find((item) => item.id === (draggingId || event.dataTransfer.getData("text/plain"))); if (appointment) void moveAppointment(appointment, date); }}>
        <header><strong>{shortDays[new Date(`${date}T00:00:00Z`).getUTCDay()]}</strong><span>{date.slice(5)}</span></header><div className="schedule-day-grid" style={{ height: `${((GRID_END_MINUTE - GRID_START_MINUTE) / 15) * GRID_QUARTER_HEIGHT}px` }}>{dayAppointments.map((item) => {
