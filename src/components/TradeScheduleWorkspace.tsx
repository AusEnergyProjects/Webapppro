...
export function TradeScheduleWorkspace({ user, permissions, onOpenJob = () => undefined, onOpenQuote, initialWeekStart, variant = "full", proposal, refreshNonce = 0, focusedMemberId, proposalStatusId = "trade-schedule-proposal-status", onProposalValidation, onProposalChange, onScheduleChanged }: TradeScheduleWorkspaceProps) {
  ...
  }), [activeWeekStart, data.rangeStart, data.rangeWeeks, data.unavailability, failedWeekStart, loadError, loading, memberFilter, members, proposal, proposalEndsAt]);
  ...
  }), [appointments, jobQuery, memberFilter, serviceFilter, siteFilter, statusFilter]);
  const visibleJobs = useMemo(() => (data.unassignedJobs || []).filter((item) => (!memberFilter || item.assigneeMemberId === memberFilter) && (!jobQuery || `${item.workNumber} ${item.title} ${item.customerDisplayName} ${item.suburbLabel}`.toLowerCase().includes(jobQuery)) && (!serviceFilter || item.serviceCategory === serviceFilter) && (!siteFilter || item.siteLabel === siteFilter) && statusFilter !== "scheduled" && (statusFilter !== "unassigned" || !item.assigneeMemberId)), [data.unassignedJobs, jobQuery, memberFilter, serviceFilter, siteFilter, statusFilter]);
      ...
      : proposalValidation.status === "not_visible"
        ...
        : proposalValidation.status === "unavailable"
          ...
          : proposalValidation.status === "clear"
              ? "This time can be booked, including alongside another appointment. TLink checks unavailability again when you save."
          ...
          return <section key={date} aria-label={`${dayIsToday ? "Today, " : ""}${formatDay(date)}`} className={`schedule-day-track${dropTarget === date ? " drop-target" : ""}${dayIsPast ? " past" : ""}${dayIsToday ? " today" : ""}`}>
            ...
            <div data-schedule-date={date} className={`schedule-day-grid${jobCalendar && onProposalChange && !dayIsPast ? " proposal-selectable" : ""}`} style={{ height: `${gridHeight}px` }}
              ...
              {dayAppointments.map((item) => {
                ...
                const canEditAppointment = calendarCanReschedule && item.status !== "completed";
                return <Fragment key={item.id}><article data-schedule-appointment draggable={canEditAppointment && !busy && !loading} tabIndex={pageIsActive ? 0 : -1} role="button" aria-label={`View appointment for ${cardLabel}`} className={`schedule-block moveable ${colourFor(item.assigneeMemberId)}${height < 62 ? " compact" : ""}${height <= 16 ? " micro" : ""}${effectiveConflict ? " conflict" : ""}${item.scheduleDraft ? " draft" : ""}${selectedAppointmentId === item.id ? " selected" : ""}${draggingId === item.id ? " dragging" : ""}`} style={{ top: `${top}px`, height: `${height}px`, left, right: "auto", width }}
                  ...
                  <strong>{item.customerDisplayName}</strong><small>{item.assigneeLabel || "Unassigned"}</small><em>{item.suburbLabel}</em><span>{formatTime(item.startsAt)} | {durationLabel(duration)}</span>{item.status === "completed" && <b>Completed</b>}{item.scheduleDraft && <b>Unsaved</b>}{item.outsideWorkingHours && <b>Outside hours</b>}{effectiveConflict && <b>Unavailable</b>}
                </article>{canEditAppointment && !busy && !loading && <span
