...
async function providerRequest(provider: CalendarProvider, accessToken: string, externalEventId: string, payload: Record<string, unknown>) {
  ...
  let response = await calendarProviderFetch(url, { method: updating ? "PATCH" : "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(requestPayload) });
  if (updating && response.status === 404) {
    ...
    response = await calendarProviderFetch(createUrl, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(payload) });
  }
...
export async function syncCalendarConnections(ownerUid: string, connections: Row[], appointments: Row[]) {
  const db = getD1(); let synced = 0; let failed = 0;
  for (const connection of connections) {
    const provider = String(connection.provider) as CalendarProvider;
    if (!CALENDAR_PROVIDERS.includes(provider)) continue;
    try {
      const result = await syncProvider(ownerUid, provider, connection, appointments);
...
export async function syncCreatedAppointmentToConnectedCalendars(ownerUid: string, appointmentId: string) {
  const db = getD1();
  ...
  if (!connections.results.length) return { connected: 0, synced: 0, failed: 0 };
  if (!appointment) throw new Error("CALENDAR_APPOINTMENT_NOT_FOUND");
  const result = await syncCalendarConnections(ownerUid, connections.results, [appointment]);
