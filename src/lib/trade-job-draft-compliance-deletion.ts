...
function balancedAnd(conditions: readonly string[]): string {
  if (conditions.length === 1) return conditions[0];
  const middle = Math.floor(conditions.length / 2);
  return `(${balancedAnd(conditions.slice(0, middle))} AND ${balancedAnd(conditions.slice(middle))})`;
}

function unfinishedCaseSql(c: string, balanced = true) {
  ...
  const conditions = [`${c}.status = 'draft'`, `${c}.evidence_status IN ('not_started', 'in_progress')`,
    `${c}.created_by_type = 'installer'`, `${c}.created_by_uid = ${c}.installer_uid`,
    `EXISTS (SELECT 1 FROM trade_work_order_compliance_intents intent
    ...
    `NOT EXISTS (SELECT 1 FROM compliance_case_events event WHERE event.case_id = ${c}.id
    ...
    `NOT EXISTS (SELECT 1 FROM compliance_case_evidence evidence WHERE evidence.case_id = ${c}.id
    ...
    `NOT EXISTS (SELECT 1 FROM compliance_activity_work_pack_browser_upload_receipts upload
    ...
    `NOT EXISTS (SELECT 1 FROM compliance_activity_work_pack_artifacts artifact
      WHERE artifact.case_instance_id IN (${instances}) AND artifact.organisation_id <> ${c}.organisation_id)`,
    `NOT EXISTS (SELECT 1 FROM compliance_manual_policy_composition_locks lock
      WHERE lock.reference_type = 'compliance_case' AND lock.reference_id = ${c}.id
        AND lock.organisation_id <> ${c}.organisation_id)`,
    `NOT EXISTS (SELECT 1 FROM compliance_equipment_records equipment WHERE equipment.case_id = ${c}.id
      AND (equipment.organisation_id <> ${c}.organisation_id OR NOT (${referencedDependency("equipment.id")})))`,
    `NOT EXISTS (SELECT 1 FROM compliance_calculation_runs calculation WHERE calculation.case_id = ${c}.id
    ...
    ...retainedCaseChildren.map(([table, column]) => `NOT EXISTS (SELECT 1 FROM ${table} retained WHERE retained.${column} = ${c}.id)`),
    ...retainedWorkPackChildren.map((table) => `NOT EXISTS (SELECT 1 FROM ${table} retained WHERE retained.case_instance_id IN (${instances}))`),
    `NOT EXISTS (SELECT 1 FROM compliance_field_custody_test_artifacts retained WHERE retained.evidence_id IN (${evidence}))`,
    `NOT EXISTS (SELECT 1 FROM compliance_field_custody_acceptance_records retained WHERE retained.evidence_id IN (${evidence}))`,
    `NOT EXISTS (SELECT 1 FROM compliance_governance_requests retained WHERE retained.target_id = ${c}.id)`,
    `NOT EXISTS (SELECT 1 FROM compliance_audit_events audit WHERE audit.target_id = ${c}.id
  ...
  // D1 caps expression trees at 100 levels. Group the same conjunction into a
  // balanced tree; the flat form is retained only to recognise 570/571 guards.
  return balanced ? balancedAnd(conditions) : conditions.join(' AND ');
}
...
function allowedCaseSql(caseSql: string, organisationSql: string, balanced = true) {
  return `EXISTS (SELECT 1 FROM compliance_cases c WHERE c.id = ${caseSql} AND c.organisation_id = ${organisationSql}
    AND ${unfinishedCaseSql("c", balanced)} AND ${jobDeletionPermitSql("c.work_order_id", "c.installer_uid")})`;
}
function allowedInstanceSql(instanceSql: string, organisationSql: string, balanced = true) {
  return `EXISTS (SELECT 1 FROM compliance_activity_work_pack_instances parent WHERE parent.id = ${instanceSql}
    AND parent.organisation_id = ${organisationSql} AND ${allowedCaseSql("parent.compliance_case_id", "parent.organisation_id", balanced)})`;
}
...
function complianceGuards(balanced: boolean): readonly JobDeletionGuard[] { return [
  guard("compliance_cases_no_delete", "compliance_cases", `${unfinishedCaseSql("OLD", balanced)} AND ${jobDeletionPermitSql("OLD.work_order_id", "OLD.installer_uid")}`,
  ...
  guard("compliance_case_events_no_delete", "compliance_case_events", allowedCaseSql("OLD.case_id", "OLD.organisation_id", balanced),
  ...
  guard("compliance_case_evidence_no_delete", "compliance_case_evidence", allowedCaseSql("OLD.case_id", "OLD.organisation_id", balanced),
  ...
  guard("compliance_equipment_records_no_delete", "compliance_equipment_records", allowedCaseSql("OLD.case_id", "OLD.organisation_id", balanced),
  ...
  guard("compliance_calculation_runs_no_delete", "compliance_calculation_runs", allowedCaseSql("OLD.case_id", "OLD.organisation_id", balanced),
  ...
  guard("compliance_evidence_integrity_receipts_no_delete", "compliance_evidence_integrity_receipts",
    `EXISTS (SELECT 1 FROM compliance_case_evidence evidence WHERE evidence.id = OLD.evidence_id AND evidence.organisation_id = OLD.organisation_id
      AND ${allowedCaseSql("evidence.case_id", "evidence.organisation_id", balanced)})`,
  ...
  guard("compliance_manual_policy_composition_delete_guard", "compliance_manual_policy_composition_locks",
    `OLD.reference_type = 'compliance_case' AND ${allowedCaseSql("OLD.reference_id", "OLD.organisation_id", balanced)}`,
...
function workPackGuards(balanced: boolean): readonly JobDeletionGuard[] { return [
  ["instance", "instances", allowedCaseSql("OLD.compliance_case_id", "OLD.organisation_id", balanced), "INSTANCE"],
  ["artifact", "artifacts", allowedInstanceSql("OLD.case_instance_id", "OLD.organisation_id", balanced), "ARTIFACT"],
  ["browser_upload", "browser_upload_receipts", allowedInstanceSql("OLD.case_instance_id", "OLD.organisation_id", balanced), "BROWSER_UPLOAD"],
...
function withPreviousSql(current: readonly JobDeletionGuard[], previous: readonly JobDeletionGuard[]) {
  return current.map((definition, index) => ({ ...definition, legacySqlVariants: [previous[index].sql] }));
}
export const draftComplianceDeletionGuardDefinitions = withPreviousSql(complianceGuards(true), complianceGuards(false));
export const draftWorkPackDeletionGuardDefinitions = withPreviousSql(workPackGuards(true), workPackGuards(false));
