...
export async function accountEntitlements(
  ...
  const rows = await getD1().prepare(`SELECT feature_key, status, expires_at, note, updated_at
    FROM trade_account_feature_grants WHERE firebase_uid = ? AND status = 'active'`)
    .bind(firebaseUid).all<Record<string, unknown>>();
  const grants = rows.results.map((row: Record<string, unknown>) => ({
  ...
  return resolveEntitlements(partnerType, billingStatus, grants);
}
