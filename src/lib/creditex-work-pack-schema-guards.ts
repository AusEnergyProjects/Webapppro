...
const CREDITEX_WORK_PACK_SCHEMA_GUARD_BASE_DEFINITIONS = [
  ...
  { name: "compliance_output_dispatch_update_guard", sql: "CREATE TRIGGER IF NOT EXISTS compliance_output_dispatch_update_guard\nBEFORE UPDATE ON compliance_output_dispatch_intents\nBEGIN\n  SELECT CASE WHEN OLD.status <> 'dispatching' OR NEW.status NOT IN ('uncertain', 'completed')\n    OR NEW.id <> OLD.id OR NEW.organisation_id <> OLD.organisation_id\n    OR NEW.packet_id <> OLD.packet_id OR NEW.packet_sha256 <> OLD.packet_sha256\n    OR NEW.adapter_id <> OLD.adapter_id OR NEW.requested_by_uid <> OLD.requested_by_uid\n    OR NEW.started_at <> OLD.started_at\n    THEN RAISE(ABORT, 'OUTPUT_ACTION_DISPATCH_IMMUTABLE') END;\n  SELECT CASE WHEN NEW.status = 'completed' AND NOT EXISTS (\n    SELECT 1 FROM compliance_output_action_adapter_receipts receipt\n    JOIN compliance_output_action_events event ON event.adapter_receipt_id = receipt.id\n      AND event.organisation_id = receipt.organisation_id AND event.packet_id = receipt.packet_id\n    WHERE receipt.id = NEW.adapter_receipt_id AND receipt.organisation_id = NEW.organisation_id\n      AND receipt.packet_id = NEW.packet_id AND receipt.adapter_id = NEW.adapter_id\n  ) THEN RAISE(ABORT, 'OUTPUT_ACTION_DISPATCH_RECEIPT_REQUIRED') END;\nEND;" },
  { name: "compliance_output_dispatch_delete_guard", sql: "CREATE TRIGGER IF NOT EXISTS compliance_output_dispatch_delete_guard\nBEFORE DELETE ON compliance_output_dispatch_intents\nBEGIN\n  SELECT RAISE(ABORT, 'OUTPUT_ACTION_DISPATCH_DELETE_BLOCKED');\nEND;" },
  { name: "compliance_output_dispatch_manual_receipt_guard", sql: "CREATE TRIGGER IF NOT EXISTS compliance_output_dispatch_manual_receipt_guard\nBEFORE INSERT ON compliance_output_action_adapter_receipts\nWHEN NEW.adapter_id = 'manual-provider-record/v1' AND NEW.provider_status = 'submitted'\nBEGIN\n  SELECT CASE WHEN EXISTS (SELECT 1 FROM compliance_output_dispatch_intents intent\n    WHERE intent.organisation_id = NEW.organisation_id AND intent.packet_id = NEW.packet_id)\n    THEN RAISE(ABORT, 'OUTPUT_ACTION_DISPATCH_ALREADY_RESERVED') END;\nEND;" },
  { name: "compliance_output_dispatch_manual_event_guard", sql: "CREATE TRIGGER IF NOT EXISTS compliance_output_dispatch_manual_event_guard\nBEFORE INSERT ON compliance_output_action_events\nWHEN NEW.to_status = 'submitted' AND NEW.actor_kind IN ('admin', 'compliance')\nBEGIN\n  SELECT CASE WHEN EXISTS (SELECT 1 FROM compliance_output_dispatch_intents intent\n    WHERE intent.organisation_id = NEW.organisation_id AND intent.packet_id = NEW.packet_id)\n    THEN RAISE(ABORT, 'OUTPUT_ACTION_DISPATCH_ALREADY_RESERVED') END;\nEND;" },
...
export const CREDITEX_WORK_PACK_REQUIRED_SCHEMA_TABLES = [
  ...
  "compliance_activity_work_pack_artifacts",
  ...
  "compliance_output_dispatch_intents",
  "compliance_output_action_packets",
  "compliance_output_action_reviews",
  "compliance_output_action_adapter_receipts",
  "compliance_output_action_events",
  "compliance_sres_activation_records",
  "compliance_sres_activation_reviews",
  "compliance_sres_activation_snapshots",
] as const;

const INSTALL_BATCH_SIZE = 24;
const readinessByDatabase = new WeakMap<object, Promise<void>>();

export function canonicalCreditexWorkPackSchemaGuardSql(sql: string) {
  ...
  for (let index = 0; index < normalisedPrefix.length; index += 1) {
    ...
    if (quote) {
      ...
      if (quote === "'" && normalisedPrefix[index + 1] === "'") {
...
async function requireCreditexSchemaMigrations(
  database: D1Database,
  requiredTables: readonly string[],
  errorCode: string,
) {
  const tableNames = requiredTables
    .map((name) => `'${name}'`)
    .join(", ");
  const rows = await database.prepare(
  ...
  const installed = new Set(rows.results.map((row) => String(row.name)));
...
async function installCreditexSchemaGuards(
  ...
  await requireCreditexSchemaMigrations(
    database,
    requiredTables,
  ...
  if (mismatched.length) {
    throw new Error(
      `${errorPrefix}_GUARD_MISMATCH:${mismatched
  ...
  for (let offset = 0; offset < missing.length; offset += INSTALL_BATCH_SIZE) {
    const batch = missing.slice(offset, offset + INSTALL_BATCH_SIZE);
  ...
  const unavailable = definitions.filter(
    (definition) => canonicalCreditexWorkPackSchemaGuardSql(
      verified.get(definition.name) || "",
    ) !== canonicalCreditexWorkPackSchemaGuardSql(definition.sql),
  );
  if (unavailable.length) {
    throw new Error(
      `${errorPrefix}_GUARDS_UNAVAILABLE:${unavailable
        .map((definition) => definition.name)
        .join(",")}`,
    );
  }
...
async function ensureCreditexSchemaGuards(
  database: D1Database,
  readinessByDatabase: WeakMap<object, Promise<void>>,
