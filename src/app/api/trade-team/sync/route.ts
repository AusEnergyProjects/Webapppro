...
async function accessibleJobs(access: TeamAccess) {
  const db = getD1();
  const [jobRows, taskRows, mediaRows, formRows, complianceRows, rentalRows] = await Promise.all([
      ...
      ...(() => {
        ...
        return { rentalInspection: {
          ...
          permissions: {
            ...
            canIssue: access.canRunReports && access.memberId === String(rental.assessor_member_id || "") && !terminal,
          },
        } };
      })(),
      ...(() => {
...
async function fieldFinishState(ownerUid: string, workOrderId: string) {
  const db = getD1();
  const [tasks, forms, issues, plan, compliance, rentalReport, photo] = await Promise.all([
