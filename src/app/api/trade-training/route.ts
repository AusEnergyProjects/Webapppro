  ...
  try {
    ...
    if (access.isOwner || access.canManageTeam) {
      ...
      for (const member of members.results) {
        ...
        team.push({ memberId: member.id, displayName: member.display_name, officeOnly: memberScope.officeOnly, modules: (await getTrainingModulesForMember(db, access.ownerUid, member.id, data, memberScope)).filter(module => memberScope.assignedModuleIds.includes(module.id)).map(module => ({ id: module.id, title: module.title, serviceCategory: module.serviceCategory, businessServiceEnabled: module.businessServiceEnabled, status: module.status, reference: module.completion?.reference || '', expiresAt: module.completion?.expiresAt || '' })) });
      }
    }
