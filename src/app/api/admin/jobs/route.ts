...
import { adminJobQuery, ADMIN_JOB_JOINS,ADMIN_JOB_STATUS_SQL } from "@/lib/admin-job-register";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const db = getD1();
    const [rows, count, facets] = await Promise.all([db.prepare(`SELECT w.id, w.work_number, w.title, w.service_category, ${ADMIN_JOB_STATUS_SQL} stage,w.record_status,
      ...
      db.prepare(`SELECT DISTINCT ${ADMIN_JOB_STATUS_SQL} stage, w.service_category, a.business_name
        FROM trade_work_orders w JOIN trade_accounts a ON a.firebase_uid = w.firebase_uid
        WHERE w.partner_type = 'installer' AND w.record_status = ?`).bind(query.recordStatus).all<{stage: string; service_category: string; business_name: string}>(),
    ]);
    ...
    return adminJson({ ok: true, pagination: { page: query.page, pageSize: query.pageSize, total, hasNext: query.offset + rows.results.length < total },
      ...
      stage: row.stage,recordStatus:row.record_status, siteArea: row.site_area, scheduledStart: row.scheduled_start,
