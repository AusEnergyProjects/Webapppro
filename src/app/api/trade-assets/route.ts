...
function errorResponse(error: unknown) {
  ...
  if (code === "INSTALLER_ONLY" || code === "TRADE_ROLE_REQUIRED") return adminJson({ ok: false, error: "The asset register is available to installer accounts only." }, 403);
  if (code === "ACCOUNT_INACTIVE" || code === "FULL_ACCESS_REQUIRED" || code === "ABN_REVIEW_REQUIRED" || code === "EMAIL_VERIFICATION_REQUIRED") return adminJson({ ok: false, error: "Complete trade verification before using the asset register." }, 403);
