...
function requiredBody(value: unknown): Body {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new CreditexOperationsError(
      ...
      400,
      "Enter a valid Creditex access request.",
    );
  }
...
function errorResponse(error: unknown) {
  ...
  if (
    ...
    return json({
      ok: false,
      code: "CREDITEX_INVITATION_EXISTS",
      error: "This person already has a membership or invitation record.",
    }, 409);
  }
  if (
    error instanceof Error
    ...
    return json({
      ...
      error: "The access record changed before the operation completed. Refresh and try again.",
    }, 409);
  }
  if (
    error instanceof Error
    ...
    return json({
      ok: false,
      code: "CREDITEX_STATE_CONFLICT",
      error: "The governed access record no longer satisfies its compliance constraints. Refresh and try again.",
    }, 409);
  }
  console.error("Creditex access request failed", error);
  return json({
    ok: false,
    code: "CREDITEX_ACCESS_UNAVAILABLE",
    error: "Creditex access records are temporarily unavailable. Try again.",
  }, 500);
}
