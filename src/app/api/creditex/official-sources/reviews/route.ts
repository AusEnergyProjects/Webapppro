...
import {
  ...
  listCreditexOfficialSourceReviewDecisions,
...
function bodyRecord(value: unknown) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new CreditexSourceLookupReviewError(
      "SOURCE_REVIEW_REQUEST_INVALID",
      400,
      "Enter a valid official source review decision.",
    );
  }
...
function errorResponse(error: unknown) {
  ...
  if (
    ...
    && (
      error.message.includes("COMPLIANCE_SOURCE_REVIEW_")
      || error.message.includes("SQLITE_CONSTRAINT")
    )
  ) {
    return json({
      ok: false,
      code: "SOURCE_REVIEW_STATE_CONFLICT",
      error: "The source review state changed. Refresh and review it again.",
    }, 409);
  }
  console.error("Creditex official source review failed", error);
  return json({
    ok: false,
    code: "SOURCE_REVIEW_UNAVAILABLE",
    error: "The official source decision could not be recorded safely.",
  }, 500);
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return json({
      ok: true,
      decisions: await listCreditexOfficialSourceReviewDecisions(
        database,
        member,
      ),
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (String(body.action || "").trim() !== "record_decision") {
      throw new CreditexSourceLookupReviewError(
        "SOURCE_REVIEW_ACTION_INVALID",
        400,
        "Choose the supported source review action.",
      );
    }
    const result = await reviewCreditexOfficialSource(
      database,
