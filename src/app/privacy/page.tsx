...
const sections = [
  ...
  },
  {
    ...
    body: "Up to 40 recent messages, a small conversation summary and the home profile you choose to complete are kept in that browser for up to 30 days so Surge AI can continue the discussion across Australian Energy Assessments pages. For an answer, the browser sends the current question, a bounded set of recent conversation turns and a bounded home-profile summary to Australian Energy Assessments' stateless guide endpoint. If you have completed steps in the home energy planner in the same browser tab, it may also send a bounded list of those saved plan answers so you do not need to repeat them. Planner photos, uploaded files and contact details are not included. Newer details you tell Surge AI override conflicting profile or saved-plan answers. The endpoint securely sends this bounded question context and relevant maintained energy guidance to an external AI processing provider with provider-side response storage disabled. It does not create or read a server-side conversation record, and conversation text is not placed in analytics. You can edit the home profile or clear the browser copy at any time. A random first-party security cookie and short-lived, one-way security counters are used to limit automated misuse. They do not contain conversation text, a raw network address or a cross-device identity and are not used for advertising or visitor analytics. No third-party tracking cookie or fingerprint is used. Trade mode does not read a locally saved household profile or plan, another customer or job unless an authorised platform workflow explicitly supplies that context.",
  },
  {
