...
const sections = [
  ...
  {
    title: "Surge conversations",
    body: "Up to 40 recent messages and a small conversation summary are kept in that browser for up to 30 days so Surge can continue the discussion across Australian Energy Assessments pages. For an answer, the browser sends the current question, a bounded set of recent user turns and that summary to Australian Energy Assessments' stateless guide endpoint. The endpoint securely sends the bounded question context and relevant maintained energy guidance to OpenAI for response generation with provider-side response storage disabled. It does not create or read a server-side conversation record, and conversation text is not placed in analytics. You can clear the browser copy at any time. No third-party tracking cookie, fingerprint or cross-device anonymous identity is used. Trade mode does not read another customer or job unless an authorised platform workflow explicitly supplies that context.",
  },
  {
