...
export default function DirectTradeStandardsPage() {
  return <main className="wrap direct-trade-standards-page">
    ...
    <section className="standards-intro" aria-labelledby="standards-principle-title"><div><span>Core principle</span><h2 id="standards-principle-title">The work should determine the trade, not the biggest sales margin</h2><p>Direct Trade Services is designed to shorten the path between a household, qualified installers and reputable product suppliers. Participation is reviewed, matching is based on project fit and the household receives the trade&apos;s own scope and quote.</p></div><aside><strong>Subscription is disclosed</strong><p>Paid membership will fund access to the service. It will not replace verification, purchase a favourable ranking or create a separate charge for each opportunity.</p></aside></section>
    ...
    <section className="standards-actions"><div><span>Choose your path</span><h2>Use the standards before making a connection</h2><p>Households can prepare a project brief. Installers and suppliers can create a business profile for direct review.</p></div><div><a className="btn" href="/direct-trade">Start a household brief</a><a className="btn ghost" href="/direct-trade/partners">Trade and supplier participation</a><a className="btn ghost" href="/direct-trade/membership">Membership and referrals</a></div></section>
