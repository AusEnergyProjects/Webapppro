...
export default function WholesaleElectricityPage() {
  return <main className="wrap">
    ...
    <header className={styles.intro}><h1>What is energy worth right now?</h1><p>Compare wholesale electricity with the closest available wholesale gas market or upstream reference on the same c/kWh scale. <strong>These are energy market prices, not the rates on your household bills.</strong> Choose a region to explore the last 24 hours.</p></header>
    ...
    <section className={styles.explainers} aria-label="Understanding wholesale energy prices">
      ...
      <div><h2>Which gas price is shown?</h2><p>NSW and the ACT use the Sydney gas hub as a nearby reference. Queensland uses Brisbane, South Australia uses Adelaide and Victoria has its own wholesale gas market. Tasmania uses Victoria&apos;s wholesale price as the nearest upstream reference because its pipeline supply comes from Longford, Victoria. It is not a Tasmanian market or household tariff. Western Australia and the Northern Territory are outside the NEM and are not shown.</p></div>
    </section>
