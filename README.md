...
## Live production and source control

The Australian Energy Assessments public website is live at [ausenergyassessments.com](https://ausenergyassessments.com). This apex hostname is the canonical public origin. Public, non-API requests to `https://compare.ausenergyassessments.com` are permanently redirected so older links continue to work; API routes remain available on the service host where required. Durable no longer serves the apex or `www` and is retained only as the emergency rollback target for the initial 48-hour monitoring window. Netlify and Vercel are not deployment targets for this repository.

The current public release is OpenAI Sites version 499 from application commit `4908a1467d90b2fff57b70517fad5e41e18e12b0`. The active release branch is `codex/surge-durability-release`; GitHub `main` is the default production source record and must remain fast-forward aligned with an accepted release. The OpenAI Sites managed `main` branch receives the exact validated application commit before a version is saved and deployed.

For every public release: validate the scoped change, commit only task-owned files, push the exact commit to GitHub, push the same application commit to the Sites source branch, save and deploy that commit, then verify the apex route, health endpoint, canonical redirect and release provenance. Update the canonical status documents for a material milestone, migration or live-domain change. Historical references to a pre-launch public website or the old comparison subdomain do not override the current status above. Protected TLink, mobile-preview and Creditex operational data remain separately governed and must not be treated as live customer data unless their own release record says so.
...
The current Google Apps Script processor is tracked at `integrations/google-apps-script/lead-email-relay.gs`. Website submissions use schema version 6 and one of five explicit events: electricity comparison results, electricity upgrades, gas upgrades, Direct Trade household projects and Direct Trade partner applications. A consented public home-plan enquiry is allocated to every active platform-approved installer whose declared services and active service area match at least one selected service and the household postcode. There is no paid ranking or recipient cap. Each allocation creates a durable trade notification and a protected CRM lead. Suppliers cannot access household opportunities. The household email, postcode, selected services and any written message are included for matching trades; the household independently chooses whether to share the name, phone or full property address. The protected administration record keeps the unit number, street address, suburb, state and postcode as separate fields. The full home plan and PDF stay private. Usage, meter files, NMIs, bill dates, plan results, scenario costs, savings and adjustment reasons are never placed in a handoff URL. Partner applications begin a structured review that cannot automatically approve or publicly list the applicant. Every delivered request receives an `AEA` reference shared by the browser acknowledgement, customer email, internal email and spreadsheet row. Customer and internal messages use the same navy, teal, emerald and restrained gold visual system as the website, include plain text fallbacks and show only fields relevant to that request.
