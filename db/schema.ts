...
export const tradeAccounts = sqliteTable("trade_accounts", {
  ...
  documentBusinessName: text("document_business_name").notNull().default(""),
  documentPhone: text("document_phone").notNull().default(""),
  documentEmail: text("document_email").notNull().default(""),
  bannerCropXBasisPoints: integer("banner_crop_x_basis_points").notNull().default(0),
  bannerCropYBasisPoints: integer("banner_crop_y_basis_points").notNull().default(0),
  bannerCropWidthBasisPoints: integer("banner_crop_width_basis_points").notNull().default(10000),
  bannerCropHeightBasisPoints: integer("banner_crop_height_basis_points").notNull().default(10000),
  ...
  invoicePaymentAccountName: text("invoice_payment_account_name").notNull().default(""),
  invoicePaymentBsb: text("invoice_payment_bsb").notNull().default(""),
  invoicePaymentAccountNumber: text("invoice_payment_account_number").notNull().default(""),
  invoicePaymentReference: text("invoice_payment_reference").notNull().default(""),
  invoiceDefaultTerms: text("invoice_default_terms").notNull().default(""),
...
export const tradeActivityCustomerDocumentDeliveries = sqliteTable("trade_activity_customer_document_deliveries", {
  id: text("id").primaryKey(),
  workOrderId: text("work_order_id").notNull().references(() => tradeWorkOrders.id, { onDelete: "restrict" }),
  appointmentId: text("appointment_id").notNull().references(() => tradeCrmAppointments.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  recipientEmailSha256: text("recipient_email_sha256").notNull(),
  activityBindings: text("activity_bindings").notNull(),
  documentIds: text("document_ids").notNull(),
  documentSha256Set: text("document_sha256_set").notNull(),
  ...
  uniqueIndex("trade_activity_customer_document_delivery_generation_idx").on(
    table.workOrderId,
    table.appointmentId,
    table.firebaseUid,
    table.recipientEmailSha256,
    table.packSha256,
    table.deliveryGeneration,
  ),
  uniqueIndex("trade_activity_customer_document_delivery_idempotency_idx").on(table.idempotencyKey),
  uniqueIndex("trade_activity_customer_document_delivery_provider_idx")
    .on(table.provider, table.providerMessageId)
    .where(sql`${table.providerMessageId} <> ''`),
  index("trade_activity_customer_document_delivery_job_idx").on(
    table.firebaseUid,
    table.workOrderId,
    table.appointmentId,
    table.deliveryGeneration,
  ),
  index("trade_activity_customer_document_delivery_status_idx").on(table.status, table.updatedAt),
  check("trade_activity_customer_document_delivery_recipient_hash_check", sql`length(${table.recipientEmailSha256}) = 64 AND ${table.recipientEmailSha256} = lower(${table.recipientEmailSha256}) AND ${table.recipientEmailSha256} NOT GLOB '*[^0-9a-f]*'`),
  check("trade_activity_customer_document_delivery_activity_bindings_check", sql`json_valid(${table.activityBindings}) AND json_type(${table.activityBindings}) = 'array' AND json_array_length(${table.activityBindings}) > 0`),
  check("trade_activity_customer_document_delivery_document_ids_check", sql`json_valid(${table.documentIds}) AND json_type(${table.documentIds}) = 'array' AND json_array_length(${table.documentIds}) > 0`),
  check("trade_activity_customer_document_delivery_document_hashes_check", sql`json_valid(${table.documentSha256Set}) AND json_type(${table.documentSha256Set}) = 'array' AND json_array_length(${table.documentSha256Set}) = json_array_length(${table.documentIds})`),
  check("trade_activity_customer_document_delivery_pack_hash_check", sql`length(${table.packSha256}) = 64 AND ${table.packSha256} = lower(${table.packSha256}) AND ${table.packSha256} NOT GLOB '*[^0-9a-f]*'`),
  check("trade_activity_customer_document_delivery_generation_check", sql`${table.deliveryGeneration} > 0`),
  check("trade_activity_customer_document_delivery_provider_check", sql`${table.provider} = 'resend'`),
  check("trade_activity_customer_document_delivery_status_check", sql`${table.status} IN ('queued','sending','provider_accepted','sent','delivered','failed','bounced','complained','suppressed')`),
  check("trade_activity_customer_document_delivery_binding_check", sql`(${table.providerMessageId} = '' AND ${table.status} IN ('queued','sending','failed','suppressed')) OR (${table.providerMessageId} <> '' AND ${table.status} IN ('provider_accepted','sent','delivered','failed','bounced','complained','suppressed'))`),
  check("trade_activity_customer_document_delivery_accepted_check", sql`(${table.status} IN ('provider_accepted','sent','delivered') AND ${table.acceptedAt} <> '' AND datetime(${table.acceptedAt}) IS NOT NULL) OR (${table.status} NOT IN ('provider_accepted','sent','delivered'))`),
  check("trade_activity_customer_document_delivery_failed_check", sql`(${table.status} IN ('failed','bounced','complained','suppressed') AND ${table.failedAt} <> '' AND datetime(${table.failedAt}) IS NOT NULL) OR (${table.status} NOT IN ('failed','bounced','complained','suppressed'))`),
  check("trade_activity_customer_document_delivery_time_check", sql`(${table.acceptedAt} = '' OR datetime(${table.acceptedAt}) IS NOT NULL) AND (${table.sentAt} = '' OR datetime(${table.sentAt}) IS NOT NULL) AND (${table.deliveredAt} = '' OR datetime(${table.deliveredAt}) IS NOT NULL) AND (${table.failedAt} = '' OR datetime(${table.failedAt}) IS NOT NULL) AND datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeActivityCustomerDocumentDeliveryEvents = sqliteTable("trade_activity_customer_document_delivery_events", {
  id: text("id").primaryKey(),
  deliveryId: text("delivery_id").notNull().references(() => tradeActivityCustomerDocumentDeliveries.id, { onDelete: "restrict" }),
  ...
  uniqueIndex("trade_activity_customer_document_delivery_events_provider_idx").on(table.providerEventKey),
  index("trade_activity_customer_document_delivery_events_delivery_idx").on(table.deliveryId, table.occurredAt),
  check("trade_activity_customer_document_delivery_events_identity_check", sql`trim(${table.providerEventKey}) <> '' AND trim(${table.eventType}) <> ''`),
  check("trade_activity_customer_document_delivery_events_time_check", sql`datetime(${table.occurredAt}) IS NOT NULL AND datetime(${table.createdAt}) IS NOT NULL`),
]);
...
export const tradeTeamMembers = sqliteTable("trade_team_members", {
  ...
  jobScope: text("job_scope").notNull().default("own"),
  canViewCustomers: integer("can_view_customers", { mode: "boolean" }).notNull().default(false),
  canManageCustomers: integer("can_manage_customers", { mode: "boolean" }).notNull().default(false),
  canViewQuotes: integer("can_view_quotes", { mode: "boolean" }).notNull().default(false),
  canManageQuotes: integer("can_manage_quotes", { mode: "boolean" }).notNull().default(false),
  canSendQuotes: integer("can_send_quotes", { mode: "boolean" }).notNull().default(false),
  ...
  canRescheduleJobs: integer("can_reschedule_jobs", { mode: "boolean" }).notNull().default(false),
...
export const tradeTeamMemberFiles = sqliteTable("trade_team_member_files", {
  ...
  nextCleanupAt: text("next_cleanup_at").notNull().default(""),
  lastCleanupError: text("last_cleanup_error").notNull().default(""),
...
export const tradeTeamMemberCredentials = sqliteTable("trade_team_member_credentials", {
  id: text("id").primaryKey(),
  ownerUid: text("owner_uid").notNull(),
  teamMemberId: text("team_member_id").notNull().references(() => tradeTeamMembers.id, { onDelete: "restrict" }),
  credentialType: text("credential_type").notNull(),
  rentalGate: text("rental_gate").notNull().default(""),
  name: text("name").notNull(),
  credentialNumber: text("credential_number").notNull().default(""),
  ...
  index("trade_team_member_credentials_member_idx").on(table.ownerUid, table.teamMemberId, table.status, table.expiresAt),
  uniqueIndex("trade_team_member_credentials_file_idx").on(table.fileId).where(sql`${table.fileId} <> ''`),
  check("trade_team_member_credentials_type_check", sql`${table.credentialType} IN ('licence', 'registration', 'training', 'accreditation', 'insurance', 'other')`),
  check("trade_team_member_credentials_rental_gate_check", sql`${table.rentalGate} IN ('', 'licensed_electrician', 'licensed_gasfitter', 'licensed_plumber', 'registered_plumber', 'refrigerant_handler', 'suitably_qualified_smoke_alarm_worker', 'sres_installer_accreditation', 'sres_designer_accreditation')`),
  check("trade_team_member_credentials_jurisdiction_check", sql`${table.jurisdiction} IN ('', 'ACT', 'NSW', 'NT', 'QLD', 'SA', 'TAS', 'VIC', 'WA', 'NATIONAL')`),
  check("trade_team_member_credentials_sres_national_check", sql`${table.rentalGate} NOT IN ('sres_installer_accreditation', 'sres_designer_accreditation') OR (${table.credentialType} = 'accreditation' AND ${table.jurisdiction} = 'NATIONAL')`),
  check("trade_team_member_credentials_specialist_role_type_check", sql`(${table.rentalGate} <> 'licensed_plumber' OR ${table.credentialType} = 'licence') AND (${table.rentalGate} <> 'registered_plumber' OR ${table.credentialType} = 'registration') AND (${table.rentalGate} <> 'refrigerant_handler' OR ${table.credentialType} = 'licence')`),
...
export const tradeTeamMemberEvents = sqliteTable("trade_team_member_events", {
  ...
  index("trade_team_member_events_member_time_idx").on(table.ownerUid, table.teamMemberId, table.createdAt),
  index("trade_team_member_events_entity_time_idx").on(table.ownerUid, table.entityType, table.entityId, table.createdAt),
  check("trade_team_member_events_entity_check", sql`${table.entityType} IN ('member', 'file', 'credential')`),
  check("trade_team_member_events_metadata_check", sql`json_valid(${table.metadata})`),
  check("trade_team_member_events_time_check", sql`datetime(${table.createdAt}) IS NOT NULL`),
]);
...
export const publicTradeLeadContactReleases = sqliteTable("public_trade_lead_contact_releases", {
  ...
  customerUnitNumber: text("customer_unit_number").notNull().default(""),
  customerStreetAddress: text("customer_street_address").notNull().default(""),
  customerSuburb: text("customer_suburb").notNull().default(""),
  customerAddressState: text("customer_address_state").notNull().default(""),
  postcode: text("postcode").notNull(),
...
export const tradeCrmJobMediaCleanup = sqliteTable("trade_crm_job_media_cleanup", {
  ...
  index("trade_crm_job_media_cleanup_due_idx").on(table.status, table.nextAttemptAt, table.createdAt, table.objectKey),
  check("trade_crm_job_media_cleanup_object_key_check", sql`length(${table.objectKey}) BETWEEN 1 AND 512 AND ${table.objectKey} NOT GLOB '*[^A-Za-z0-9._/-]*'`),
  check("trade_crm_job_media_cleanup_status_check", sql`${table.status} IN ('staged', 'claimed', 'retry')`),
  check("trade_crm_job_media_cleanup_attempts_check", sql`${table.attempts} >= 0`),
  check("trade_crm_job_media_cleanup_time_check", sql`datetime(${table.nextAttemptAt}) IS NOT NULL AND datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeCrmJobMediaEvents = sqliteTable("trade_crm_job_media_events", {
  id: text("id").primaryKey(),
  firebaseUid: text("firebase_uid").notNull(),
  workOrderId: text("work_order_id").notNull(),
  jobMediaId: text("job_media_id").notNull(),
  actorUid: text("actor_uid").notNull(),
  actorMemberId: text("actor_member_id").notNull(),
  eventType: text("event_type").notNull(),
  createdAt: text("created_at").notNull(),
}, (table) => [
  index("trade_crm_job_media_events_job_idx").on(table.firebaseUid, table.workOrderId, table.createdAt, table.id),
  check("trade_crm_job_media_events_type_check", sql`${table.eventType} = 'viewed'`),
  check("trade_crm_job_media_events_time_check", sql`datetime(${table.createdAt}) IS NOT NULL`),
]);
...
export const tradeCrmJobDetails = sqliteTable("trade_crm_job_details", {
  ...
  acceptedDisclosureSnapshot: text("accepted_disclosure_snapshot").notNull().default("{}"),
  acceptedDisclosureSha256: text("accepted_disclosure_sha256").notNull().default(""),
  acceptedDisclosureAt: text("accepted_disclosure_at").notNull().default(""),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  ...
  check("trade_crm_job_details_accepted_disclosure_snapshot_check", sql`json_valid(${table.acceptedDisclosureSnapshot}) AND json_type(${table.acceptedDisclosureSnapshot}) = 'object' AND length(${table.acceptedDisclosureSnapshot}) <= 65536`),
  check("trade_crm_job_details_accepted_disclosure_sha256_check", sql`${table.acceptedDisclosureSha256} = '' OR (length(${table.acceptedDisclosureSha256}) = 64 AND ${table.acceptedDisclosureSha256} = lower(${table.acceptedDisclosureSha256}) AND ${table.acceptedDisclosureSha256} NOT GLOB '*[^0-9a-f]*')`),
  check("trade_crm_job_details_accepted_disclosure_at_check", sql`${table.acceptedDisclosureAt} = '' OR datetime(${table.acceptedDisclosureAt}) IS NOT NULL`),
]);
...
export const tradeCrmQuoteVersions = sqliteTable("trade_crm_quote_versions", {
  ...
  issuedPdfObjectKey: text("issued_pdf_object_key").notNull().default(""),
  issuedPdfSha256: text("issued_pdf_sha256").notNull().default(""),
  issuedPdfSizeBytes: integer("issued_pdf_size_bytes").notNull().default(0),
...
export const tradeCrmQuoteDeliveries = sqliteTable("trade_crm_quote_deliveries", {
  ...
  attachmentSha256: text("attachment_sha256").notNull().default(""),
  emailRendererRevision: integer("email_renderer_revision").notNull().default(1),
...
export const tradeCrmJobMedia = sqliteTable("trade_crm_job_media", {
  ...
  evidenceEnvelope: text("evidence_envelope").notNull().default("{}"),
  originalSha256: text("original_sha256").notNull().default(""),
  acceptedLeadSourcePhotoId: text("accepted_lead_source_photo_id").notNull().default(""),
  acceptedLeadSourceOpportunityId: text("accepted_lead_source_opportunity_id").notNull().default(""),
  acceptedLeadSourcePreparationId: text("accepted_lead_source_preparation_id").notNull().default(""),
  acceptedLeadSourceReleaseId: text("accepted_lead_source_release_id").notNull().default(""),
  acceptedLeadPromptId: text("accepted_lead_prompt_id").notNull().default(""),
  acceptedLeadServiceCategories: text("accepted_lead_service_categories").notNull().default("[]"),
  acceptedDisclosureSha256: text("accepted_disclosure_sha256").notNull().default(""),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  ...
  uniqueIndex("trade_crm_job_media_accepted_source_idx").on(table.firebaseUid, table.workOrderId, table.acceptedLeadSourcePhotoId)
    .where(sql`${table.source} = 'accepted_public_lead'`),
  index("trade_crm_job_media_accepted_job_idx").on(table.firebaseUid, table.workOrderId, table.source, table.createdAt, table.id),
  ...
  check("trade_crm_job_media_accepted_services_check", sql`json_valid(${table.acceptedLeadServiceCategories}) AND json_type(${table.acceptedLeadServiceCategories}) = 'array'`),
  check("trade_crm_job_media_accepted_fingerprint_check", sql`${table.acceptedDisclosureSha256} = '' OR (length(${table.acceptedDisclosureSha256}) = 64 AND ${table.acceptedDisclosureSha256} = lower(${table.acceptedDisclosureSha256}) AND ${table.acceptedDisclosureSha256} NOT GLOB '*[^0-9a-f]*')`),
]);
...
export const tradeRentalInspections = sqliteTable("trade_rental_inspections", {
  id: text("id").primaryKey(),
  workOrderId: text("work_order_id").notNull().references(() => tradeWorkOrders.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  serviceSiteId: text("service_site_id").notNull().default(""),
  ...
  templateKey: text("template_key").notNull(),
  templateVersion: integer("template_version").notNull(),
  rulesEffectiveFrom: text("rules_effective_from").notNull(),
  assessmentScope: text("assessment_scope").notNull().default("current_minimum_standards"),
  moduleSelectionSnapshot: text("module_selection_snapshot").notNull(),
  selectedModulesSnapshot: text("selected_modules_snapshot"),
  propertySnapshot: text("property_snapshot").notNull().default("{}"),
  assessorUid: text("assessor_uid").notNull().default(""),
  assessorMemberId: text("assessor_member_id").notNull().default(""),
  assessorSnapshot: text("assessor_snapshot").notNull().default("{}"),
  revision: integer("revision").notNull().default(1),
  creationRequestId: text("creation_request_id").notNull().default(""),
  issuedReportId: text("issued_report_id").notNull().default(""),
  submittedAt: text("submitted_at").notNull().default(""),
  issuedAt: text("issued_at").notNull().default(""),
  supersededAt: text("superseded_at").notNull().default(""),
  createdByUid: text("created_by_uid").notNull(),
  ...
  uniqueIndex("trade_rental_inspections_owner_work_idx").on(table.firebaseUid, table.workOrderId),
  uniqueIndex("trade_rental_inspections_owner_number_idx").on(table.firebaseUid, table.inspectionNumber),
  uniqueIndex("trade_rental_inspections_creation_request_idx").on(table.firebaseUid, table.creationRequestId).where(sql`${table.creationRequestId} <> ''`),
  index("trade_rental_inspections_owner_status_idx").on(table.firebaseUid, table.status, table.updatedAt),
  index("trade_rental_inspections_work_idx").on(table.workOrderId, table.updatedAt),
  ...
  check("trade_rental_inspections_scope_check", sql`${table.assessmentScope} IN ('current_minimum_standards', 'energy_readiness_2027')`),
  ...
  check("trade_rental_inspections_selected_modules_check", sql`${table.selectedModulesSnapshot} IS NULL OR (json_valid(${table.selectedModulesSnapshot}) AND json_type(${table.selectedModulesSnapshot}) = 'array' AND json_array_length(${table.selectedModulesSnapshot}) BETWEEN 1 AND 4 AND json_extract(${table.selectedModulesSnapshot}, '$[0]') IN ('minimum_standards', 'electrical_safety_check', 'gas_safety_check', 'smoke_alarm_check') AND coalesce(json_extract(${table.selectedModulesSnapshot}, '$[1]'), '') IN ('', 'minimum_standards', 'electrical_safety_check', 'gas_safety_check', 'smoke_alarm_check') AND coalesce(json_extract(${table.selectedModulesSnapshot}, '$[2]'), '') IN ('', 'minimum_standards', 'electrical_safety_check', 'gas_safety_check', 'smoke_alarm_check') AND coalesce(json_extract(${table.selectedModulesSnapshot}, '$[3]'), '') IN ('', 'minimum_standards', 'electrical_safety_check', 'gas_safety_check', 'smoke_alarm_check') AND (json_array_length(${table.selectedModulesSnapshot}) < 2 OR json_extract(${table.selectedModulesSnapshot}, '$[1]') <> json_extract(${table.selectedModulesSnapshot}, '$[0]')) AND (json_array_length(${table.selectedModulesSnapshot}) < 3 OR json_extract(${table.selectedModulesSnapshot}, '$[2]') NOT IN (json_extract(${table.selectedModulesSnapshot}, '$[0]'), json_extract(${table.selectedModulesSnapshot}, '$[1]'))) AND (json_array_length(${table.selectedModulesSnapshot}) < 4 OR json_extract(${table.selectedModulesSnapshot}, '$[3]') NOT IN (json_extract(${table.selectedModulesSnapshot}, '$[0]'), json_extract(${table.selectedModulesSnapshot}, '$[1]'), json_extract(${table.selectedModulesSnapshot}, '$[2]'))))`),
  ...
  check("trade_rental_inspections_time_check", sql`datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalInspectionModules = sqliteTable("trade_rental_inspection_modules", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  moduleKey: text("module_key").notNull(),
  required: integer("required", { mode: "boolean" }).notNull().default(false),
  selectedRequired: integer("selected_required", { mode: "boolean" }),
  ...
  templateSnapshot: text("template_snapshot").notNull(),
  answers: text("answers").notNull().default("{}"),
  credentialSnapshot: text("credential_snapshot").notNull().default("{}"),
  revision: integer("revision").notNull().default(1),
  completedByUid: text("completed_by_uid").notNull().default(""),
  completedAt: text("completed_at").notNull().default(""),
  createdAt: text("created_at").notNull(),
  ...
  uniqueIndex("trade_rental_modules_inspection_key_idx").on(table.inspectionId, table.moduleKey),
  ...
  check("trade_rental_modules_selected_required_check", sql`${table.selectedRequired} IS NULL OR ${table.selectedRequired} = 1`),
  ...
  check("trade_rental_modules_completion_check", sql`(${table.status} = 'complete' AND trim(${table.completedByUid}) <> '' AND datetime(${table.completedAt}) IS NOT NULL) OR (${table.status} <> 'complete' AND ${table.completedByUid} = '' AND ${table.completedAt} = '')`),
  check("trade_rental_modules_time_check", sql`datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalInspectionItems = sqliteTable("trade_rental_inspection_items", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  moduleId: text("module_id").notNull().references(() => tradeRentalInspectionModules.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  itemKey: text("item_key").notNull(),
  sectionKey: text("section_key").notNull(),
  checkKey: text("check_key").notNull(),
  instanceKey: text("instance_key").notNull().default("property"),
  locationLabel: text("location_label").notNull().default(""),
  outcome: text("outcome").notNull().default("not_assessed"),
  responseJson: text("response_json").notNull().default("{}"),
  publicNotes: text("public_notes").notNull().default(""),
  internalNotes: text("internal_notes").notNull().default(""),
  requiredEvidenceCount: integer("required_evidence_count").notNull().default(0),
  sortOrder: integer("sort_order").notNull().default(0),
  revision: integer("revision").notNull().default(1),
  completedByUid: text("completed_by_uid").notNull().default(""),
  completedAt: text("completed_at").notNull().default(""),
  ...
  uniqueIndex("trade_rental_items_instance_idx").on(table.inspectionId, table.itemKey),
  index("trade_rental_items_section_idx").on(table.inspectionId, table.sectionKey, table.sortOrder),
  index("trade_rental_items_module_idx").on(table.moduleId, table.outcome, table.sortOrder),
  check("trade_rental_items_key_check", sql`trim(${table.itemKey}) <> '' AND trim(${table.sectionKey}) <> '' AND trim(${table.checkKey}) <> '' AND trim(${table.instanceKey}) <> ''`),
  check("trade_rental_items_outcome_check", sql`${table.outcome} IN ('not_assessed', 'meets', 'does_not_meet', 'specialist_verification_required', 'not_accessible', 'not_applicable', 'exemption_evidence_pending')`),
  check("trade_rental_items_response_check", sql`json_valid(${table.responseJson}) AND json_type(${table.responseJson}) = 'object'`),
  check("trade_rental_items_bounds_check", sql`${table.requiredEvidenceCount} BETWEEN 0 AND 20 AND ${table.sortOrder} BETWEEN 0 AND 10000 AND ${table.revision} > 0 AND length(${table.publicNotes}) <= 4000 AND length(${table.internalNotes}) <= 4000`),
  check("trade_rental_items_time_check", sql`datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalFindings = sqliteTable("trade_rental_findings", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  moduleId: text("module_id").notNull().references(() => tradeRentalInspectionModules.id, { onDelete: "restrict" }),
  ...
  title: text("title").notNull(),
  description: text("description").notNull().default(""),
  standardReference: text("standard_reference").notNull().default(""),
  findingStatus: text("finding_status").notNull(),
  severity: text("severity").notNull().default("information"),
  tradeCategory: text("trade_category").notNull().default(""),
  locationLabel: text("location_label").notNull().default(""),
  ...
  uniqueIndex("trade_rental_findings_key_idx").on(table.inspectionId, table.findingKey),
  index("trade_rental_findings_status_idx").on(table.inspectionId, table.findingStatus, table.severity, table.sortOrder),
  index("trade_rental_findings_trade_idx").on(table.inspectionId, table.tradeCategory, table.sortOrder),
  index("trade_rental_findings_item_idx").on(table.itemId, table.updatedAt),
  check("trade_rental_findings_status_check", sql`${table.findingStatus} IN ('compliant', 'recommendation', 'faulty', 'non_compliant', 'safety_issue', 'disconnected', 'not_tested', 'not_applicable')`),
  check("trade_rental_findings_severity_check", sql`${table.severity} IN ('immediate_safety_risk', 'urgent', 'required', 'recommended', 'information')`),
  check("trade_rental_findings_details_check", sql`json_valid(${table.details}) AND json_type(${table.details}) = 'object'`),
  check("trade_rental_findings_bounds_check", sql`trim(${table.findingKey}) <> '' AND trim(${table.category}) <> '' AND trim(${table.title}) <> '' AND ${table.quantityMilli} BETWEEN 0 AND 1000000000 AND length(${table.description}) <= 8000 AND length(${table.scopeSummary}) <= 8000 AND length(${table.internalNotes}) <= 4000 AND ${table.revision} > 0`),
  check("trade_rental_findings_time_check", sql`datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalEvidenceLinks = sqliteTable("trade_rental_evidence_links", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  moduleId: text("module_id").notNull().references(() => tradeRentalInspectionModules.id, { onDelete: "restrict" }),
  itemId: text("item_id").notNull().default(""),
  findingId: text("finding_id").notNull().default(""),
  jobMediaId: text("job_media_id").notNull().references(() => tradeCrmJobMedia.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  requirementKey: text("requirement_key").notNull(),
  evidenceType: text("evidence_type").notNull().default("photo"),
  purpose: text("purpose").notNull().default(""),
  captionSnapshot: text("caption_snapshot").notNull().default(""),
  sortOrder: integer("sort_order").notNull().default(0),
  status: text("status").notNull().default("active"),
  createdByUid: text("created_by_uid").notNull(),
  createdAt: text("created_at").notNull(),
  ...
  uniqueIndex("trade_rental_evidence_requirement_media_idx").on(table.inspectionId, table.requirementKey, table.jobMediaId),
  index("trade_rental_evidence_module_idx").on(table.inspectionId, table.moduleId, table.requirementKey, table.status),
  index("trade_rental_evidence_finding_idx").on(table.findingId, table.status, table.sortOrder),
  index("trade_rental_evidence_media_idx").on(table.jobMediaId),
  check("trade_rental_evidence_type_check", sql`${table.evidenceType} IN ('photo', 'document', 'measurement', 'signature') AND ${table.status} IN ('active', 'superseded')`),
  check("trade_rental_evidence_bounds_check", sql`trim(${table.requirementKey}) <> '' AND length(${table.purpose}) <= 1000 AND length(${table.captionSnapshot}) <= 1000 AND ${table.sortOrder} BETWEEN 0 AND 10000`),
  check("trade_rental_evidence_time_check", sql`datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalReports = sqliteTable("trade_rental_reports", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  ...
  revision: integer("revision").notNull(),
  status: text("status").notNull().default("staged"),
  reportSchemaVersion: text("report_schema_version").notNull().default("tlink-rental-report-v1"),
  ...
  issuedByUid: text("issued_by_uid").notNull().default(""),
  issuedByMemberId: text("issued_by_member_id").notNull().default(""),
  issuerSnapshot: text("issuer_snapshot").notNull().default("{}"),
  ...
  uniqueIndex("trade_rental_reports_inspection_revision_idx").on(table.inspectionId, table.revision),
  uniqueIndex("trade_rental_reports_owner_number_idx").on(table.firebaseUid, table.reportNumber),
  index("trade_rental_reports_owner_status_idx").on(table.firebaseUid, table.status, table.updatedAt),
  index("trade_rental_reports_inspection_idx").on(table.inspectionId, table.status, table.revision),
  check("trade_rental_reports_status_check", sql`${table.status} IN ('staged', 'issued', 'failed', 'superseded', 'withdrawn')`),
  check("trade_rental_reports_snapshot_check", sql`json_valid(${table.reportSnapshot}) AND json_type(${table.reportSnapshot}) = 'object' AND json_extract(${table.reportSnapshot}, '$.schemaVersion') = ${table.reportSchemaVersion} AND json_valid(${table.issuerSnapshot}) AND json_type(${table.issuerSnapshot}) = 'object'`),
  check("trade_rental_reports_hash_check", sql`length(${table.sourceSnapshotSha256}) = 64 AND ${table.sourceSnapshotSha256} = lower(${table.sourceSnapshotSha256}) AND ${table.sourceSnapshotSha256} NOT GLOB '*[^0-9a-f]*' AND (${table.pdfSha256} = '' OR (length(${table.pdfSha256}) = 64 AND ${table.pdfSha256} = lower(${table.pdfSha256}) AND ${table.pdfSha256} NOT GLOB '*[^0-9a-f]*'))`),
  check("trade_rental_reports_pdf_check", sql`(${table.status} = 'issued' AND trim(${table.pdfObjectKey}) <> '' AND length(${table.pdfSha256}) = 64 AND ${table.pdfSizeBytes} BETWEEN 5 AND 52428800 AND trim(${table.issuedByUid}) <> '' AND datetime(${table.issuedAt}) IS NOT NULL) OR (${table.status} <> 'issued')`),
  check("trade_rental_reports_revision_check", sql`${table.revision} > 0`),
  check("trade_rental_reports_time_check", sql`datetime(${table.stagedAt}) IS NOT NULL AND datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.updatedAt}) IS NOT NULL`),
]);

export const tradeRentalReportLinks = sqliteTable("trade_rental_report_links", {
  id: text("id").primaryKey(),
  reportId: text("report_id").notNull().references(() => tradeRentalReports.id, { onDelete: "restrict" }),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  firebaseUid: text("firebase_uid").notNull(),
  tokenHash: text("token_hash").notNull(),
  encryptedToken: text("encrypted_token").notNull().default(""),
  tokenIssue: integer("token_issue").notNull().default(1),
  status: text("status").notNull().default("active"),
  expiresAt: text("expires_at").notNull(),
  revokedAt: text("revoked_at").notNull().default(""),
  createdByUid: text("created_by_uid").notNull(),
  lastViewedAt: text("last_viewed_at").notNull().default(""),
  lastDownloadedAt: text("last_downloaded_at").notNull().default(""),
  viewCount: integer("view_count").notNull().default(0),
  downloadCount: integer("download_count").notNull().default(0),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  uniqueIndex("trade_rental_report_links_token_idx").on(table.tokenHash),
  uniqueIndex("trade_rental_report_links_active_report_idx").on(table.reportId).where(sql`${table.status} = 'active'`),
  index("trade_rental_report_links_owner_idx").on(table.firebaseUid, table.inspectionId, table.updatedAt),
  index("trade_rental_report_links_expiry_idx").on(table.status, table.expiresAt),
  check("trade_rental_report_links_hash_check", sql`length(${table.tokenHash}) = 64 AND ${table.tokenHash} = lower(${table.tokenHash}) AND ${table.tokenHash} NOT GLOB '*[^0-9a-f]*'`),
  check("trade_rental_report_links_status_check", sql`${table.status} IN ('active', 'revoked', 'expired', 'superseded') AND ${table.tokenIssue} > 0`),
  check("trade_rental_report_links_expiry_check", sql`datetime(${table.expiresAt}) IS NOT NULL AND datetime(${table.createdAt}) IS NOT NULL AND datetime(${table.expiresAt}) > datetime(${table.createdAt})`),
  check("trade_rental_report_links_counts_check", sql`${table.viewCount} >= 0 AND ${table.downloadCount} >= 0`),
]);

export const tradeRentalInspectionEvents = sqliteTable("trade_rental_inspection_events", {
  id: text("id").primaryKey(),
  inspectionId: text("inspection_id").notNull().references(() => tradeRentalInspections.id, { onDelete: "restrict" }),
  reportId: text("report_id").notNull().default(""),
  reportLinkId: text("report_link_id").notNull().default(""),
  firebaseUid: text("firebase_uid").notNull(),
  actorType: text("actor_type").notNull().default("system"),
  actorUid: text("actor_uid").notNull().default(""),
  eventType: text("event_type").notNull(),
  ...
  summary: text("summary").notNull().default(""),
  metadata: text("metadata").notNull().default("{}"),
  sourceIpSha256: text("source_ip_sha256").notNull().default(""),
  userAgentSha256: text("user_agent_sha256").notNull().default(""),
  createdAt: text("created_at").notNull(),
}, (table) => [
  uniqueIndex("trade_rental_events_request_idx").on(table.inspectionId, table.requestId).where(sql`${table.requestId} <> ''`),
  index("trade_rental_events_inspection_idx").on(table.inspectionId, table.createdAt),
  index("trade_rental_events_report_idx").on(table.reportId, table.createdAt),
  index("trade_rental_events_link_idx").on(table.reportLinkId, table.createdAt),
  index("trade_rental_events_owner_type_idx").on(table.firebaseUid, table.eventType, table.createdAt),
  check("trade_rental_events_actor_check", sql`${table.actorType} IN ('assessor', 'owner', 'viewer', 'system')`),
  check("trade_rental_events_metadata_check", sql`json_valid(${table.metadata}) AND json_type(${table.metadata}) = 'object' AND length(${table.summary}) <= 2000`),
