...
As of 19 July 2026, the AEA Resend account is active under the business Google identity. A sending-only production key and webhook signing secret are protected in Sites. The dedicated `reminders.ausenergyassessments.com` DKIM, SPF and return-path records are present in Squarespace DNS and the domain is verified in Resend. The production email channel was deliberately enabled after administrator health checks confirmed its credentials and callbacks were ready. One photo-request email was accepted by the provider. This is not mailbox-delivery proof until an authenticated callback records that state. Sites environment revision 14 was retained for that release evidence.
