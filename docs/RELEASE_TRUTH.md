...
| Creditex compliance operations foundation application source | `2ef8ce19fd5423fd95652a7bc88265e80d7b827f` | Validated, pushed to GitHub and Sites managed `main`; saved and deployed as Sites version 246 |
| Current saved-version identity | `appgprj_6a550c378000819185caf094173422bb~appgver_3cef6ddd92e88191a54d034d3a6e72e3` | Exact saved version 246 built from `2ef8ce19fd5423fd95652a7bc88265e80d7b827f`; package content hash `sha256:9d6ac6f6e5a3036ba8fedf14c94b0fdc61e608b32b203346fc327a8119f625ea` |
| Current executable application identity | Sites version 246 from `2ef8ce19fd5423fd95652a7bc88265e80d7b827f` at `https://compare.ausenergyassessments.com` | Deployment `appgdep_6a6d5c42819081919d81dcd9451338bd`; saved-version provenance, custom-domain production deployment and environment revision 19 verified on 1 August 2026 |
| Current source checkpoint | Documentation-only child of `2ef8ce19fd5423fd95652a7bc88265e80d7b827f` | This release record does not change the exact executable source for public Sites version 246 |
...
The additive schema expansion, reviewed-ABN application, authorised contract cleanup, customer home advisor, advisor-context release, independent customer-plan release, customer plan evidence-and-history release, professional-review, browser-native PDF, premium customer-report, technical presentation, spacing, customer-plan trust, customer-project cleanup, customer-roadmap-context, customer-installer-request, customer-plan-durability, installer-request-and-multi-photo, authoritative-installer-submit, installer-enquiry-pack, complete customer-installer-handoff, customer quote-communications, customer-to-trade contact, customer-plan trade-enquiry, customer-account-trust, protected-trade-locality and empty Creditex-foundation releases are deployed to production. Sites version 246 from `2ef8ce19fd5423fd95652a7bc88265e80d7b827f` is the exact current executable application source. Failed versions 243, 244 and 245 never activated and did not supersede version 242. The current release publishes no real activity or evidence policy and keeps every unverified calculator and external execution path disabled.

## Released Creditex compliance operations foundation

`CREDITEX-COMPLIANCE-OPERATIONS-25` is released only as an empty, invitation-gated foundation and preview. Exact application commit `2ef8ce19fd5423fd95652a7bc88265e80d7b827f` is deployed as Sites version 246 through `appgdep_6a6d5c42819081919d81dcd9451338bd` with environment revision 19.
...
The release passes 81 of 81 focused Creditex, governance, access, viewer, field-evidence, guided-job, photo and database-console tests. `npm.cmd run validate` passes type checking, warning-free lint, 31 of 31 integration tests, 1,077 main tests with 1,075 passed, 2 intentionally skipped and 0 failed, all 98 migrations through `0097_creditex_operations_lifecycle.sql`, the customer-plan PDF audit, production build and Sites server-bundle audit. Failed Sites versions 243, 244 and 245 did not activate; each stopped at the D1 migration gate with `SQLITE_ERROR: incomplete input`. Migrations `0093` to `0097` now carry only complete line-bounded schema and seed statements; the original `0096` and `0097` history slots are retained to avoid a filename collision with any rejected saved version. Because the Sites migration parser cannot accept trigger bodies, `src/lib/creditex-schema-guards.ts` preserves the same 132 definitions with only `IF NOT EXISTS` added, installs them in at most 40-statement prepared D1 batches with an automatic portal retry boundary, verifies every installed definition against the exact expected `sqlite_schema` SQL, and blocks Creditex access, governed case creation, verified trade access, team access and governed customer-media deletion on any missing, stale or failed installation. AEA Field separately passes 6 of 6 tests, lint, type checking and Android and iOS export. Independent final review approved release only as an empty invitation-gated baseline.

The 7,265,958-byte local archive has SHA-256 `D8EAACACACE4955C472851807344F2D824C6F8460614A4547E0371292EDD3801`, contains all 98 migrations and the Vinext server entrypoint, and produced a 331-file Sites package with content hash `sha256:9d6ac6f6e5a3036ba8fedf14c94b0fdc61e608b32b203346fc327a8119f625ea`. The active custom domain and SSL are healthy. Live `/api/health` returned 200; unauthenticated `/api/creditex/session` and `/api/creditex/access` returned 401; and `/creditex/compliance` displayed the controlled sign-in boundary at desktop and 390-pixel widths without document overflow and with its full no-index directive. The post-release Sites Worker error-only query reported those two expected 401 boundary invocations at information level with Worker outcome `ok` and no exception or error-level message.

The Firebase account for `info@ausenergyassessments.com` already existed, so no duplicate was created. A fresh password-activation email was sent. The account's current verified-email state, invitation claim, first authenticated installation and exact-SQL verification of all 132 runtime guards, signed-in administration, non-admin least privilege, physical-device capture, hosted original-evidence viewing, rule-pack accuracy, approved calculator provenance, retention, legal hold, backup and restore and all external provider behavior remain unverified or blocked.
...
The current verified deployed topology for Sites version 246 is:
