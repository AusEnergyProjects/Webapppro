...
- P6-2E continues from the exact version 128 source and adds team scheduling, capacity and server-side conflict checks. It also makes the admin account's saved verification status authoritative in both admin and signed-in trade entitlement responses. Publication still requires the exact-commit validation, GitHub push, Sites version creation and live provenance verification.
