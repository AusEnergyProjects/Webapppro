...
- P6-2B continues from that exact version 123 source and adds migration `0048_unified_enquiry_inbox.sql`, the owner-scoped enquiry inbox, explicit duplicate conversion and generic CSV or Excel imports. Publication still requires `npm run validate`, GitHub push, Sites version creation and live provenance verification.
