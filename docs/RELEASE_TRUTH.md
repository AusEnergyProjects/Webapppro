...
Local verification passed 170 focused tests, 37 integration tests, typecheck, lint, all 189 migrations and the production build. The eight remaining full-suite assertion failures and expired Surge source-review audit independently reproduce on the original source; one crashed test worker passed all 16 tests on isolated rerun. This feature is not published: the installed Sites packaging helpers disappeared during execution and no replacement was found. Source checkpoint `87e0f94883052f67332f55e1cf7478d8a0d92b82` and detailed recovery instructions are retained in the current handover. The last observed live version remains Sites 613.
