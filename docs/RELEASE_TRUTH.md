...
The registry release passed 170 focused tests, 37 integration tests, typecheck, lint, all 189 migrations and the production build. The eight remaining full-suite assertion failures and expired Surge source-review audit independently reproduce on the original source; one crashed test worker passed all 16 tests on isolated rerun. Sites 614 was published successfully from `9b76d686fcbe7f49c2bae429b70c3e62910dd21e` through the native remote-build fallback while local packaging helpers were unavailable. Local packaging helpers remain intermittently unavailable; the native source-build fallback remains supported.

The follow-up adds an interactive iPhone/Samsung form preview beside the administrator and Creditex field master editor, using the same worker-form projection, conditional pages and repeating groups as the field app. Next and Back work without answers or signatures; optional answer checks show validation hints. Unsaved edits are previewed locally; camera, GPS, product choices and signatures are test simulations. Active Creditex members can browse and test forms, while saving retains the existing named-editor role gate. Team access is now a direct administrator navigation item using the existing invitation and membership system; invitations remain exact-email-bound and no invitation email is automatically sent. Claim shortcuts retain the selected account and packet, open existing retained files directly, and bound bulk selection to the official file limit. Final publication of this follow-up is recorded by the coordinating task.

Follow-up validation: 142 focused phone/projection/master/security/flow checks, 123 editor/access checks, 50 team-access checks, 111 registry checks and 34 publication/registry/release reruns passed (these groups overlap). Typecheck, full ESLint and the production build passed, including Sites bundle and public-performance audits. Eight integrated editor browser combinations passed across desktop/mobile, day/night and both phone frames. The full suite reported 5,323 passed and 11 skipped, with the same eight baseline assertions, two stale assertions subsequently corrected and passed, and one worker crash whose isolated rerun passed. The full validation still stops at the existing expired Surge source approvals. Native typecheck was unavailable because mobile dependencies are not installed; shared native-flow tests passed. No native application build or real regulatory transaction was performed.
...
The accounting job query now retains distinct accepted-invoice and quick-invoice references, preventing a joined quick-invoice column from replacing the immutable accepted invoice number. All three providers receive issued invoices; Xero uses AUTHORISED with an explicitly chosen active sales account. The interface selects the connected provider, loads choices automatically and offers one Sync action. Migration 0181 remembers a successfully used account/item per connection, preserves it on same-business reconsent and clears it when the external business changes. Xero adds accounting.settings.read to load accounts and requires reconsent for older grants. TLink does not request invoice emails; users are directed to check their accounting system's automatic sending rules. Existing owner, privacy, amount and duplicate-export checks remain intact.

The existing Intuit account and TLink app were recovered, their production credentials stored as Sites secrets and the authorised apex callback saved. Xero platform credentials are also configured in Sites environment 97. Actual ledger OAuth/export acceptance remains to be completed; MYOB developer application ticket 52438 awaits provider approval. Do not describe any provider as connected based on this source change. The first reference/status correction passed focused checks (41), integration checks (37), typecheck, lint, 180 migration applications, PDF audit and build. The issued-invoice follow-up adds migration 0181 and has its own focused, migration and release validation recorded by the coordinating task. Seven unrelated full-suite failures reproduce on the clean baseline; the changed milestone assertions are corrected. The coordinating release response records the exact subsequent commit, Sites version and live checks.
...
The follow-up on this branch makes the rental default a full current-minimum-standards assessment plus 2027 readiness, with practical measurements, mandatory quotation details/photos before finalisation, a dark TLink PDF organised by work category and regime limitations. Named-trade assignment and the further-information/site-visit option are removed. It adds canonical electrical/plumbing capabilities with migration 0174 and simplifies the mobile job screen while fixing a dispatched-action acknowledgement race. The audit's follow-up section records scope and validation. The previous public baseline was Sites 556 at `dece3ffcb14eab422c6df0637df4ade6361d036d`; the release task records publication of this subsequent source.
