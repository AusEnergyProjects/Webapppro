...
| Current Creditex government-activity workflow application source | `a33b7053301a64bea4bbcbe76713067a2c1782dd` | Exact validated source for `CREDITEX-GOVERNMENT-ACTIVITY-WORKFLOW-27`; pushed to GitHub and Sites managed `main` |
| Current saved-version identity | `appgprj_6a550c378000819185caf094173422bb~appgver_a8b4368a16a88191be90ea1a3ce33481` | Exact saved Sites version 251 built from `a33b7053301a64bea4bbcbe76713067a2c1782dd`; package content hash `sha256:917cf16e38b0a69e2081992a8f2944699bf9492b78f40c8ce4745b55612bf285` |
| Current executable application identity | Sites version 251 from `a33b7053301a64bea4bbcbe76713067a2c1782dd` at `https://compare.ausenergyassessments.com/creditex/compliance` | Deployment `appgdep_6a6dbc598f0c81918d1e6656addd0463` succeeded; environment revision 19; provider URL `https://aea-energy-comparison.info294029.chatgpt.site` |
| Current source checkpoint | Documentation-only child of `a33b7053301a64bea4bbcbe76713067a2c1782dd` | This release record does not change the exact executable source for public Sites version 251 |
...
The additive schema expansion, reviewed-ABN application, authorised contract cleanup, customer, installer and trade releases, protected-trade locality, Creditex foundation, operations portal, evidence-policy governance and national government-activity workflow are deployed to production. Sites version 251 from `a33b7053301a64bea4bbcbe76713067a2c1782dd` is the exact current executable application source. Failed versions 243, 244 and 245 never activated. Intermediate version 247 activated long enough for signed-in QA, exposed a production-D1 incompatibility in one aggregate query and was replaced by version 248, which is now historical. Release QA for version 251 performed no production mutation. The release exposes 32 discovery-only program pathways and 207 controlled activity templates; the governed database remains at 0 published programs, 0 activity versions, 0 policies and 0 cases, and keeps every unverified calculator and external execution path disabled.

## Current Creditex government-activity workflow

`CREDITEX-GOVERNMENT-ACTIVITY-WORKFLOW-27` is deployed from exact application commit `a33b7053301a64bea4bbcbe76713067a2c1782dd` as Sites version 251. Saved version `appgprj_6a550c378000819185caf094173422bb~appgver_a8b4368a16a88191be90ea1a3ce33481` reports the exact application source, and deployment `appgdep_6a6dbc598f0c81918d1e6656addd0463` succeeded with environment revision 19.

The signed-in portal now exposes a discovery-only national reference catalogue of 32 Australian government program pathways and 207 controlled activity templates. The installer handoff starts after the separate customer quote and acceptance workflow and uses chained program, activity, product-category, scenario and exact effective-version dropdowns. The Creditex operations workspace uses separate persistent program and activity tab rows, retains the Dataforce-equivalent filter families, and gives administrators deliberate audited access to complete private case detail.

Government departments, regulators and scheme administrators remain the sole rule authors. Creditex verifies exact operational transcriptions, audits evidence, manages corrections and performs authorised program actions within its accreditation and contractual connector scope. The reference catalogue creates no production rule, eligibility decision, quantity, rebate value, certificate or external submission. It separately types tradable certificates, project credits, retailer obligations, rebates, grants, loans, tariffs and procurement outcomes.

The shared `info@ausenergyassessments.com` bootstrap account can maintain drafts and invite named users but cannot request or approve publication. Publication requires two different independently verified named administrators. The real governed production inventory remains 0 programs, 0 activity versions, 0 evidence policies and 0 cases until exact official instruments are independently verified and imported.

The exact application source passes the complete `npm.cmd run validate` gate: type checking, warning-free lint, 31 of 31 integration tests, the complete main test suite, all 99 migrations through `0098_creditex_rule_governance.sql`, the customer-plan PDF audit, production build and Sites server-bundle audit. The catalogue-focused set passed 30 of 30 tests, the final portal regression set passed 21 of 21 and `git diff --check` passed.

The local release package is 6,790,614 bytes with SHA-256 `B14686D098A1FF76D8DBF1F2CA26DE2AABB6D600D991289891A9CF31C6E50FFB`. Sites stored 178 files and 18,227,200 bytes with content hash `sha256:917cf16e38b0a69e2081992a8f2944699bf9492b78f40c8ce4745b55612bf285`.

Signed-in Chrome QA confirmed successful reload into Operations, the 32-program and 207-activity government-source workspace, controlled template and outcome selectors, separate program and activity tabs, audited-private-case wording and the shared-account publication boundary. No production mutation was performed. The post-release Sites Worker error-only query returned zero events.
...
The current verified deployed topology for Sites version 251 is:
