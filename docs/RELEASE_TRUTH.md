...
| Authentication-corrected Creditex VEU pilot application source | `ebae330dab6c42881c14bc57548095b111d9c850` | Historical exact source retaining the pilot and correcting authentication recovery; deployed as Sites version 253 |
| Creditex VEU dense-register application source | `e8d12a4b562de3f9ac5b6821c4e1b062547722e0` | Current exact validated application source; pushed to GitHub and Sites managed `main` |
| Current saved-version identity | `appgprj_6a550c378000819185caf094173422bb~appgver_d9dff81ff0c4819185def4ad16b9a889` | Exact source-only Sites version 254 built from `e8d12a4b562de3f9ac5b6821c4e1b062547722e0`; no local archive was supplied |
| Current executable application identity | Sites version 254 from `e8d12a4b562de3f9ac5b6821c4e1b062547722e0` at `https://compare.ausenergyassessments.com/creditex/compliance` | Deployment `appgdep_6a6dec1ba16c8191b572bf49dc958aa7` succeeded; environment revision 19; provider URL `https://aea-energy-comparison.info294029.chatgpt.site` |
| Current source checkpoint | Documentation-only child of `e8d12a4b562de3f9ac5b6821c4e1b062547722e0` | This release record does not change the exact executable source for public Sites version 254 |
...
The additive schema expansion, reviewed-ABN application, authorised contract cleanup, customer, installer and trade releases, protected-trade locality, Creditex foundation, operations portal, evidence-policy governance, national government-activity workflow and isolated VEU synthetic pilot are deployed to production. Sites version 254 from `e8d12a4b562de3f9ac5b6821c4e1b062547722e0` is the exact current executable application source. Failed versions 243, 244 and 245 never activated. Intermediate version 247 activated long enough for signed-in QA, exposed a production-D1 incompatibility in one aggregate query and was replaced by version 248, which is now historical. The release retains 32 Australian government program pathways and 207 controlled activity templates and exposes the controlled 10-installer, 30-technician, 300-job VEU pilot as a dense Dataforce-style compliance register. The real governed inventory remains 0 published programs, 0 activity versions, 0 policies and 0 regulated cases, and every unverified calculator and external execution path remains disabled.

## Current Creditex VEU dense register

`CREDITEX-VEU-SYNTHETIC-PILOT-28` was first deployed from application commit `3ac6c72057a8afea61e85817ba566ec543079886` as Sites version 252, and authentication-corrected commit `ebae330dab6c42881c14bc57548095b111d9c850` became version 253. `CREDITEX-VEU-DENSE-REGISTER-29` is the current release from exact application commit `e8d12a4b562de3f9ac5b6821c4e1b062547722e0`. Saved version `appgprj_6a550c378000819185caf094173422bb~appgver_d9dff81ff0c4819185def4ad16b9a889` reports that exact source, and deployment `appgdep_6a6dec1ba16c8191b572bf49dc958aa7` succeeded with environment revision 19.

The signed-in portal retains the discovery-only national reference catalogue, chained program and activity selectors and audited private case workspace. Its VEU Jobs workspace now defaults to the complete 300-job queue and renders one semantic table row per job. It exposes 49 data columns plus one row-action column: every heading visible in the supplied Dataforce screenshot, explicit TLink installer and field-technician identity, government activity dimensions, all five fail-closed compliance states, trade workflow states and audit dates. Every column header has a dropdown; 41 verified fields support stable server-side ascending and descending order, while unsupported legacy semantics open an exact mapping explanation. Every returned row has its own record-action dropdown.

The right-side advanced filter rail remains available beside the desktop register and stacks below it on mobile. It contains 12 filter groups, 27 pre-populated selectors, two bounded date inputs and Search and Clear actions. The fixed bottom Dashboard plus one tab for each of the 34 represented VEU activity families remains unchanged. Part `6` is the official family identifier. Categories and scenarios remain separate governed dimensions; `6(23)` was only an informal example and is not a special implementation path.
...
The exact current application source passes the complete `npm.cmd run validate` gate: type checking, warning-free lint, 31 of 31 integration tests, the complete main test suite, all 100 migrations through `0099_creditex_synthetic_pilot.sql`, the customer-plan PDF audit, Vinext production build and Sites server-bundle audit. The final focused pilot suite passed 13 of 13 tests, the earlier integrated Creditex, catalogue and API focused set passed 56 of 56, and `git diff HEAD^ --check` passed.

Version 254 was saved from the exact managed source commit without a local archive. Package size, package hash, stored-file count and package content hash are therefore not claimed for this release. The prior version-253 source and package metrics remain historical evidence.
...
Signed-in Chrome QA confirmed 10 of 10 installers, 30 of 30 technicians, 300 of 300 jobs and 34 of 34 activity families. The Jobs workspace reported `300 shown of 300 matching jobs`; the table contained 300 body rows, 50 visible columns, 50 header dropdowns and 300 row action dropdowns. Selecting installer `I01` returned exactly 30 rows belonging only to `[TEST] VEU Installer Company 01`, and Clear restored all 300. Selecting Job ID descending changed the first job from `TEST-VEU-B5BA21F9-I01-T01-J01` to `TEST-VEU-B5BA21F9-I10-T03-J10` with `aria-sort="descending"`.

Desktop and 390-pixel Chrome inspection passed. At 390 pixels, the root and body remained 375 pixels wide, the 8,413-pixel register stayed inside its 334-pixel local scroll viewport and the 349.6-pixel advanced filter rail stacked below it, so there was no page-level horizontal overflow. Production Worker evidence contained successful `200` reads for the VEU pilot and every Creditex bootstrap endpoint and no `5xx`. Two cutover-time asset requests from Cloudflare browser rendering returned `404`; the signed-in portal and pilot API remained operational. Version 254 retains the bounded authentication recovery introduced in version 253.
...
The current verified deployed topology for Sites version 254 is:
