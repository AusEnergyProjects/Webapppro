...
| Current Creditex VEU synthetic pilot application source | `ebae330dab6c42881c14bc57548095b111d9c850` | Exact validated source retaining the pilot and correcting authentication recovery; pushed to GitHub and Sites managed `main` |
| Current saved-version identity | `appgprj_6a550c378000819185caf094173422bb~appgver_900ff3f8d0448191a798a5eb10ef648c` | Exact saved Sites version 253 built from `ebae330dab6c42881c14bc57548095b111d9c850`; package content hash `sha256:20934a09b4658dafbd2b9c420c402028a3619a9fc9216bd2cb154ebcf12b1e98` |
| Current executable application identity | Sites version 253 from `ebae330dab6c42881c14bc57548095b111d9c850` at `https://compare.ausenergyassessments.com/creditex/compliance` | Deployment `appgdep_6a6dde1747308191bf5c78bd4f674030` succeeded; environment revision 19; provider URL `https://aea-energy-comparison.info294029.chatgpt.site` |
| Current source checkpoint | Documentation-only child of `ebae330dab6c42881c14bc57548095b111d9c850` | This release record does not change the exact executable source for public Sites version 253 |
...
The additive schema expansion, reviewed-ABN application, authorised contract cleanup, customer, installer and trade releases, protected-trade locality, Creditex foundation, operations portal, evidence-policy governance, national government-activity workflow and isolated VEU synthetic pilot are deployed to production. Sites version 253 from `ebae330dab6c42881c14bc57548095b111d9c850` is the exact current executable application source. Failed versions 243, 244 and 245 never activated. Intermediate version 247 activated long enough for signed-in QA, exposed a production-D1 incompatibility in one aggregate query and was replaced by version 248, which is now historical. The release retains 32 Australian government program pathways and 207 controlled activity templates and adds a controlled 10-installer, 30-technician, 300-job VEU pilot. The real governed inventory remains 0 published programs, 0 activity versions, 0 policies and 0 regulated cases, and every unverified calculator and external execution path remains disabled.

## Current Creditex VEU synthetic pilot

`CREDITEX-VEU-SYNTHETIC-PILOT-28` was first deployed from application commit `3ac6c72057a8afea61e85817ba566ec543079886` as Sites version 252. Corrective application commit `ebae330dab6c42881c14bc57548095b111d9c850` retains that pilot and is the current Sites version 253. Saved version `appgprj_6a550c378000819185caf094173422bb~appgver_900ff3f8d0448191a798a5eb10ef648c` reports the exact current application source, and deployment `appgdep_6a6dde1747308191bf5c78bd4f674030` succeeded with environment revision 19.

The signed-in portal retains the discovery-only national reference catalogue, chained program and activity selectors, Dataforce-equivalent filters and audited private case workspace. It adds Pilot control, Jobs, Sources, Lookups, Evidence, Calculators and Connectors work areas; pre-populated filters; and a fixed Dashboard plus one bottom tab for each of the 34 represented VEU activity families. Part `6` is the official family identifier. Categories and scenarios remain separate governed dimensions; `6(23)` was only an informal example and is not a special implementation path.
...
The exact current application source passes the complete `npm.cmd run validate` gate: type checking, warning-free lint, 31 of 31 integration tests, 1,180 main tests with 1,178 passed, 2 intentionally skipped and 0 failed, all 100 migrations through `0099_creditex_synthetic_pilot.sql`, the customer-plan PDF audit, Vinext production build and Sites server-bundle audit. The focused pilot suite passed 13 of 13 tests, the focused Creditex portal suite passed 25 of 25 and `git diff --check` passed.

The current local release package is 7,420,447 bytes with SHA-256 `51DF880CF8C919FA0386B891BC98C18064491988325049B59CC3F4A4BCE370DA`. Sites stored 333 files and 29,624,320 bytes with content hash `sha256:20934a09b4658dafbd2b9c420c402028a3619a9fc9216bd2cb154ebcf12b1e98`.
...
Signed-in Chrome QA created the synthetic seed and confirmed 10 of 10 installers, 30 of 30 technicians, 300 of 300 jobs and 34 of 34 activity families. Filtering installer `I01` plus Part `6` returned one job. A controlled status update persisted after refresh and the dry-run connector retained one 300-item manifest while reporting zero regulator acceptances, rejected or unmatched items. Structural counters remained 0 regulated cases, 0 Firebase test users and 0 certificates. Desktop and 390-pixel mobile inspection passed without page-level horizontal overflow; production Worker evidence contained no `5xx`. Version 253 removed the unconditional forced token refresh, bounds recovery to one forced refresh after an authentication-specific `401`, preserves the signed-in identity on workspace failure and separates authentication errors from workspace errors. A signed-in production reload returned directly to the protected dashboard and reopened the populated VEU pilot. Browser logging contained only the Chrome extension's closed asynchronous message-channel warning, not an application exception.
...
The current verified deployed topology for Sites version 253 is:
