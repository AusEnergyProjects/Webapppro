...
| Protected trade locality and reciprocal navigation application source | `399b04f4a5d680080610f9e88b994506bb60c16f` | Validated, pushed to GitHub and Sites managed `main`; saved and deployed as Sites version 242 |
| Current saved-version identity | `appgprj_6a550c378000819185caf094173422bb~appgver_bc9f3157a9e88191881c5989f7de7ba0` | Exact saved version 242 built from `399b04f4a5d680080610f9e88b994506bb60c16f`; package content hash `sha256:3d7535003e6b3fae6b2b7f4f86b5c69a59737a8aa607ba7feabdbd407fd890f0` |
| Current executable application identity | Sites version 242 from `399b04f4a5d680080610f9e88b994506bb60c16f` at `https://compare.ausenergyassessments.com` | Deployment `appgdep_6a6cc08dc6f881919a349de607f5a8a9`; saved-version provenance, custom-domain production deployment and environment revision 19 verified on 1 August 2026 |
| Current source checkpoint | Documentation-only child of `399b04f4a5d680080610f9e88b994506bb60c16f` | This release record does not change the exact executable source for public Sites version 242 |
...
The additive schema expansion, reviewed-ABN application, authorised contract cleanup, customer home advisor, advisor-context release, independent customer-plan release, customer plan evidence-and-history release, professional-review, browser-native PDF, premium customer-report, technical presentation, spacing, customer-plan trust, customer-project cleanup, customer-roadmap-context, customer-installer-request, customer-plan-durability, installer-request-and-multi-photo, authoritative-installer-submit, installer-enquiry-pack, complete customer-installer-handoff, customer quote-communications, customer-to-trade contact, customer-plan trade-enquiry, customer-account-trust and protected-trade-locality releases are deployed to production. Sites version 242 from `399b04f4a5d680080610f9e88b994506bb60c16f` is the exact current executable application source. Version 237 from `40b4396b9ef41166a61ee346b023c00bcc9df11b` is documentation-only saved evidence and was never deployed; version 236 remained public until version 238. Version 231 is failed non-live evidence only; it never superseded version 230. Version 214 is historical and superseded because its signed-in action could save the project, process pending images and upload evidence before PDF generation, then attempt a delayed synthetic download. Version 216 established the non-mutating attachment path. Version 218 established the shared premium PDF and email hierarchy, version 220 added the exact AEA mark, technical visual system, truthful completed-plan state and one non-duplicated household or professional evidence boundary, and version 222 applied consistent spacing and rounded surfaces. Version 224 added shared premium preview, duplicated bottom actions, guided private photos, bounded revision compare and restore, a tagged-PDF foundation and adaptive email compatibility. Version 227 added compact dashboard and project-detail controls plus guarded permanent deletion for unused private drafts. Version 229 moved recommendation-shaping home and work context before roadmap generation and removed repeated quote-preparation priorities. Version 230 made completed stages explicit and put private profile completion and request recovery into one focused dialog. Version 232 kept guided photos visible where they were added, hardened resumable evidence and deletion, added useful comparison and check-in history, submitted from one request confirmation, and used a worker-safe embedded-font PDF boundary. Version 233 removed the trigger-amplified profile conflict and let each guided section hold several separately controlled photos. Version 234 made modal contact authoritative and committed contact plus matching state in one guarded request. Version 235 added the bounded installer enquiry pack, consent-gated evidence presentation, automatic durable business-notification queue and shorter submit critical path. Version 236 records all follow-up work durably before returning, shares every active project photo by explicit request consent, exposes the complete protected plan and PDF to the exact eligible installer, independently queues operations and business alerts, and gives the customer staged progress while external delivery drains outside the response. Version 238 added the customer quote centre, exact quote deep links, customer and installer quote emails, trade Work updates, retry-safe submission and a concurrency-safe one-business selection. Version 239 replaced public shortlist and acceptance semantics with a single contact-only choice, made new allocations visible in the owner Work updates bell, compacted lead cards, aligned quote fields and returned Continue navigation to the active step heading. Version 240 added the private account bridge from the public roadmap, more precise hot-water and electrical-phase choices, technical ventilation facts and prominent connected-customer identity in the authorised trade lead. Version 241 replaces the customer-facing technical ventilation questions with one plain visible-fan question, makes the email account controls clearly usable and refreshes Firebase customer verification state and the trusted ID token on return. Version 242 keeps the installer-request consent and any error beside the submit action, aligns the shared customer navigation, adds reciprocal TLink and Australian Energy Assessments branding, and discloses only a current-consented immutable suburb, postcode and state snapshot to eligible installers. The retained internal `accepted` identifier is compatibility state for the one-business claim and is not evidence of payment, contract, invoice, quote acceptance or authorised work. Production provider inbox receipt, provider credentials and sender approval, hosted row counts, and a newly generated customer verification email action remain unverified. Earlier free-access and integration boundary checks returned `200`; retired membership, billing, referral and payment-link routes returned `404`; and an unauthenticated trade CRM request returned `401`.

## Unreleased Creditex compliance operations

`CREDITEX-COMPLIANCE-OPERATIONS-25` is a release candidate in the working tree based on repository checkpoint `f3b453c5b556fac62a548a4e7b74841f67a3c0d6`. Until an exact application commit, Sites version and deployment are recorded here, current production remains Sites version 242 from `399b04f4a5d680080610f9e88b994506bb60c16f`.

The candidate introduces a separate invitation-only compliance organisation and membership domain; immutable source-backed program, activity and evidence-policy versions; atomic installer job and compliance-case creation; participant abilities, assignments, tasks, findings, decisions, equipment and local external-state ledgers; original-evidence custody through AEA Field; and a no-index Creditex case workspace with audited evidence viewing, correction, dual control, access administration and provider-neutral batch staging. The accompanying [Australian program source register](./compliance/AUSTRALIAN_PROGRAM_SOURCE_REGISTER.md), [Creditex operating model](./compliance/CREDITEX_OPERATING_MODEL.md) and [Dataforce and Runabout parity record](./compliance/CREDITEX_DATAFORCE_PARITY.md) define the public discovery baseline, target workflow and explicit private-source gaps.

No activity or evidence policy is seeded as published. Unverified calculators, manual response assertion, registry submission, certificate creation, trading, settlement, Dataforce or Runabout migration and real regulated cases remain hard-disabled. A policy withdrawn after a case is opened remains available for evidence correction and audit, but approvals and batch staging fail closed. The `info@ausenergyassessments.com` invitation is a time-bounded bootstrap only; routine Creditex access requires named verified users and least privilege.

The settled candidate passes 81 of 81 focused Creditex, governance, access, viewer, field-evidence, guided-job, photo and database-console tests. `npm.cmd run validate` passes type checking, warning-free lint, 31 of 31 integration tests, 1,077 main tests with 1,075 passed, 2 intentionally skipped and 0 failed, all 98 migrations through `0097_creditex_operations_lifecycle.sql`, the customer-plan PDF audit, production build and Sites server-bundle audit. Failed Sites versions 243, 244 and 245 did not activate; each stopped at the D1 migration gate with `SQLITE_ERROR: incomplete input`. Migrations `0093` to `0097` now carry only complete line-bounded schema and seed statements; the original `0096` and `0097` history slots are retained to avoid a filename collision with any rejected saved version. Because the Sites migration parser cannot accept trigger bodies, `src/lib/creditex-schema-guards.ts` preserves the same 132 definitions with only `IF NOT EXISTS` added, installs them in at most 40-statement prepared D1 batches with an automatic portal retry boundary, verifies every installed definition against the exact expected `sqlite_schema` SQL, and blocks Creditex access, governed case creation, verified trade access, team access and governed customer-media deletion on any missing, stale or failed installation. AEA Field separately passes 6 of 6 tests, lint, type checking and Android and iOS export. Independent final review approves release only as an empty invitation-gated baseline. Exact release provenance and production portal checks remain pending. Authenticated Creditex administration, non-admin least privilege, physical-device capture, hosted original-evidence viewing, rule-pack accuracy, approved calculator provenance, retention, legal hold, backup and restore and all external provider behavior remain unverified or blocked.
...
The current verified deployed topology for Sites version 242 is:
