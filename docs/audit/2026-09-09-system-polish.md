...
## Follow-up: field workflow and rental quoting reports

James expanded the active scope using a Detector Inspector gas report and a privately supplied Homes Victoria/Jazz upgrade report as usability benchmarks. The latter is a quotation-information benchmark, not a replacement legal standard. No source document, private customer details or reference photos are committed to the application.

- Mobile job search now works across downloaded assignments, with protected contact fields excluded. Call, text, directions, files, time and commercial actions are near the top of the job. Photo/time/form saves use inline confirmation; required actions still await durable local writes. Picker guards prevent duplicate capture; failed time entries preserve their input. Optional program activity selection stays collapsed until requested.
- Fixed a queue acknowledgement race: dispatched actions retain an immutable ID/payload; an edit racing with dispatch creates a new action. Pending form answers are projected over refreshes, preserving server revisions. Legacy queued actions are frozen transactionally on upgrade. The new SQLite race suite passed five cases; physical-device restart and bad-network operation still need field validation.
- General electrical and plumbing work are now canonical business/team capabilities, alongside the existing insulation service. Duplicate picker overrides were removed. Migration 0174 increases the existing enquiry selection limit from 14 to 16, preserving rows, events and constraints. Both services have practical enquiry questions.
- New rental jobs default to 24 current checks across all 15 minimum-standard categories plus eight future energy checks, with their March/July 2027 triggers and July 2030 cooling deadline. Existing unissued checklists can be explicitly expanded while retaining observations. Issued report snapshots remain immutable.
- Rental capture includes safe assessor observations: equipment identity and dimensions, room/recess measurements, uninsulated area, insulation-label evidence, seal lengths, door sizes, timed water-flow readings, visible switchboard labels and access/service-route constraints. Flow does not establish WELS rating; insulation depth does not establish R-value. Design, concealed services and statutory tests retain qualified-trade boundaries.
- Findings prefill a relevant trade, including insulation installer. Mobile now preserves measured quantities and units. One work-scope entry replaces duplicate action questions. Each proposed job records measurements, specification, access, inclusions/exclusions and unresolved information. Quote-ready status requires a positive quantity, these details and at least two evidence files; unverified/inaccessible conditions or outstanding information prevent that status. A documented follow-up scope can still be issued honestly.
- The PDF has a simple owner/agent summary, separate trade schedules, two-column photo evidence, cross-references instead of repeated photos, the complete checklist and evidence/source appendix. The public viewer uses the same quoting-readiness checks. A reusable synthetic preview script exercises the actual PDF builder; sample output is prominently marked as unissued.
- Ordinary Schedule 4 rental rules are distinguished from community/specialised housing, rooming houses and unconfirmed regimes. Other regimes produce a prominent applicability limitation and observations only; they do not become ordinary-rental compliance conclusions. Current failures and future planning gaps are classified separately at report issue, even if the recorded regime changed after an observation.

Follow-up validation: full Node 22 suite 4,063 passed, 11 intentional skips, zero failed; mobile suite 101 passed. Root/mobile typechecks, lint and all 173 migrations passed. Rental/quotation focused suite passed 40 cases; later boundary additions are covered by the full run. PDF layout was rendered and reviewed, including the summary, trade pages, evidence and source appendix. Publication build and exact deployment evidence are recorded by the release task after the source commit. No real reports were issued, messages sent or on-device success inferred.
