...
- `npm.cmd run validate` passes type checking, warning-free lint, 31 of 31 integration tests, 1,073 main tests with 1,071 passed, 2 intentionally skipped and 0 failed, all 98 migrations through `0097_creditex_operations_lifecycle.sql`, the customer-plan PDF audit, production build and Sites server-bundle audit. Sites rejected the original multi-line migration before production activation because its D1 runner executes each physical line as a statement. New migrations `0093` to `0097` therefore retain one complete SQLite statement per line, with the operations SQL also split into sub-25 KB inputs; the regression is covered by the focused suite.
