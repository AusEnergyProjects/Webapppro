...
## Active contract: approved VEU API access and a verified submission

Outcome: Creditex selects reviewed jobs and submits them through an approved VEU integration with minimal clicks. Account defaults, existing review gates, immutable receipts and automatic per-job outcomes remain authoritative. No manual-reference action may be presented as direct submission.

Owner: C:/Webproject/aea-energy-creditex-submissions, branch codex/creditex-registry-submissions. Current changes include corrected duplicate handling, dispatch response validation, scheme/activity-aware file selection, automatic application of the sole eligible saved account, direct lodgement-reference recording and REGO/ACCU routes through migration 0191.

External step in progress: the VEU software-provider access email was sent from info@ausenergyassessments.com to veu@esc.vic.gov.au on 25 September 2026, offering a TLink demonstration before access is granted. Do not send a duplicate request. ESC must send the MuleSoft invitation. Private account/password setup stays with James. In Exchange, request VEU Registry Experience API UAT access, read the current full contract, complete required testing, then seek separate production access and Creditex delegation.

Acceptance: use the approved current schema/authentication; demonstrate one permitted activity through UAT with verified returned reference and status; test partial rejection, timeout, duplicate clicks, authority revocation and reconciliation; retain original government evidence; distinguish lodgement, invoice, payment, assessment and registration. Only then describe the native connector as working. Extend to other activity contracts using the same verified gates. No public NSW or REC certificate-creation API has been verified, so do not invent one. Their official file/portal routes remain available.

Validation: focused submission/UI/concurrency/format tests, populated and fresh D1 migration checks, typecheck and publication build. The full suite has eight known baseline failures; task-related exact migration counts now include 191. Full lint's unchanged official-product-refresh test rule violation is outside this scope. Current evidence is under C:/Webproject/outputs/creditex-submission-assurance-2026-09-25/.
...
Verified publication: GitHub main and task branch were pushed to `9b76d686fcbe7f49c2bae429b70c3e62910dd21e`. Sites 614 deployment `appgdep_6ab4bea6e6508191bf5ccc03ca077d87` succeeded from that same source with environment 116. The current isolated worktree preserves all fourteen pre-existing live-source commits. Follow-up edits add the live phone form preview, direct Team access, read-only form browsing and quicker registry file preparation; the coordinating task records their later exact commit and publication.

Validation completed: 170/170 focused registry, dispatch, navigation, governance and release tests; 37/37 integration tests; typecheck and full ESLint; 189 production migrations replayed against fresh local D1; customer PDF, source custody, assessor education, conversation quality, community response and continuity checks; production build with Sites bundle and public-performance audits. Eight synthetic desktop/mobile screenshots of the real component passed overflow and browser-error checks. The authenticated Sites 614 Submissions workspace loaded successfully without performing a real submission.

Follow-up acceptance: preview Next/Back must work without answers or signatures; optional checks report missing answers. Eight integrated phone/editor combinations passed, along with 142 phone/projection/master/security/flow checks, 123 editor/access checks, 50 team-access checks, 111 registry checks and 34 publication/registry/release reruns (overlapping groups). Root typecheck, full ESLint and the production build with bundle/performance audits passed. The full suite reported 5,323 passed, 11 skipped, eight existing assertion failures, two stale assertions subsequently corrected and passed, and one worker crash whose isolated rerun passed. Mobile dependencies are absent, so no native typecheck or build is claimed. No additional migration or dependency was introduced. Final GitHub/Sites identity and live follow-up checks are recorded by the coordinating task.
...
Resolved publishing interruption: the missing Sites packaging helpers were bypassed using the native tool's documented remote-build fallback, after verifying the exact source push. The official helpers remain intermittently available under `C:/Users/james/.codex/plugins/cache/openai-curated-remote/sites/0.1.71/`; use that workflow when present, or the documented native source-build fallback when local packaging cannot complete. Logs and visual fixtures are in `C:/Webproject/outputs/creditex-certificate-submissions-2026-09-24/` and `C:/Webproject/outputs/creditex-registry-ui-2026-09-24/`. No regulator submission, customer communication or payment was performed.
...
Scope: existing accounting OAuth, issued invoice export/query, one-click sync controls, saved per-business account mapping, regression tests, provider app registration and runtime credentials. Additive migration 0181 stores the mapping on the integration and clears it when its external business changes. Preserve encrypted owner-scoped tokens, consent boundaries, signed invoice totals and duplicate-export safeguards. No payment collection, automatic customer emails or unrelated portal changes.

Acceptance: accepted and quick invoices retain their correct references; all three providers receive issued invoices with amounts owing; a connected business reuses its confirmed income account or product mapping and uses one Sync action; provider callbacks and actual credentials are configured before claiming a connection works. Verify one authorised test organisation per provider without sending customer emails. Responsive integration controls must remain usable.
...
Validation evidence: 95 related accounting, OAuth, privacy, release and complete-schema tests pass; typecheck, lint and all 181 migration applications pass. The actual accounting component was checked with synthetic provider responses: one click produced exactly one sync request and an issued result for each provider; desktop and narrow layouts remained usable. Seven unrelated service-selection/brand test failures reproduce on the clean live baseline. Exact publication build and final full-suite evidence are recorded by the coordinating release task. No dependencies or customer data changed.
...
The Samsung sign-in update block is fixed at its source. Build 2 is version 1.0.1, but the mobile request configuration had been hardcoded to 1.0.0. TLink now reads the native installed application version and keeps the server minimum at 1.0.1. An accessible cog is always visible at the top right of the signed-out and restricted startup screens. Its settings panel checks for a compatible EAS update, opens the secure full-build page and shows the installed version and build.
...
- Scope selection is identical in the public request, TLink new job and TLink Field new job surfaces. The server rejects empty or unknown combinations.
...
- TLink Team presents add person, open details and generate and email PIN as one visible three-step flow. The office controls the unique TLink username, a saved email is required for PIN delivery, and a username change revokes any unused PIN.
- The account owner has a `Set up my app` action and can create their own field username and PIN without adding a duplicate team member.
- A TLink administrator can issue one active setup PIN per person, see it once, revoke a device or sign the person out everywhere. PIN attempts are rate limited and stored credentials are salted, peppered hashes.
- Get the app is always visible in the TLink dashboard header with a compact TLink icon. The dedicated install page and live release API return signed Android version 1.0.1 build 2.
