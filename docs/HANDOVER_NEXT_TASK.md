...
Milestone ID: `CUSTOMER-PLAN-DIRECT-PDF-06`

Implementation baseline: `480e8a76545b8033b3b6c7ae3913276b43ef7373`

Released application for this milestone: Sites version 214 from application commit `d5c675a5ceffa6e924df033e8cb8b505bb4d6336`
...
Let a household or self-declared accredited adviser download one readable, useful and brand-agnostic home energy plan without invoking Chrome's native print system or turning general guidance into a credential check, NatHERS assessment, product endorsement or savings promise.

The optional professional-review declaration and bounded everyday comfort actions remain unchanged from the prior milestone. Public and signed-in plans now project the same privacy-filtered report into a direct A4 PDF in a dedicated lazy worker. The customer-plan path no longer creates a print iframe or calls a browser print API, removing the native-print subsystem that continued to freeze Chrome after the earlier temporary-frame mitigation.
...
- Public and signed-in PDF actions use one background generation job, prevent duplicate downloads, transfer the generated bytes from a lazy worker and revoke the temporary object URL after download.
- Long adviser names, references and notes wrap safely in the A4 report.
- The PDF format version is `2026-07-29-direct-download-pdf-v1`; the plan remains `2026-07-29-adviser-print-comfort-v3`, the adviser profile remains `2026-07-29-advisor-profile-v4`, the declaration remains `2026-07-29-self-declared-adviser-v1`, the document remains `2026-07-29-plan-document-v2`, and the concise report remains `2026-07-29-concise-report-v2`.
...
- Generate customer-plan PDFs off the main thread with locally bundled fonts, direct Blob download and bounded cleanup.
...
- A server-side browser, print service or persisted PDF object. The PDF is generated locally in the customer browser and is not uploaded.
...
- Customer-plan delivery has no native print, iframe or `afterprint` path, allows one generation job at a time and performs bounded worker, Blob and object-URL cleanup.
...
- a customer-plan action could invoke native print, print application chrome, allow overlapping generation jobs or leak a worker, Blob URL or hidden download element;
...
```powershell
node --experimental-strip-types --test test/customer-plan-pdf.test.mjs test/customer-project-advisor-ui.test.mjs test/site-navigation.test.mjs
...
- focused PDF, customer-project UI and navigation set: 40 of 40 passed;
...
- GitHub and Sites managed source branch: exact application SHA `d5c675a5ceffa6e924df033e8cb8b505bb4d6336`;
- Sites application version: 214, deployment `appgdep_6a69e79a91548191987f12631559cb1f`, public, environment revision 19;
- a representative maximum-content seven-page A4 PDF with long adviser content and six everyday actions was visually inspected without clipping, split action text, unreadable contrast or application chrome;
- live public `/plan` exposed the exact `Preview and download PDF` route, and `/plan/print` exposed one enabled `Download PDF` action with no native-print copy, alert or JavaScript dialog;
- the live action created `home-energy-plan-2026-07-29.pdf`; independent parsing confirmed a valid unencrypted three-page A4 document, the expected title and author, readable first-page text and no embedded JavaScript;
...
- Application commit: `d5c675a5ceffa6e924df033e8cb8b505bb4d6336`
- Sites application version: 214
- Sites application deployment: `appgdep_6a69e79a91548191987f12631559cb1f`
- Production URL: `https://compare.ausenergyassessments.com`
...
## Prior released milestone: `CUSTOMER-PLAN-PRO-PRINT-05`
