...
Public research is discovery evidence only. It is not authority to activate claims. Before any activity can accept a real case, Creditex must supply or approve its current private accreditation scope, participant agreements, operating rules, evidence interpretation, submission schema, reconciliation process and trading or settlement controls. The controlling regulator or program material must also be current and effective for the relevant activity date.
...
10. A public program page can establish that a program exists. It cannot establish Creditex authority, exact eligibility, an evidence interpretation or a certificate quantity.
...
| 3. Atomic job and case creation | TLink | Create the installer job and linked Creditex case together. Snapshot rule-pack identifiers and source hashes. | Both records exist or neither exists. |
...
| 3. Creditex private authority | Current accreditation schedules, retailer or regulator agreements, participant contracts, approved interpretations, field dictionaries, evidence manuals, submission schemas and settlement controls | Establish what Creditex is authorised and contractually able to do, and how it must do it. Required for activation. |
...
An activity can be published only when the controlling and operational sources are captured, Creditex private authority is approved, contradictions are resolved and two authorised people sign the release. If a Tier 1 or Tier 2 source conflicts with Creditex instructions, the activity remains blocked until Creditex obtains and records a lawful resolution.

## Immutable rule-pack contracts
...
| Evidence-policy version | Ordered conditional requirements with timing, minimum and maximum count, original-file rule, metadata and GPS rule, signatures, declarations, documents, allowed types, validation schema and exact source citation. |
...
| --- | --- | --- |
| Administrator | Organisation settings, invitations, rule governance and emergency suspension | Does not inherit installer workspace access and cannot self-approve controlled changes. |
...
| --- | --- | --- |
| Public research by itself | Disabled | Exact effective official source plus Creditex private authority and dual approval. |
...
| Dataforce and Runabout private schemas or APIs | Unknown, no live connector | Creditex provides authorised documentation, sample exports, field definitions and permitted access. |
...
| 1. Shadow foundation | Configure one exact Creditex-approved program and activity version, evidence policy and calculator in a non-production environment. | Test vectors, access denial, custody, backup and audit checks pass. |
...
- at least one complete activity pack has authoritative source hashes, evidence rules, calculator tests where applicable, participant and product checks and dual approval;
