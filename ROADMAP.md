...
- P6-3B adds reusable job packets inside the price-book workspace, combining current saved items with existing tasks, forms, duration, capabilities and suggested crew while immutable quote revisions retain exact packet references.
- P6-3C adds one-action standard or Essential, Recommended and Complete quotes, customer-facing sections, optional extras, choose-one groups and exact server-validated customer selection evidence without exposing internal margin.
- P6-3D adds a revocable and expiring no-account quote link, automatic business branding, phone-first and print review, deliberate email, bounded questions, typed signature and one office activity timeline. Accepted records carry stable AUD integer-cent commercial references compatible with Xero, MYOB, QuickBooks, Stripe and Square adapter boundaries.
- P6-3E turns that acceptance into one immutable commercial handoff with a simple percentage or fixed deposit, one idempotent provider-hosted Stripe or Square request, complete Xero, MYOB and QuickBooks accounting export compatibility and one recovery-oriented financial timeline. Provider callbacks remain the only payment truth and no accounting draft is approved or emailed automatically.
- P6-3F adds the accepted-scope job-plan foundation, one readiness view for scope, forms, technician, materials and deposit, and safe technician preassignment before invitation acceptance. The linked catalogue usability batch adds searchable include and exclude header filters, obvious horizontal navigation and verified wholesaler profiles with self-maintained dispatch locations and approved products.
- P6-3J removes duplicate-entry friction from initial job intake and gives field workers one phone-first job flow. Existing customers and service sites are reused through accessible search, new customer, site and job records are committed together with duplicate protection, structured addresses retain a manual fallback behind a provider-neutral suggestion boundary, optional notes have one owner, and the audited Start travel, Arrive, Start work and Finish sequence reuses current tasks, forms, proof, issues, invoice and handover records without exposing protected AEA contacts.
