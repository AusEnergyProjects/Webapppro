...
test("lead events map every live email journey to a stable contract", () => {
  assert.equal(
    leadEventType({ submissionType: "comparison" }),
    "comparison.results",
  );
  assert.equal(
    leadEventType({ ...base, enquiry: "electricity-solar-battery" }),
    "electricity.upgrade",
  );
  assert.equal(
    leadEventType({ ...base, enquiry: "gas-heating" }),
    "gas.upgrade",
  );
  assert.equal(
    leadEventType({ ...base, enquiry: "direct-trade-project" }),
    "direct_trade.project",
  );
  assert.equal(
    leadEventType({ ...base, enquiry: "direct-trade-partner" }),
...
test("Direct Trade partner envelopes begin a non-automatic participant review", () => {
  const envelope = createLeadEnvelope(
    { ...base, enquiry: "direct-trade-partner", partnerType: "supplier" },
    {
      createId: () => "12345678-abcd-4000-8000-123456789abc",
    },
  );
...
test("lead envelopes reject unsupported upgrade events", () => {
  assert.throws(
    () => createLeadEnvelope({ ...base, enquiry: "unknown" }),
    /unsupported/i,
  );
});
