...
const source = fs.readFileSync(new URL("../integrations/google-apps-script/lead-email-relay.gs", import.meta.url), "utf8");
...
function relay() {
  ...
  const context = vm.createContext({
    ...
    Utilities: {
      formatDate() { return "14 Jul 2026, 11:02 am AEST"; },
      getUuid() { return "12345678-abcd-4000-8000-123456789abc"; },
      computeDigest() { return [1, 2, 3, 4, 5, 6, 7, 8]; },
      DigestAlgorithm: { MD5: "MD5" },
    },
    ContentService: { MimeType: { TEXT: "text" }, createTextOutput(value) { return { value, setMimeType() { return this; } }; } },
    HtmlService: { createHtmlOutput(value) { return value; } },
  });
...
const comparison = {
  ...
  top3: [
    { rank: 1, brand: "Example Energy", plan: "Example Saver", offerId: "EX123", annual: 1412, link: "https://example.com/plan" },
    { rank: 2, brand: "Second Energy", plan: "Second Saver", offerId: "SE456", annual: 1498, link: "https://second.example/plan" },
  ],
  provenance: { sourceFetchedAt: "2026-07-14T00:00:00.000Z", annualSource: "manual", conditionalDiscountsAssumed: false },
};

test("the relay recognises every versioned event and operational probes", () => {
  const { context } = relay();
  assert.equal(context.eventType_({ submissionType: "comparison" }), "comparison.results");
  assert.equal(context.eventType_({ enquiry: "electricity-battery" }), "electricity.upgrade");
...
test("customer and internal acknowledgements are tailored to each workflow", () => {
  ...
  const payloads = [
    ...
    { ...comparison, eventType: "direct_trade.project", enquiry: "direct-trade-project", projectSource: "gas-heating", projectCategories: ["assessment", "solar"], propertyType: "house", propertyRelationship: "owner-occupier", projectPriorities: ["need-advice"], projectStage: "researching", timeframe: "urgent", preferredContact: "phone", projectNotes: "Review the home before recommending equipment.", directTradeTriage: { status: "manual_matching_review", priority: "urgent_manual_review", autoSend: false, reviewFlags: ["assessment_or_advice_may_be_needed_first"] } },
    { ...comparison, eventType: "direct_trade.partner", enquiry: "direct-trade-partner", partnerType: "supplier", businessName: "Example Supply", serviceStates: ["VIC", "NSW"], projectCategories: ["battery"], partnerNotes: "Local stock and warranty support.", participantReview: { status: "application_received", autoApprove: false, publicListing: false, checks: [{ id: "product", label: "Product specifications and compliance evidence", status: "not_started" }] } },
  ];
...
test("all untrusted email values are escaped and retailer buttons require HTTPS", () => {
  const { context } = relay();
  const html = context.planCards_([{ rank: 1, brand: '<img src=x>', plan: '<script>alert(1)</script>', annual: 1000, link: 'javascript:alert(1)' }]);
