...
test("Resend requests use provider idempotency, an HTML alternative and optional attachments without exposing credentials", async () => {
  let captured;
  const result = await sendServiceReminderProviderMessage({ channel: "email", recipient: "customer@example.com", subject: "Service due", body: "Review your service reminder.", html: "<p>Review your service reminder.</p>", idempotencyKey: "key-1", callbackUrl: "https://example.com/callback",
  ...
  assert.equal(payload.text, "Review your service reminder.");
  assert.equal(payload.html, "<p>Review your service reminder.</p>");
