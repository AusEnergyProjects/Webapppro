...
const matchesRoute = read("../src/app/api/admin/opportunities/matches/route.ts");
const partnerOpportunities = read("../src/app/api/trade-opportunities/route.ts");
...
test("operations portal is unlisted and every administrator API has server-side roles", () => {
  assert.match(portalPage, /robots: \{ index: false, follow: false, noarchive: true, nosnippet: true \}/);
  for (const route of [sessionRoute, accountsRoute, adminsRoute, evidenceRoute, opportunitiesRoute, matchesRoute]) {
...
test("administration is least privilege and protects the final owner", () => {
  ...
  assert.match(accountsRoute, /Reviewers can update verification status and internal notes only/);
...
test("opportunities remain privacy-safe and partner responses stay owner scoped", () => {
  ...
  assert.doesNotMatch(schema, /household_name|customer_email|customer_phone|street_address/);
  assert.match(partnerOpportunities, /WHERE m\.firebase_uid = \?/);
  assert.match(partnerOpportunities, /WHERE id = \? AND firebase_uid = \?/);
  assert.match(dashboard, /Household identity and street address are not exposed/);
...
test("operations UI covers accounts, evidence, projects, access and audit", () => {
  ...
  assert.match(portal, /www\.gstatic\.com\/firebasejs\/ui\/2\.0\.0\/images\/auth\/google\.svg/);
  ...
  assert.match(portal, /Assign to dashboard/);
