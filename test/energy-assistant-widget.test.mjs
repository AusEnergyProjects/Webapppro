...
test("browser storage failures cannot break widget hydration, persistence or reset", () => {
  const helperSource = [
    ...
    functionSource(widget, "availableSessionStorages"),
  ...
  const helpers = Function(
    ...
    (value) => ({
      ...
      continuation: null,
      profile: value?.profile || { reviewed: [], completed: false },
  ...
  try {
    ...
    const validValue = JSON.stringify({ profile: { reviewed: [], completed: false } });
    helpers.storeSession(validValue);
...
test("desktop context answers have an independently usable scroll region", () => {
  ...
  assert.match(styles, /@media \(min-width: 641px\) \{[\s\S]*?\.rootDedicated \.contextProgress \{[^}]*padding-inline: 16px;/);
...
test("optional help navigation targets and focuses the newly opened service form", () => {
  ...
  assert.match(widget, /const container = conversationRef\.current/);
  assert.match(widget, /container\.scrollTo\(\{[\s\S]{0,220}container\.scrollTop \+ formTop - containerTop - 16/);
...
test("page navigation stays restricted and validated official citations render as compact links", () => {
  ...
  assert.match(styles, /\.practicalSteps\s*\{[^}]*list-style:\s*decimal outside;/);
  assert.match(styles, /\.practicalSteps > li::marker\s*\{/);
