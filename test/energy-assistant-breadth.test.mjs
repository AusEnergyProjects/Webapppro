...
test("remaining whole-market P1 decisions reach a specific bounded answer", () => {
  const cases = [
    ...
    ["How do I invite an apprentice to help me in TLink?", /open Team.*Add team member.*Generate 6-digit PIN.*TLink app/i, "trade"],
...
test("P1 decision families generalise to unseen wording and values", () => {
  const cases = [
    ...
    ["Where in TLink do I add a junior technician and create their private login?", /dashboard, open Team.*Generate 6-digit PIN.*owner or delegated access manager/i, "trade"],
