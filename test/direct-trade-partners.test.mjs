...
test("partner form uses visible shared controls and required address fields", () => {
  ...
  assert.match(form, /<AustralianAddressLookup[\s\S]{0,120}label="Business street address"[\s\S]{0,120}required/);
  assert.match(form, /setAddressLine1\(\[selection\.addressLine2, selection\.addressLine1\]\.filter\(Boolean\)\.join\(", "\)\)/);
  assert.match(addressLookup, /required=\{required\}/);
