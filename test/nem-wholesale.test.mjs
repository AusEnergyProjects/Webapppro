...
test("page discovery, consumer explanations and accessible interaction are present without adding market code to the header", () => {
  ...
  const usefulEnergy = read("src/lib/useful-energy.ts");
  const priceSnapshot = read("src/lib/wholesale-price-snapshot.ts");
  const location = read("src/lib/wholesale-location.ts");
  ...
  assert.match(page,/illustrative room-heating and hot-water examples using 24-hour average wholesale prices/);
  assert.match(page,/Western Australia and the Northern Territory are outside the NEM/);
  assert.doesNotMatch(page,/A fair heating comparison/);
  ...
  assert.match(component,/What could this energy do\?/);
  ...
  assert.match(component,/payload\.postcode !== submittedPostcode/);
  assert.match(component,/requestId !== postcodeRequestId\.current/);
