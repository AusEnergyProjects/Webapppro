...
test("job setup and field workspace attach one frozen rental workflow with guarded evidence", async () => {
  const [form, route, assessmentRoute, assessmentPanel, mobilePanel, fieldRoute, assignmentRoute, migration, schema] = await Promise.all([
    ...
    source("drizzle/0142_trade_rental_inspections.sql"),
    source("db/schema.ts"),
  ]);
  ...
  assert.match(assignmentRoute, /UPDATE trade_rental_inspections/);
  assert.match(assignmentRoute, /assessor_snapshot/);
  for (const table of [
