...
test("orders are created only from a wholesaler response and snapshot selected commercial items", () => {
  ...
  assert.match(route, /verifiedTradeAccountPredicate\("a"\)/);
  assert.match(route, /p\.listing_status = 'published'/);
  assert.match(route, /lineItems\.results\.length !== Number\(expectedItems\?\.count \|\| 0\)/);
