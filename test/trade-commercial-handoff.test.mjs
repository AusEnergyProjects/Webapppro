...
test("Xero, MYOB and QuickBooks reuse the accepted handoff", () => {
  ...
  assert.match(accountingUi, /"Sync to " \+ providerLabel/);
});
...
test("the office flow is progressive and exposes one commercial timeline", () => {
  ...
  for (const copy of ["Invoice preview", "Export preview", "Sync this invoice", "Accounting system", "TLink does not send a second invoice email"]) assert.match(accountingUi, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
