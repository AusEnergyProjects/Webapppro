...
function harness(responder, component = "TradeTrainingWorkspace", props = {}, runtime = {}) {
  ...
  const require = (id) => id === "react" ? hooks : id === "react/jsx-runtime" ? jsx : id === "@/lib/energy-service-catalogue.mjs" ? { ENERGY_SERVICE_CATALOGUE } : id === "@/lib/training-service-sections.mjs" ? trainingSections : id === "./TrainingServiceGuide" ? { TrainingServiceGuide: () => null } : id.endsWith(".module.css") ? { default: new Proxy({}, { get: (_, key) => String(key) }) } : (() => { throw new Error(`Unexpected client runtime import: ${id}`); })();
