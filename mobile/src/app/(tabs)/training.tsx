...
function TrainingWorkspace({ online }: { online: boolean }) {
  ...
  return <Screen scrollKey={`${selected?.id || 'list'}:${attempt ? `question-${questionIndex}` : result ? 'result' : `lesson-${lessonIndex}`}`}>
    ...
    {!selected && <>
      <View style={styles.card}><View style={styles.row}><Text accessibilityRole="header" style={styles.title}>{data?.officeOnly && !modules.length ? 'Office-only team member' : `${modules.filter(module => module.status === 'passed').length} of ${modules.length} modules passed`}</Text><FieldButton variant="secondary" loading={busy === 'load'} disabled={!online || Boolean(busy)} onPress={() => void refresh()}>Refresh</FieldButton></View>
      ...
      {data && !modules.length && <Text style={styles.body}>{data.officeOnly ? 'No installation training is required for your office-only role. You can book qualified technicians if your existing account permissions allow it. If you also do work on site, the business owner must select your services in Team so the relevant training can be assigned.' : 'No modules are assigned to your work types and service locations. Ask the business owner to check business service selections and your Team capabilities. An empty list does not approve program work.'}</Text>}
